...
| ID | 场景 | 通过标准 | 状态 |
|---|---|---|---|
| BE-B5-001 | 22 个 AI 节点逐项使用主应用模型 ID | 不读取第二套配置；请求与模型选择一致 | 部分通过：`ai_chat`、8 个文本任务、两个视觉节点、两个 AI 网页理解节点、三个 Firecrawl 网页数据节点、两个验证码节点及两个 AI 媒体节点共 20 节点已迁入；需要模型的 15 节点统一使用主应用 `modelId` 和系统凭据，Firecrawl 与验证码节点统一复用运行中的 CloakBrowser Profile。12 个模型节点已通过本地受控 HTTP 真实 worker；Firecrawl 与验证码节点已在 darwin-arm64 的 CloakBrowser 151 对本地受控站点真实执行，OCR 本地模型也已进入冻结后端；视觉、网页及媒体节点仍需集中正式 UI 验收。其余 2 节点待迁入。[模型边界证据](studio-backend-migration/evidence/b5/model-boundary-ai-chat.json)、[文本任务族](studio-backend-migration/evidence/b5/ai-text-task-family.json)、[视觉节点](studio-backend-migration/evidence/b5/ai-vision.json)、[视觉操作](studio-backend-migration/evidence/b5/ai-vision-act.json)、[网页理解节点](studio-backend-migration/evidence/b5/ai-scraper-family.json)、[Firecrawl网页数据族](studio-backend-migration/evidence/b5/firecrawl-family.json)、[验证码节点族](studio-backend-migration/evidence/b5/captcha-family.json)、[AI媒体节点族](studio-backend-migration/evidence/b5/ai-media-family.json) |
| BE-B5-002 | 流式响应、取消、超时、供应商错误、限流和大结果 | 状态、错误和结果引用符合合同；取消不继续写结果 | 尚未验收 |
| BE-B5-003 | MCP 保存、测试、重载、调用、拒绝和断线 | 修订与命令幂等；权限结果不被绕过 | 尚未验收 |
| BE-B5-004 | API key、代理密码和 License | 不进入快照、事件、日志、诊断或导出 | 部分通过：需要模型的 15 个已迁入 AI 节点仅从系统 CredentialStore 解析模型密钥并经 worker stdin 传递；AI 媒体节点已删除节点级密钥和地址输入，生成文件经 artifact 边界落盘；Firecrawl 三节点没有第二套凭据或直接 HTTP 客户端，继承 CloakBrowser Profile 的网络边界。前端已清除旧 AI 网页节点内嵌的模型地址和密钥字段，受控 worker 秘密扫描通过。全 B5 日志、诊断、导出与 LangGraph checkpoint 扫描待完成。[模型边界证据](studio-backend-migration/evidence/b5/model-boundary-ai-chat.json)、[文本任务族](studio-backend-migration/evidence/b5/ai-text-task-family.json)、[视觉节点](studio-backend-migration/evidence/b5/ai-vision.json)、[视觉操作](studio-backend-migration/evidence/b5/ai-vision-act.json)、[网页理解节点](studio-backend-migration/evidence/b5/ai-scraper-family.json)、[Firecrawl网页数据族](studio-backend-migration/evidence/b5/firecrawl-family.json)、[AI媒体节点族](studio-backend-migration/evidence/b5/ai-media-family.json) |
