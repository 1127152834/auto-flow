...
| ID | 场景 | 通过标准 | 状态 |
|---|---|---|---|
| BE-B4-001 | 88 个节点的默认值、类型、空值、错误和独有分支 | 所有逐节点 source-parity/contract/real-execution 通过 | 部分通过：`data_structure.py` 的 15 个、数学/统计的 31 个及 `utility_tools.py` 的 9 个批准节点均已完成冻结源码差分、正式注册、HTTP 准入和无浏览器 worker 执行，共 55 个；未批准的文件比较、目录比较和打印节点没有混入。正式 Electron 开发入口已实测数据结构 5 节点与数学/统计 4 节点，其余 79 个节点的逐节点 UI 尚未完成；剩余 33 个 B4 节点仍待生产迁入。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json) |
| BE-B4-002 | 字符串、正则、JSON、列表、字典、数学和统计边界 | 与原 type_utils/safe_expr/json_safe 行为一致 | 部分通过：基础列表、字典、字符串、正则、数学、统计及 9 个实用工具节点的默认值、字段别名、变量变化和原版错误已与冻结源对照；非 JSON 数值在事件边界结构化失败，排列组合及随机密码增加有记录的容量保护。高级列表/字典表达式、CSV 与表格仍待关闭。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json) |
| BE-B4-003 | 表格/CSV 的中文、空单元格、大列表和文件往返 | 值、顺序、编码和哈希一致，不静默截断 | 尚未验收 |
| BE-B4-004 | 无效编码、NaN/Infinity、超大索引和错误类型 | 结构化失败且后续策略按配置执行 | 部分通过：`list_export` 非法编码、data_structure 节点索引及原版异常文本已通过差分；数学族的 NaN、Infinity 和复数已在事件边界结构化失败，排列组合超容量在计算前拒绝；哈希/URL 编码、无效时间戳及 HEX 格式错误已按冻结源验证；其余 B4 模块待迁入。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json) |
| BE-B4-005 | 运行中取消 CPU 密集或大序列化任务 | 在定义的检查点响应；无半份产物 | 部分通过：列表导出的逐项序列化、旧文件复制和提交前均有取消检查；复制中取消保留原文件且无产物或临时文件。数学/统计大列表与随机生成已使用生产取消令牌验证，排列组合在同步计算前协作检查容量；随机密码每 4096 项协作取消且结果限制为 1 MiB。其它 C 扩展内部计算的细粒度中断及其余 B4 节点仍待验证。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json) |
