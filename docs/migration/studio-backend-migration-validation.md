...
| ID | 场景 | 通过标准 | 状态 |
|---|---|---|---|
| BE-B5-001 | 22 个 AI 节点逐项使用主应用模型 ID | 不读取第二套配置；请求与模型选择一致 | 部分通过：`ai_chat`、8 个文本任务、`ai_vision` 及 `ai_vision_act` 共 11 节点已使用主应用 `modelId`、系统凭据和 worker 模型网关；冻结源码差分及本地受控 HTTP 真实 worker 已通过。其余 11 节点及正式 UI 待验收。[模型边界证据](studio-backend-migration/evidence/b5/model-boundary-ai-chat.json)、[文本任务族](studio-backend-migration/evidence/b5/ai-text-task-family.json)、[视觉节点](studio-backend-migration/evidence/b5/ai-vision.json)、[视觉操作](studio-backend-migration/evidence/b5/ai-vision-act.json) |
| BE-B5-002 | 流式响应、取消、超时、供应商错误、限流和大结果 | 状态、错误和结果引用符合合同；取消不继续写结果 | 尚未验收 |
| BE-B5-003 | MCP 保存、测试、重载、调用、拒绝和断线 | 修订与命令幂等；权限结果不被绕过 | 尚未验收 |
| BE-B5-004 | API key、代理密码和 License | 不进入快照、事件、日志、诊断或导出 | 部分通过：已迁入的 11 个 AI 节点仅从系统 CredentialStore 解析模型密钥并经 worker stdin 传递；受控 worker 的运行快照、事件及节点结果秘密扫描通过。全 B5 日志、诊断、导出与 LangGraph checkpoint 扫描待完成。[模型边界证据](studio-backend-migration/evidence/b5/model-boundary-ai-chat.json)、[文本任务族](studio-backend-migration/evidence/b5/ai-text-task-family.json)、[视觉节点](studio-backend-migration/evidence/b5/ai-vision.json)、[视觉操作](studio-backend-migration/evidence/b5/ai-vision-act.json) |
