...
| ID | 场景 | 通过标准 | 状态 |
|---|---|---|---|
| BE-B5-001 | 22 个 AI 节点逐项使用主应用模型 ID | 不读取第二套配置；请求与模型选择一致 | 部分通过：主应用 `modelId`、系统凭据、父进程运行绑定和 worker 模型网关已贯通；`ai_chat` 通过本地受控 HTTP 服务真实 worker 请求与结果验证，其余 21 节点及正式 UI 待验收。[模型边界证据](studio-backend-migration/evidence/b5/model-boundary-ai-chat.json) |
| BE-B5-002 | 流式响应、取消、超时、供应商错误、限流和大结果 | 状态、错误和结果引用符合合同；取消不继续写结果 | 尚未验收 |
| BE-B5-003 | MCP 保存、测试、重载、调用、拒绝和断线 | 修订与命令幂等；权限结果不被绕过 | 尚未验收 |
| BE-B5-004 | API key、代理密码和 License | 不进入快照、事件、日志、诊断或导出 | 部分通过：`ai_chat` 的模型密钥仅从系统 CredentialStore 解析并经 worker stdin 传递；运行快照、事件及节点结果秘密扫描通过。全 B5 日志、诊断、导出与 LangGraph checkpoint 扫描待完成。[模型边界证据](studio-backend-migration/evidence/b5/model-boundary-ai-chat.json) |
