...
| BE-B1-008 | 杀 worker、杀浏览器、异常退出 sidecar | 运行标记 interrupted/failed，网页动作不重放，进程和锁最终可回收 | 进程集成 | 部分通过：真实受管 worker 非零退出时先清理子进程树和临时目录，再把运行标为 `failed` 并释放资源；新 sidecar 启动将遗留活跃记录幂等标为 `interrupted`，不重放 worker 或节点事件，活跃槽可复用。浏览器单独崩溃及正式 Electron/sidecar 异常组合仍待验收。[生命周期恢复证据](studio-backend-migration/evidence/b1/lifecycle-resource-recovery-2026-09-16.json)、[browser-worker.json](studio-backend-migration/evidence/b1/browser-worker.json) |
...
| ID | 场景 | 通过标准 | 状态 |
|---|---|---|---|
| BE-B4-001 | 88 个节点的默认值、类型、空值、错误和独有分支 | 所有逐节点 source-parity/contract/real-execution 通过 | 部分通过：基础数据 15 个、数学/统计 31 个、实用工具 9 个及高级列表/字典/CSV 26 个批准节点均已完成冻结源码差分、正式注册、HTTP 准入和无浏览器 worker 执行，共 81 个；未批准的文件比较、目录比较和打印节点没有混入。正式 Electron 开发入口已实测数据结构 5 节点与数学/统计 4 节点，其余 79 个节点的逐节点 UI 尚未完成；剩余 7 个表格节点已完成独立源码差分，生产集成仍待 Excel 二进制产物端口。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json)、[高级数据证据](studio-backend-migration/evidence/b4/advanced-data-family.json) |
| BE-B4-002 | 字符串、正则、JSON、列表、字典、数学和统计边界 | 与原 type_utils/safe_expr/json_safe 行为一致 | 部分通过：基础及高级列表、字典、字符串、正则、数学、统计、CSV 与 9 个实用工具节点的正常和错误语义已与冻结源对照；表达式沿用冻结 AST 白名单，并增加容量、取消、凭据错误和非法枚举保护。表格节点的生产集成仍待关闭。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json)、[高级数据证据](studio-backend-migration/evidence/b4/advanced-data-family.json) |
| BE-B4-003 | 表格/CSV 的中文、空单元格、大列表和文件往返 | 值、顺序、编码和哈希一致，不静默截断 | 尚未验收 |
| BE-B4-004 | 无效编码、NaN/Infinity、超大索引和错误类型 | 结构化失败且后续策略按配置执行 | 部分通过：`list_export` 非法编码、data_structure 节点索引及原版异常文本已通过差分；数学族的 NaN、Infinity 和复数已在事件边界结构化失败，排列组合超容量在计算前拒绝；哈希/URL 编码、无效时间戳及 HEX 格式错误已按冻结源验证；高级数据的非 JSON 结果、递归/循环引用、容量及未知枚举也已明确失败；表格模块待生产集成。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json)、[高级数据证据](studio-backend-migration/evidence/b4/advanced-data-family.json) |
| BE-B4-005 | 运行中取消 CPU 密集或大序列化任务 | 在定义的检查点响应；无半份产物 | 部分通过：列表导出的逐项序列化、旧文件复制和提交前均有取消检查；复制中取消保留原文件且无产物或临时文件。数学/统计大列表与随机生成已使用生产取消令牌验证，排列组合在同步计算前协作检查容量；随机密码每 4096 项协作取消且结果限制为 1 MiB。高级数据长循环每 256 项协作取消，并在统一变量提交边界再次检查；受容量限制的批量 C 操作不会在取消后提交。表格模块仍待生产集成。[数据结构证据](studio-backend-migration/evidence/b4/data-structure-family.json)、[数学统计证据](studio-backend-migration/evidence/b4/math-number-statistics-family.json)、[实用工具证据](studio-backend-migration/evidence/b4/utility-tools-family.json)、[高级数据证据](studio-backend-migration/evidence/b4/advanced-data-family.json) |
