...
运行页使用真实 API；截图是工作流产物，手动输入由独立 scrcpy 窗口处理。设备页提供状态查看和“打开操作窗口”直接手动操作入口，创建仍用准备命令。没有引入新的批量调度器。
...
## 设备页打开按钮修复（2026-09-13）

用户截图指出设备卡片点击无反应：原卡片只有状态文本，没有事件处理。新增明确的“打开操作窗口”按钮；无需进入工作台即可启动单人工节点会话并开原生窗口。会话区提供“结束操作”和“停止并清理”。关窗保留会话，可重新打开；最长 60 分钟，结束后保留 Android/数据。

页面只消费此次点击产生的开窗意图，刷新不会重复启动或重复开窗；未知状态、其他运行占用或环境不可用时禁用新开按钮。继续使用原有 API、运行独占和持久化回执，未另建后端执行器。

修复验收：React 全量 418 项通过；类型检查、ESLint、OpenAPI check、前端构建和 Mac 打包通过。CUA 实际点击设备卡片按钮打开原生窗口，关窗后仍可结束；结束后运行 succeeded，唯一完成节点为 device-manual，设备 idle 且 owner 为空。[证据](android-workflow-handoff-qa/device-open/result.json)。
