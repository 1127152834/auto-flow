...
真实浏览器核对了：跨域iframe中逐轮断点和修改值，结果为modified/second；重复单步不会多执行；定位失败保留浏览器，结束后仍failed；顶层直跑跳过前置，手动导航后执行；70,000字符变量（超过64KiB）文件及导出完整；1000轮追加结果长度和末项均为1000；嵌套运行至此及纯变量暂停/停止；双层循环每次单步只增加一次调度，条件未选路径不产生点击；真实SIGKILL sidecar后清理受管浏览器，恢复为interrupted且不重放，可再次取得资源。
