...
- 嵌套组合修复：`7a4a10a0` 保留自定义模块内调用子工作流的完整作用域，子节点应显示“模块 → 工作流”；真实项目 worker 用例验证输出 42、作用域和清理。关联项目 29 项、Studio 嵌套/模块针对性 6 项、节点执行器 3 项通过。重新冻结后，正式 macOS arm64 包 [`formal-project-control-electron-h1SA0V/result.json`](../formal-project-control-electron-h1SA0V/result.json) 重跑核心项目 UI 主链通过；冻结后端 SHA-256 为 `dd7bccfd89563f24189544856c0c74b7aac5efe8ef3d56eff9fe66fbc7d8c132`，`AutoFlow` 与 `app.asar` 哈希同上。组合本身由真实 worker 集成用例验收，包内脚本仅覆盖两类入口各自的主链，不能冒称包内 UI 已直接编排该组合。
- 变量引用：同一真实项目 worker 用例分别使用直接工作流 ID 与声明初值 `{child_ref}`，两项均通过；引用解析、冻结内容、变量回收和清理相同。运行中才产生的引用无法在当前准备阶段静态冻结，尚未核销，不能据此宣称该节点的动态引用分支完整。
