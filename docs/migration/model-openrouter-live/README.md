...
| baseline 合并后重启本地服务 | 设置页重启完成，原供应商、两个模型及系统凭据保留 |
| 已保存供应商改为错误 Key | 显示“供应商认证失败，请检查 API Key”，保存被拒；[截图](02-invalid-key-rejected.jpg) |
| 关闭失败编辑后测试原连接 | 成功，445 个模型、约 1319 ms；[截图](03-original-key-still-valid.jpg) |
| 同步并添加／选择目录模型 | 自动填写 `OpenAI: GPT-4o-mini` 与 `128000`；取消草稿后仍仅两个原有模型；[截图](04-catalog-metadata.jpg) |
...
本轮由本线程发起的短生成共 5 次：桌面 3 次（2 次免费模型 429、1 次 GPT-4o mini 成功），隔离脚本 2 次（均成功）。不据此推算账单金额。
...
- 隔离脚本的服务重建与实际 Electron 设置页“重启服务”均已验证；完整 Electron 进程退出重开、Windows 原生凭据存储和安装包验收仍未覆盖。
