...
`inventory-studio-services.mjs` 从当前 Studio 源码登记每个服务操作、直接请求和事件入口，生成 `service-inventory.json` 与 `contract-matrix.md`。本次扫描登记 176 个服务方法、84 个事件、52 个直接请求和 105 个 AI 画布操作；26 个服务族全部进入现有矩阵，没有未分类的新服务族。生成器会沿同文件顶层辅助函数解析实际请求，因此 MCP 保存/重连、平台动作状态等封装入口仍保留明确方法和端点，不会因复用校验函数而退化成空合同。
...
| `inputPromptApi`、`jsScriptApi`、`speechApi`、`desktopActionApi` | 运行中的输入、脚本、语音和平台动作命令回传 | `input-command-protocol`、`js-command-protocol`、`speech-command-protocol` 及平台动作专项；同 ID 重试、所有权、停止竞争和迟到响应均覆盖 |
...
| `aiAssistantApi`、`modelApi`、`projectResourceApi` | 会话、模型配置、连接测试、助手请求，以及项目默认 Profile/模型的读取与显式会话覆盖 | `project-resource-defaults`、`project-model-defaults`、`project-model-protocol`、F5.3 模型设置、AI 权限、取消及连接成功/失败证据；项目默认读取失败不静默退回全局首项，真实模型执行单列后端交接 |
...
- 服务清单脚本：8/8 通过，包含确定性生成、multipart、动态方法、辅助函数内请求、直接请求、普通事件和全部 26 个服务族登记。
- OpenAPI 一致性：通过。
