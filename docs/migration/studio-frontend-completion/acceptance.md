...
本次会话离开保护：新建/打开/导入共用保存后清理入口，取消和保存失败不停止，清理失败保留文档，录制尾部与来源保留。全量292文件3492项通过，类型/lint/build通过，证据 `evidence/f5-session-leave/`。该验证触发于共享离开边界变更，后续无新风险不重复全量。正式宿主/运行会话仍归 F5.1/F5.2，不扩大本次通过范围。
...
## 2026-09-14：语音确认与在途请求重连

语音请求领取、结果确认、停止及响应丢失查询已补齐；普通SSE断流手动重连不再丢弃在途朗读/脚本。63项专项、157文件1884项全量通过，真实原生浏览器结束回传已核验，见speech-request-validation.md。真实后端、进程重启epoch及正式宿主仍未验收。

## 2026-09-14：代码编辑器复制保护

三个编辑器复制请求的失败、重复点击、迟到回执和定时器清理已修复，新增15项及联合25项通过，见editor-clipboard-validation.md。浏览器成功回执已观察，但系统剪贴板内容核对受工具虚拟剪贴板限制，明确保留该E2E缺口。

## 2026-09-14：启动占用和编辑文档绑定

接受后等待匹配事件确认，响应丢失不重复启动；可停止待确认请求；服务流程缓存绑定原文档。专项46+6及完整复核159文件1914项通过，见start-admission-validation.md。保留首次全量2项失败及未改断言复现记录；正式runId/重启/宿主协调仍未完成。

## 2026-09-14：最新有界交付与剩余门槛

- 最近全量：264文件、3036项通过，484.00秒，见request-connection-binding.md。其后录制信封、资源面板及写回执/并发有专项证据，不能将新增数直接当作重跑后的全量数。
- 已新增：共享图像元数据样例、路径选择合同、WebDAV读取/保存保护、录制确认游标及类型信封、请求发起连接绑定、资源面板读取/写回执/重命名并发保护。见本目录对应专项文档。
- 实际浏览器UI：录制3步、离线失败、恢复停止、整批撤销重做、保存刷新重开；资源离线重试。服务均为Mock，未验证真实网页自动化。
- F0仍需逐节点全部字段/分支细化；F1仍需完整动作载荷、独立运行身份及全部服务合同；F2仍需全部284节点配置工具和容量验收；F3已有pauseId/controlRevision局部合同，完整运行历史/变量命令及恢复仍未完成；F4录制/拾取全局所有权、持久恢复及完整会话清理仍未完成；F5宿主离开、工作区协调、权限未收口；F6正式Electron及打包/跨平台未验收。
- 当前产品仅服务中文，不引入i18n，也不再把英文翻译覆盖作为验收门槛。上文英文翻译补充是早期历史记录，不能用于推导当前范围。浏览器语言/时区仍是保留的业务参数。
...
## F5 凭据/WebDAV 离开保护（2026-09-14）

手动切标签、关闭按钮、遮罩和完成保存先处理服务表单草稿。34 项新组件用例及 3 条浏览器验证链已登记；关联组件回归 60 项通过。全量 272 文件/3,217 用例通过后又修复 WebDAV 确认期间换连接误投递，最后补丁由针对性回归验证；不把旧全量统计当作补丁后全量。详见 settings-leave-validation.md。AI 关闭、MCP 与正式宿主仍有缺口，F0–F6 未整体关闭。

## F5 AI 配置关闭（2026-09-14）

AI 关闭等待保存/放弃/取消结果，未确认不报成功；补齐确认弹窗卸载/替换/晚调用取消。13 项新增测试，关联回归 81 项、类型/lint/49 脚本/构建通过。见 settings-assistant-close.md；不是正式模型、Electron 或本批全量回归证据。

## F5 MCP 编辑与连接（2026-09-14）

34 项新增连接/离开用例，关联回归 141 项通过；浏览器实点保存/取消/回显并恢复测试目录。类型/lint/49 脚本/构建通过。见 mcp-leave-validation.md；不代表真实 MCP 执行、权限矩阵、正式宿主或本批全量回归。

## F5 安全归属与留存（2026-09-14）

安全批次全量 277 文件/3271 通过，后端契约 412 通过。随后留存批次新增 74 前端和 18 Python 用例，关联 117 前端通过；两批证据分开记录。留存浏览器保存重开/错误保护/Mock 清理交互通过，详见 security-host-ownership.md 与 retention-validation.md。没有执行真实清理，F0–F6 仍未整体完成。

## F2/F5 文档连接隔离（2026-09-14）

新增16项用例，相关51通过；保存/打开浏览器链通过，280文件3361项全量通过（498.60秒），包含前批留存改动。详见 document-connection-validation.md。已发现窄工具栏越界，下一批修复；不宣称正式宿主、工作区事务及全里程碑完成。

## F2/F6 响应式与500节点编辑（2026-09-14）

工具栏/模块条窄窗口越界、误点与遮挡已按实际浏览器发现修复；500节点导入、末尾编辑、保存刷新重开通过。4新规则测试及相关68通过，类型/lint/脚本/构建通过。见 layout-capacity-validation.md，工具读取超时与未测性能如实保留；1,000执行/10,000日志及正式宿主仍未通过整体门槛。

## F3 独立运行身份与日志历史（2026-09-14）

稳定 runId、重复启动幂等、同流程多轮隔离、10,000 条服务端分页/筛选/JSONL 导出、最近运行选择及刷新恢复已实现，见 execution-log-history-validation.md。实际浏览器完成生成、分页、筛选和刷新恢复；内置浏览器 Blob 下载事件未被观测，正式 Electron 下载仍待 F6 验收。该批不代表真实后端执行或 F3 整体完成。

## F3 调试运行身份与暂停变量修改（2026-09-14）

暂停控制已绑定 `runId + pauseId + controlRevision`；继续、单步和变量修改使用可查询的幂等命令。暂停变量支持 JSON 原子批量修改、来源显示和循环局部只读；丢响应不会重复提交。专项前端 64 项、后端合同 19 项通过，详见 debug-run-variable-validation.md。正式 worker、原生 Electron 和真实浏览器调试仍未由本切片验证，F3 及 F0–F6 整体保持未完成。
