...
F2.3 前端实现与组件/Store/Mock 合同验收已经核销。原生文件入口与正式 Electron 保存主链留在 F6；真实自定义模块持久化、事务和执行留在后端交接，不再计作 F2.3 前端缺口。
...
- 两视图复杂结构、分组/便签/子流程以及自定义模块列表和依赖整包已经完成专项。实际 `BlockFlowView` 插入、重排、删除，嵌套结构往返与复制，分组多节点移动，便签和子流程历史/保存恢复均通过；模块删除/收藏失败可见，嵌套依赖递归导出且缺失依赖阻止导出。见 `evidence/f2-complex-document/validation.md`。
