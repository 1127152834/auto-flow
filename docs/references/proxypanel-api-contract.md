...
`CredentialStore` 已在 `codex/proxy-management` 分支实现。端口位于 `domain/credentials.py`，平台实现位于 `infrastructure/credentials/`；SQLite 只存 secret_ref，sidecar 应用服务才解析明文。凭据库不可用时返回明确错误，禁止明文文件降级。
...
Key 请求字段为 writeOnly/SecretStr 语义；Pydantic/FastAPI 校验错误必须去掉敏感 input，防止自动 422 响应或验证日志回显 Key。上述 host token、IPC 与凭据库已在代理独立分支实现并通过本机隔离测试；浏览器主线整合与跨平台实际验证状态见 `docs/migration/proxy-management-status.md`。
