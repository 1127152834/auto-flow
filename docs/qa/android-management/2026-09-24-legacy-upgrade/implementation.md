...
- 日期：2026-09-24；状态：confirmed（RED/GREEN、Android 回归及全后端验证完成）。
...
全后端唯一运行：`uv run pytest -q --tb=short > /tmp/autoflow-legacy-replay-full-pytest.log 2>&1`；实际结果 `4124 passed, 26 skipped, 2 warnings in 780.28s (0:13:00)`，退出码 0。完整原始输出已复制至同目录 `legacy-replay-full-pytest.log`。警告仍为上述两项，无测试失败。运行结束重新核对以下 SHA-256，与开跑时一致；对应产品提交为 `0f6f8fac`。没有重复启动全量运行。
