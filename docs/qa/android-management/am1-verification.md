...
审计基线：`432d1cdc`；来源：[总体验收记录](2026-09-22-validation.md)。计划 T01–T07 的 35 个 checkbox 已附状态；状态以自动化证据和真实条件逐项判定。
...
| T04 持久操作与迁移 | `passed` | `test_android_management_operations.py` 与契约测试覆盖幂等、摘要冲突、状态栅栏、needs_verification、compact；新增 `transition_with_device` 以同一 SQLAlchemy 事务提交操作状态和设备投影；迁移 head 为 `am01_management_operations`，父节点为当前 `0019_recording_commands`。旧 1000 条回执上限已移除，Android 聚焦集合 `175 passed, 1 warning`。 |
| T05 控制会话 | `blocked` | ConsoleController、heartbeat 和 generation 身份测试存在；原生 scrcpy 窗口、切换写端、30 秒失联、重启后会话和中断恢复没有完整真实证据。 |
| T06 管理首页 | `blocked` | AndroidPage/ManagementOverview 现使用 management devices 快照，不再轮询旧 `/api/v1/android/devices`；前端 Android 聚焦 `13 files, 47 passed`。计划指定的长名称/缩放/键盘/断线/保留数据完整流程无证据。 |
