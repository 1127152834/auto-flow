...
- AM2镜像登记/拉取/删除、固定镜像身份、模板revision和快照已实现；[AM2证据](am2-verification.md)。[真实未知拉取](2026-09-24-pull-disk-and-bulk.md)已验证重启发现、核实前禁新请求、按原编号核实；实际pull仅一次。另补[真实下载中断与恢复](2026-09-24-network-interruption.md)：独立Docker/containerd中3MiB真实暂存文件后截断连接，失败回执/重启/同编号重放保持，恢复网络后新编号成功拉取，测试镜像和守护进程清理完成，外部容器/镜像未变。
