...
| <a id="t14"></a>T14 | `not_run` | [五台/双预览](2026-09-23-final-review-remediation.md)通过；[1/5台后台观察](2026-09-24-observation.md)已记录计数/间隔/停机降频，隐藏页批次GET暂停已补RED→GREEN；桌面隐藏页面同链与前台延迟仍not_run；早前解锁记录已被后续锁屏状态取代，最新真实桌面验证blocked。 |
...
| <a id="t16"></a>T16 | `not_run` / 十台 `blocked` | 1/5 台 API、双实例隔离有证据；[7921 MiB < 8192 MiB](2026-09-24-advanced-logs-and-capacity.md)阻塞十台，[1/5台API/内存/后台计数](2026-09-24-observation.md)已补；真实桌面前台/隐藏页仍not_run；Mac再次锁屏，最新真实桌面验证blocked。 |
...
最终复审续行：产品78bf8c92后端4118/前端5689通过，五项Important均独立复审关闭，见[当前最终验收](2026-09-24-final-acceptance.md)。上述旧门禁数均为历史快照。权限拒绝扩展故障passed不改变85/14/3原步骤计数。

旧版本兼容补验续行：0f6f8fac修复真实升级后的历史批次重放409，独立复审无C/I/Minor；后端全量4124passed/26skipped/2warnings（780.28秒），定向540passed；前端1050文件与此前5689passed候选hash一致。本次真实升级及权限拒绝不追认历史RED，不改变85/14/3计数。
