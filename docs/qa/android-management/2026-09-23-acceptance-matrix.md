...
当前软件或验证缺口是 旧temporary历史实例真实兼容同链（契约已验）、T09镜像内容删除桌面链和拉取断线核实、T12阶段停用与向前回退演练，T14/T16 的后台探测指标与桌面前台交互，T20 的完整真实负向矩阵；无持久归属标记的旧卷文件继续按安全边界排除，不能把它们视作可自动删除对象。十台实测因本机 Lima 内存预算不足标记 `blocked`，不计软件通过。外部阻塞主要为 AM-AC14 的专用谷歌镜像与账号/网络条件；本地测试 APK 已用于自建实例，不代表商店下载链。真实备份与恢复实验只清理本轮自建资源，已有外部卷和实例不纳入删除范围。
