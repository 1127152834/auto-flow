...
## C2c 前端scope到传输身份投影（2026-09-13）

controller首次真实consumer携workspaceKey传入CatalogScope，原API spread把该UI身份写入recordRef/fieldRef，真实后端extra=forbid导致预检422。两API改为显式取projectId/tableId/datasetGeneration；既有作用域隔离保留在controller，线上的身份只含契约字段。root两项RED2fail61pass→修复后63APItests/ESLint通过。pm2_table_editing_controller_review独立捕获真实controller的两个预检body，交Python严格DTO校验并验证roundtrip全等，2项通过，可独立提交；controller其他会话问题继续修复。
