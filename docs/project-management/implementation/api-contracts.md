...
`202 Accepted` 只表示已持久接受，不表示成功。同步完成的短命令可直接返回 200/201 的最终资源；若同一操作已存在，重复请求按现状返回 200（终态）或 202（非终态）。首次响应丢失时，客户端以原 `Idempotency-Key` 重发原命令，或用 §3.7 的同作用域查询路由找回；创建项目尚无 projectId，因此使用 workspace 级找回路由。不得生成新 key 盲重试。`Operation.result` 按 `kind` 使用本文定义的结果类型，不使用未定义字典。
...
type ProjectSummary = Project & {automationCount:number; tableCount:number; activeBatchCount:number; manualCount:number; attentionCount:number}
...
type ProjectOverview = {project:Project; counts:{automations?:number;tables?:number;batches?:number;environments?:number}; availability:Record<'automations'|'data'|'runs'|'environments'|'sync',Availability>; activity:AttentionItem[]; recent:ActivityItem[]}
