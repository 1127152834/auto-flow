...
2026-09-24 并行补证/CI诊断（confirmed局部）：ARM正式打包API通过2/3次循环变量隔离与精确五条写入、唯一End声明输出关联、并行人工串行交接和停止保留前写。公开事件不提供内部scope、同workflow重复绑定两次脚本错误已修正并保留失败报告。当前69baeeb2三平台35886627514的ARM在打包必填提示失败，截图节点未选中，metadata HTTP断言和后端3486/前端5473通过；旧点击helper对覆盖目标误点已由隔离Electron复现并修复，102脚本/4映射及完整ARM打包桌面通过。最终三平台均失败：Intel同为未选中节点，Windows为紧凑重开窗口隐藏就绪文字。恢复视口及显式展开面板已复现通过，最终完整新版桌面通过；全部旧job结束后准备一次三平台后续。详见parallel-packaged-follow-through.json和ci-metadata-click-follow-through.json。251条状态未升级，releaseAccepted=false。
...
2026-09-23 S4 最终候选复验（confirmed，覆盖前段“待最终候选”状态）：[Actions 35833223498](https://github.com/1127152834/auto-flow/actions/runs/35833223498) @dadb24a1 的 Windows x64、macOS Intel、Apple Silicon 三个 checks 作业全部成功。Windows 原生边界 251 passed/28 skipped，完整后端 3469 passed/85 skipped；两种 Mac 后端各 3477 passed/77 skipped；三平台前端各 5471 passed，选定真实 worker 各 29 passed/11 deselected。源码、打包 sidecar、桌面 smoke 和 Windows NSIS / Intel、ARM DMG 构建均通过。独立 native/worker probe 作业在该矩阵 skipped，Windows 原生专项另由 35832925704 证明。详情及实际负载见 [当前 CI 报告](ci-s4-final-follow-through.json)。

本次 Windows 打包桌面五路 HTTP 写入 10000 行耗时 834889 ms、1 次 busy 重试；真实 worker 1004 条日志耗时 128882 ms，报告观察值 467 条/分钟。固定 1000 条合成输入耗时 60120 ms、最大批次滞后 143 ms，只证明该合成链通过，不等于真实 worker 每分钟 1000 条。既有文件覆盖/追加仍 501，[S4 补充规格](../../../superpowers/specs/2026-09-23-pm9-windows-existing-file-replacement-boundary.md) 和 [待决计划](../../../superpowers/plans/2026-09-23-pm9-windows-existing-output-options.md) 仅为 proposed。L1–L3、M1–M3 仍是未批准实现缺口；当前打包 Sheets/OAuth 实网、系统 UUID/远端增列实网、物理安装、原生面板/凭据、签名公证需要外部条件。PM6 历史服务账号 Sheets 实网读写与系统凭据证据不被否定。251 条台账仍为 248 条有范围断言、3 条未定位，204 partial/47 planned/0 verified；`releaseAccepted=false`，草稿 PR 不合并、不发布。

同日当前 ARM CI DMG 本机补证（confirmed）：从 35833223498 下载的 `AutoFlow-0.1.0-arm64.dmg` SHA256 为 `61b6ff8b20058acf231db8087cffee42f9bff1ace802956a156967761d0a7d3f`，`hdiutil verify` 通过；只读挂载、`ditto` 复制到隔离安装目录、卸载镜像后从复制应用运行原版完整桌面脚本成功。公开版 145 内核归档 SHA256 与发布清单一致。真实浏览器 worker、人工声明输入、六页、归档/恢复、五路 10000 行（53884 ms/0 busy）、1004 条真实 worker 日志（32709 ms，观察 1842/min）、固定 1000 条合成输入（60017 ms，最大批次滞后 25 ms）、200% 缩放、第二窗口及应用重启均通过。两次前置失败分别为本机 Pro 内核与公开版 profile 不匹配，以及临时 Pro 配置下 `LICENSE_INVALID`；临时脚本改动已撤回，失败不算通过。详见 [当前 ARM 安装证据](installed-arm-current-candidate.json)。这是一台 Mac 的命令行隔离安装；Finder/Gatekeeper、当前原生面板/凭据、完整导入导出/卸载、签名公证、Windows/Intel 实机和 Google/OAuth 仍未验收，`releaseAccepted=false`。

2026-09-23 DATA-LINK-02 跨项目关联边界（confirmed，本机与新生产源码）：251 条映射文件/符号校验通过；73 条历史预定路径仍缺文件，但 72 条有真实范围断言，XE-G07 明确保留缺测试。直接 HTTP/SQLite 反例发现修复关联时 `recordRef.projectId` 可写成外部项目，而本地 table/generation/key 不变，旧预检仍会把本地行关联成功。共享仓库的读取预检与提交重验都增加项目身份核对。跨项目实例与固定环境 ID 被拒、两本地行加一外部行整组预检不部分提交、伪造项目 ID 修复保持 `saved_unlinked`、正确修复不重存环境，以及并发版本冲突组回滚均有直接断言。契约/规则 33 passed，本机完整后端 3479 passed/77 skipped，Ruff/mypy407 通过；本机真实浏览器 `data-link-race` 1 passed，现含实际 End 保存后的伪造项目身份修复拒绝、原环境保留及正确修复不重跑断言。DATA-LINK-02 从 planned 升为 partial；当前 248/251 有范围断言、3 未定位，205 partial/46 planned/0 verified。新 guard 后续已进入完整后端、三平台 CI 和 ARM 隔离安装；之前 dadb24a1 CI/ARM 安装只保留历史证据。见 [子片报告](link-boundary-follow-through.json)；`releaseAccepted=false`。

2026-09-23 `c16532d7` ARM 打包补证（confirmed，本机限定）：三平台候选 Actions 35846519367 的 ARM job success，后端 3479 passed/77 skipped、前端 5471 passed、选定真实 worker 29 passed/11 deselected，CI 包内桌面报告 passed。当前 DMG SHA256 `bde5e6b263c9cb86fadd3e9f1bc9ca5aa66e83bb7f3283c4901226545bdba40f` 且 `hdiutil verify` VALID；只读挂载、隔离复制和卸载镜像后，复制应用的原版完整桌面脚本 passed。真实 worker 日志 1004 条/38740 ms（本机单次观察约 1555/min）、五路 10000 行 141442 ms/0 busy、固定 60 秒合成日志 1000 条/最大批次滞后 243 ms；这些不是跨平台达标标准。详情见 [当前 ARM 新源码安装报告](installed-arm-link-guard-candidate.json) 和原始 `qa-runs/2026-09-23-arm-link-guard/project-desktop.json`。Windows 当前 job 完整成功；Intel 首次 job 的测试及打包 smoke 通过、DMG 临时磁盘弹出 Resource busy 导致 job 失败，同源码 Intel-only 重跑后完整成功。首轮失败原始报告与日志引用保留。Finder/Gatekeeper、签名/公证、其他平台实机、Google/OAuth 仍待验收；`releaseAccepted=false`。

同日 `c16532d7` 三平台最终候选（confirmed，来源：[Actions 35846519367](https://github.com/1127152834/auto-flow/actions/runs/35846519367) 与[最终报告](ci-link-boundary-final-follow-through.json)）：Windows/ARM 首轮成功，Intel 首轮仅 DMG 临时磁盘 Resource busy 失败；同源码 Intel-only 第二次完整成功。Windows 原生边界 251 passed/28 skipped、完整后端 3471 passed/85 skipped；Mac 两平台后端各 3479 passed/77 skipped；三平台前端各 5471 passed、选定真实 worker 各 29 passed/11 deselected。Windows NSIS、ARM DMG 与 Intel x64 DMG 均构建并上传；Intel DMG 在 ARM 本机校验有效、只读挂载并确认 x86_64 应用，但未在 Intel 实机安装。Windows 本次打包真实 worker 日志 1004 条耗时 169159 ms（观察 356/min），五路 10000 行写入 1033383 ms/7 次 busy；固定 60 秒合成日志 1000 条单独通过，不据此宣称真实 worker 达到 1000/min。独立 native/worker probe 作业 skipped，Windows 原生专项仍引用独立历史运行。251 条台账 248 有范围断言、3 未定位，205 partial/46 planned/0 verified；完整业务、Google/OAuth、物理安装/面板/凭据及签名公证仍待验收，`releaseAccepted=false`。
