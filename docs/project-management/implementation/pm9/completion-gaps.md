...
2026-09-23 DATA-LINK-02 跨项目关联边界（confirmed，本机与新生产源码）：251 条映射文件/符号校验通过；73 条历史预定路径仍缺文件，但 72 条有真实范围断言，XE-G07 明确保留缺测试。直接 HTTP/SQLite 反例发现修复关联时 `recordRef.projectId` 可写成外部项目，而本地 table/generation/key 不变，旧预检仍会把本地行关联成功。共享仓库的读取预检与提交重验都增加项目身份核对。跨项目实例与固定环境 ID 被拒、两本地行加一外部行整组预检不部分提交、伪造项目 ID 修复保持 `saved_unlinked`、正确修复不重存环境，以及并发版本冲突组回滚均有直接断言。契约/规则 33 passed，Ruff/mypy407 通过；本机真实浏览器 `data-link-race` 1 passed。DATA-LINK-02 从 planned 升为 partial；当前 248/251 有范围断言、3 未定位，205 partial/46 planned/0 verified。新 guard 尚未进入完整后端、三平台和安装包，之前 dadb24a1 CI/ARM 安装只保留历史证据。见 [子片报告](link-boundary-follow-through.json)；`releaseAccepted=false`。
