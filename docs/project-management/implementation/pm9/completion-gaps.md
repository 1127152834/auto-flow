...
当前可执行缺口：Windows 已有文件覆盖/追加/安全读取仍拒绝；251 条中 44 条尚未定位直接断言，其余也须逐项闭合生产/UI 子条件。外部条件：当前打包 Sheets/OAuth 授权、签名公证身份及三平台完整实机/原生专项。历史 PM6 Sheets 实网与凭据证据、ARM 安装和 Excel 原生子条件继续有效。releaseAccepted=false，草稿 PR 不合并、不发布。
...
完整 S4 的已存在文件覆盖/追加/读取仍未实现；已验证的新文件发布和有证明 Job 恢复不填补这一缺口。当前 44 条无直接断言是测试台账缺口，不全等于实现缺失；其余项目仍按每条生产、UI、实机条件验收。旧 PM6 实网和历史安装证据不删除，当前专项需要自己的版本与授权证据。
