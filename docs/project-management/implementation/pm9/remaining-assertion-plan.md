...
当前 251 条中 233 条有明确范围的断言，18 条尚无已定位断言；196 partially_verified / 55 planned / 0 verified。这个数量不衡量完成率，任一条的未闭合子条件继续保留。历史 73 条失效预定路径仍保留为原意，实际映射另列，未创建空测试文件冒充交付。
...
## 当前 18 条尚无定位断言：执行分组（2026-09-22，confirmed 现状）
