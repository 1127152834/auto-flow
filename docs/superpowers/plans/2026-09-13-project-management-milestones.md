...
- [ ] **PM1-B 组件与导航上下文**：复用统一控件，完成项目头与六页签路由；资源引用组件随PM3真实消费者交付，影响展示/生命周期弹窗随PM8交付。加载/空/错误/只读/保存中/冲突均走真实状态；尚未接入的能力不提供假操作。
- [ ] **PM1-C 最小生命周期保护**：项目写入校验归属和活动状态；PM1不开放删除、归档、恢复或影响查询。安全删除及依赖检查在PM8真实交付。后续每引入运行/同步/环境，就同步接入 blocker，不能等 PM8 才防止误删。
