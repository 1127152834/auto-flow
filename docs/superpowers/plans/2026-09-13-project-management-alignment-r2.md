> **2026-09-14 当前状态：Gallery R2已完成本机验收，实现a08c2fe。** 原始003–007为基准；下文早期B0/进行中记录保留历史，其未验收说明由本次完成记录替代，不改写历史事实。详见 [R2机器报告](../../project-management/design-alignment/acceptance/gallery-r2/machine-report.json)、[逐步手测](../../project-management/design-alignment/acceptance/gallery-r2/manual-test.md)、[原图并排](../../project-management/design-alignment/acceptance/gallery-r2/comparison.html)。
...
## 2026-09-14 Gallery R2 当前执行卡（进行中，尚未验收）

- 当前基线：`8ccf969`。R1 已完成独立提交、真实端到端、原图对照和审查；用户手测仍未执行。R2 不以旧 B0 图为依据。
- 唯一参考：Gallery 原 003 详情、004 新增、005 编辑校验、006 未保存、007 删除。只调整全局菜单为顶部，保持其余层级与动作关系。
- 文件责任：表单智能体负责 `RecordEditorForm`、`ScalarValueEditor`、`record-draft` 与测试；详情智能体负责 `RecordDetailPage`、`RecordFieldsView`、`RecordStatusDialog` 与测试。主协调负责共享日历、删除与未保存确认、页面/导航装配、QA、锁文件和记录。审查智能体只读核验。
- 复用现有 typed record 路由、HTTP、恢复协调器和受控外链。新增日期视觉采用 `@daypicker/react` 的键盘日历与现有 Radix Popover；文本输入保留异常草稿，不通过 UTC 转换日期，不引入系统原生下拉或日历。参考：<https://daypicker.dev/start>。
- 文本字段当前没有“多行展示”持久元数据。提供真实多行切换并保留换行值；不按“摘要”等显示名称猜字段含义。截图须通过该入口切换，不能注入假表单状态。
- 编辑身份使用只读输入与锁；系统身份新增时明确“保存时自动生成”，不捏造 R019 等业务编号。
- 原 006 显示真实脏字段及未通过校验的字段；原 007 显示真实表/记录与删除影响。保留先检查后确认，不用图中示例引用数量替代后台结果。

顺序与门槛：

- [x] 组件反例：日期、原始值、只读、草稿确认、删除影响先失败。
- [ ] 组件和装配定向绿；规格审查后工程审查；修复所有 P0/P1/P2。
- [ ] 实际 Electron + FastAPI + SQLite 完成新增/详情/编辑/保存/状态/删除/返回。
- [ ] 完成真实冲突、响应丢失、服务/应用重启、旧身份/代次隔离及外链边界测试，标明 UI、API 竞争及故障注入各自来源。
- [ ] 原 003–007 与实图同视口对照；每个画面结构通过且独立评分至少 85；200%/键盘/焦点另验。
- [ ] 保存机器报告、详细步骤/结果、截图、人工操作方案及独立提交，之后才进入 R3。

当前检查不代表 R2 交付。Windows、其他架构、打包和用户手测均未执行。主目录、旧项目及外部记录网格设计任务不修改。
...
## Gallery R2 完成记录（2026-09-14）

- [x] 原003详情分栏，004/005通栏表单及一体底栏，006/007真实确认。
- [x] null/missing/空串/0/false、只读身份、日期精度与偏移；原命令/会话隔离保留。
- [x] 真冲突、丢响应原身份核验、整应用重启、服务重连、双工作区、删除与200%键盘。
- [x] 规格→工程→视觉审查发现问题后修正复验；127文件1030前端、38后端、29脚本、3结构检查及PM2最终回归通过。
- [x] 实现a08c2fe，原图逐页86/88/89/91/88；手测和50张实际截图在gallery-r2。
- [ ] 用户本人手测、Windows/其他架构/打包（未执行，非本机声明）。

按已授权连续执行机制进入R3；不进入PM3。
