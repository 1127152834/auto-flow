...
- [ ] 关闭 `BE-B1-001` 至 `BE-B1-012`，为五节点写入真实证据；B1 不通过则不得扩大 B2。
...
**前置依赖：** B1 关闭；跨 origin、iframe、Shadow DOM、下载、弹窗和网络监听 fixture 可用。
...
**B2 退出门槛：** 35 个网页节点真实行为通过；拾取结果可以由独立运行重放；页面/字段迟到结果不会覆盖新状态；大值、下载、截图与网络产物按引用完整读取。B3 只有在共享定位与会话所有权稳定后启动。
