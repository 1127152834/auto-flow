...
责任边界：PM4-A 负责单次最多 10,000 条记录的有界检测、组合预算及准确阻断，不把预算耗尽误报为数据耗尽；候选游标、继续查找和后续重新调度由 PM4-C 交付，PM4 最终验收前必须消除当前硬阻断。
...
## 11. 2026-09-15 PM4-A 检查点

完整输入选择已接入真实 SQLite 领取和管理端预检：固定记录、三类关联、同表角色回溯、可选输入、有效必要性传播、歧义证据、逐输入错误定位及双重 10,000 上限均已实现。并发领取和领取中途故障验证没有半个 Task；预检后修改数据，启动会重新选择当前事实。

有限审查发现的隐式共享 lease、全局错误污染每一行、错误扫描证据和有效必要性丢失均已修正。详细证据见 `docs/project-management/implementation/pm4/a-verification.md`。候选续查仍按既定责任留在 PM4-C；PM4 最终退出前必须交付。

## 12. 2026-09-16 PM4-B 恢复检查点

现场以 HEAD `48e5919` 和当前未提交源码为准。V1 与 A 不重新实施；B 保留既有 capability、投影、前端和 QA 工作，最近退出目标是先闭合审查缺陷，再运行完整 B 管理链。C 只并行准备不触碰 B 事务文件的失败反例。

| 当前目标 | 真实状态 | 退出证据 |
|---|---|---|
| B 四项审查缺陷 | 当前精确反例已通过：完整 FieldRef、删除引用保护、无初始输入 capability binding、1,000 行/4 MiB 回填预算 | scoped re-review 与相关 B 回归 |
| B 管理侧完整操作链 | 现有历史 B 运行不是本次最终证据 | fresh Electron + FastAPI + SQLite + fake executor 结果、截图及数据库事实 |
| C 调度与恢复 | 15 个测试中 12 个按预期为红、3 个现有保护为绿；尚未开始生产实现 | B 提交后按失败层级逐项实现 |
