...
- [x] 逐条读取 source 需求及实际测试函数断言，先处理 73 条失效预定路径，再检查其余 178 条。
- [x] 验证器只检验映射完整性和引用真实性，不把静态检查称为业务验收：251 个唯一 id；每项有对应检查或明确 test_missing；引用文件和函数真实存在；分类枚举合法；未闭合 gap 不得新晋 verified。
- [x] 使用缺少映射/不存在函数的输入运行失败，再补最小实现与逐项数据；运行 `node --test scripts/verify-pm9-coverage.test.mjs` 和 `node scripts/verify-pm9-coverage.mjs`，预期结构检查通过，业务未关闭项如实输出。
- [x] 提交映射与分类证据，记录没有证明的子条件。
...
- [x] 复用已有 HTTP 创建项目/多表/自动化/正式 Studio 文档；配置人员可重复、邮箱显式改态和多任务，断言实际 Task 的 typed 输入与业务状态，不直接构造 Task/Run 事实。
- [x] 同一真实任务依次写入并在人工等待窗口修改记录，继续后断言本任务版本推进成功而人工新值不被覆盖；后续节点失败仍保留此前写入。
- [x] 在已有 capability 响应边界注入一次已提交响应丢失，用同一 commandId 查询/重发，断言只有一条新增记录与一个操作结果；不注入替身执行器。
- [x] 在实际 End 保存/关联边界制造 linkRevision 冲突，断言 saved_unlinked、Run failed、环境只保存一次；修复只改关联且历史 Run 不变。另断言旧候选不覆盖新代次、混合关联与未经授权替换被拒绝。
- [x] 复用现有人工命令并发测试核对继续/到期唯一转换；增加缺失的真实 worker 情形。每个暴露缺陷先保留 RED，再根因修复并取得 GREEN。
- [x] 运行受影响 pytest/ruff/mypy、脚本回归与本机真实源码 smoke；构建新包后运行同一场景。将实际断言映射回 Task 1，不对未覆盖分支升级。
...
- [x] 子流程：明确冻结依赖图、调用栈/访问身份、输入输出拷贝、取消与能力传播、依赖环和深度拒绝、旧内容兼容与测试。
- [x] 人工：明确声明输入 schema、合法后继/可验证目标、缺项响应、版本/代次/CAS 与 TTL，组件先于页面；不支持跨进程恢复。
- [x] 并行：明确分支状态所有权、循环栈/取消/人工额度、join 与唯一 End、副作用次数和故障回收；证明安全后再开放准入。
- [x] Windows：核对原生安全文件句柄、原子发布与重解析点拒绝、进程 birth/handle 所有权、未知归属保持隔离；每项用原生 CI 验证。
- [x] 对超出已批准 R1–R4 的变更提交完整规格、接口、垂直切片与验证方案后请求一次确认；等待期间完成 Task 1/2/4 中独立工作。确认前不实现新的架构协议。
...
补充（2026-09-21 confirmed）：XE-A10 真实候选发布故障暴露 retained_unsaved/占用遗漏，已完成恢复时原子重获占用、同实例保存串行、条件释放及未知结果保护；18 真实场景通过，最终占用修正后旧候选 1 项与 30 契约/规则复测通过。f25b3867 三平台在旧清理 DTO 类型遗漏处失败，本机记录更正；修复后 typecheck/lint/OpenAPI/100 脚本通过，推送新候选重验。Task 2 打包覆盖仍由新 CI 证明；Task 3 仅规格已完成，实施审批未取得。

最新进度（confirmed）：Task 2 源码/打包扩展场景在当前候选 Windows 与 ARM 全 CI 通过，Intel attempt 2 仍运行；首轮失败保留。Task 4 尚缺 Intel 终态。新增 DATA-E2E-06 手工包验收定位多 Run 单槽限制，已完成独立 specs/2026-09-21-pm9-multi-run-capacity.md 设计、契约和 S5.1–S5.4 切片并单独请求批准；它不是 S3 分支隔离，也不属于此前四片批准范围。当前可执行的顺序组合与原生 ARM Excel 子条件已补，未将完整并发场景升级为完成。

新增有界修复（confirmed）：XE-A23 的 inputEnvironment 资源冻结遗漏已按既有契约补齐，并完成 54 定向/真实链/独立复审，未涉及 S1–S5 架构开放。新打包业务 runner 纳入原 CI；此代码候选需新三平台完整验证，原 Intel 重跑因候选已变更取消。
