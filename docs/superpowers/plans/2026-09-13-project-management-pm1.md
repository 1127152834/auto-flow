...
- 日期：2026-09-13；状态：confirmed / delivered，等待用户PM1验收。
...
| 验收 | scripts/smoke-project-management.mjs、docs/migration/project-management-pm1-qa/、覆盖/账本/.ai；主协调 |
...
- [x] 运行主线后端/前端基线测试并记录结果。
- [x] 将本卡及契约勘误纳入文档，记录本轮PM0验收后进入PM1授权。
- [x] 读取控件1fb58e1及父链，仅接入本阶段使用的Table/Pagination/Search/Select/ScrollArea/反馈及其依赖、tokens和widthfix；修改既有消费者仅为API兼容。
- [x] 跑相关控件测试、类型检查和现有页面测试；规格审查→质量审查→提交。

## 包2：项目事务与HTTP（PM1-A/C后端）

- [x] 在tests/contract/test_projects.py先写下列测试并运行确认404失败；补仓储并发/迁移和领域边界测试。
...
- [x] 实现projects与project_operations两个真实表、事务仓储、用例、10项HTTP；不复用代理专用操作结构，不建后台调度器。
- [x] 注册ORM、pm01迁移及App路由，沿现有错误封装；错误details含domainCode/retryable，revision冲突给最新项目。
- [x] 测试同key/不同key竞争、同名casefold、无变化CAS、非法引用组合、操作跨项目404、停写、重启持久、open不改updatedAt。
- [x] 跑定向pytest/ruff/mypy，规格审查→质量审查→提交；真实handler后生成唯一OpenAPI类型。

## 包3：领域组件与项目页面（PM1-A/B前端）

- [x] 先写ProjectFormDialog、ProjectDirectory、ProjectTabs测试，覆盖码点长度、错误定位、保存中、Escape和未保存确认。
- [x] 先完成FormDialog、列表、Header、Tabs、CapabilityState，再组DirectoryPage/OverviewPage。服务端类型只引用generated.ts；表单值可以是独立UI类型。
- [x] api通过现有client，读取传AbortSignal，持久命令原key重查恢复，禁止生产mock。组件样例仅在测试中。
- [x] 验证409/后台刷新保草稿、未知结果查询、请求迟到的scope/form epoch保护、目录无匹配和刷新失败。
- [x] 跑领域Vitest与类型检查；规格审查→质量审查→提交。

## 包4：导航与隔离集成（PM1-B/C）

- [x] 先写路由解析/序列化、返回目录、hashchange/back/forward离开取消测试。
- [x] App唯一hash owner；顶部新增项目，路径#/projects与#/projects/{projectId}/{tab}。六页签概览真实，其余共享“暂未开放”状态无假动作。
- [x] 目录状态保留q/lifecycleState/sort/page/pageSize/滚动位置；成功加载后恢复滚动，页码失效落最近有效页。
- [x] ProjectFormDialog未保存离开提供保存后离开/放弃/继续编辑，保存失败留原处；取消恢复规范hash。提交中不可关闭。
- [x] 使用现有useDesktopSession/ApiProvider，不新建第二查询上下文；workspaceChanging禁提交，实际切换清项目路由，同工作区重连保草稿。
- [x] 真实API联调后运行全套门禁，规格与工程审查均完成才进入真实验收。

## 包5：真实应用、证据与阶段收口

- [x] 扩展现有Electron CDP/sidecar helper创建项目冒烟脚本，测试工作区及userData均为本轮临时目录，不触碰用户数据。
- [x] 实际执行新建A/B→编辑→搜索分页→打开→返回→409外部编辑→重连→重启再开，另一个工作区验证隔离。
- [x] 验收Tab/Enter/Escape/焦点、200%缩放、长名称、下拉宽度；现有浏览器/代理/模型/设置/Studio入口回归。
- [x] 保存脱敏截图和机器事实。未运行Windows/其他CPU架构/安装包明确标未执行，不以源码测试替代。
- [x] 更新覆盖仅把PM-02标完整，PM-01/03/OV-01标本阶段部分交付；PM2+证据不填。保留PM0静态报告及校验器的历史用途，不拿其“仅文档变更”约束检查PM1业务代码。
- [x] 同步执行账本、结构和.ai，提交阶段验收报告，保留分支，不合并主目录、不推送、不自动进入PM2。

## 验证命令

```bash
uv run --directory apps/backend pytest tests/contract/test_projects.py tests/integration/test_project_repository.py tests/integration/test_project_migration.py -q
...
- 基线测试：后端438项，前端57文件/372项。最终测试：后端461项，前端72文件/439项；Ruff、mypy、tsc、ESLint、OpenAPI生成检查、构建、脚本与结构检查通过。机器命令与日志见 [PM1核验](../../project-management/implementation/pm1-verification.json)。
- 实施提交：`f1411d0` 统一控件及旧消费者，`b3b1574` 后端及生成接口，`d694093` 项目页面与导航，`4006acc` 缩放修复及真实验收脚本。最终文档提交通过本卡Git历史查询，不向文档嵌入自引用哈希。
- 规格先于工程质量的三组独立审查及问题闭合：[后端](../../project-management/implementation/pm1-review-backend.md)、[前端](../../project-management/implementation/pm1-review-frontend.md)、[共享控件/导航](../../project-management/implementation/pm1-review-controls-navigation.md)。
- 构建后Electron实际通过9组流程，包含真实409、双工作区、重连和完整退出/重启。200%下应用根宽度展开前后708 CSS px，选项面板top=12、bottom=240，完全位于512高的视口；Escape恢复焦点，End/Enter可选择滚动后的末项。详见 [截图及操作依据](../../migration/project-management-pm1-qa/README.md)。
- 开发中的红态还发现行内编辑Enter冒泡、目录滚动区无有效高度、导航取消与真实历史位置不一致、结果未知与明确失败混淆、生命周期入口未只读。均先建立反例，再修复复核；截图复核追加选项面板高度反例。
- 本机范围为macOS arm64的开发形态Electron构建产物。Windows、macOS x64及安装包未执行；PM2–PM9和Run契约不记通过。
- 保留独立分支；主目录与其他任务工作区只读。停止于PM1用户验收，不自动进入PM2。
