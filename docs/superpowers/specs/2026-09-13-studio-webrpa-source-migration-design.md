...
2026-09-13 实施顺序修订（confirmed）：用户认可原型并授权先完整迁入前端组件、Store、请求、事件和SSE，接口暂用mock。本轮允许前端先行，mock仅位于服务边界并显式标识；不以模拟执行宣称真实后端或R1浏览器验收通过。此前必须真实运行通过才大规模迁UI的排期要求在本轮由该授权替代。
