...
- 定位：项目管理的总规格与模块拆分依据。确认后再为各切片编写详细规格、原型补充和实施计划，本轮不开发业务代码。
...
推荐两条建设线：项目生命周期、数据和资源管理先形成真实闭环；Studio 按已有专项规划发展编辑和执行核心。二者先对齐最小契约，在真实执行就绪后接通项目批次、人工接管和统计。这样可以逐步交付真实能力，也能保留最终完整业务模型。
...
长期浏览器登录状态由项目环境承接，不放进全局浏览器配置。两条路径共享执行核心。当前以“两者兼顾，先打通可验证的管理与数据能力”作为顺序建议，具体业务优先级可在审查时调整，不影响模块边界。
...
| **C：管理与执行分工，按业务闭环交汇（推荐）** | 管理/数据切片与 Studio 专项独立推进，运行接入以双方契约和验收为门槛 | 需要及早明确对象身份、资源解析、事件及取消语义；依赖透明、返工可控 |

选择 C 不意味着提前完成整个调度平台。先约定必要接口，真实使用时才建立对应持久化和服务；保持单 sidecar、单本地数据库和成熟基础组件。
...
这是一张职责表，不要求每行都建独立微服务、数据库或通用框架。字段精确定义在对应详细规格中确定。
...
最小共享契约先对齐：工作流保存/校验、资源解析、单次启动/取消/查询、事件身份与顺序、输入输出/产物、环境生命周期。P6 另有硬依赖：等待检查点、可继续位置校验、恢复同一 Run、人工终结命令和权威结果事件；项目层不能自行改 Task 状态冒充核心已继续/结束。名称和 HTTP 路由细节在实施计划前落实，不冻结一套未经核心实现验证的兼容协议。
...
这是模块推进路线，**不是已获批准的逐文件实施计划**。每阶段均要求前后端契约、真实持久化、组件、页面和对应测试一起交付。

| 阶段 | 交付闭环 | 依赖与完成门槛 |
|---|---|---|
| P0：冻结规则与接口边界 | 确认本方案、解决历史冲突、对齐 Studio 最小契约、核对统一控件合并 | 明确资源归属、数据来源、运行证据与环境策略 |
...
| P5：最小真实执行 | 参数型单次运行 → 数据型小批次、独占领取、取消、基本日志、资源回收 | 核心网页流程已真实运行；从输入记录到结果可追溯 |
...
P2/P3、Studio 专项以及环境保存机制的独立验证可以并行；P5/P6 的接入顺序必须服从执行和数据契约。若长期环境业务优先，可提前环境单实例保存/维护切片，不提前开放依赖任务的人工继续按钮。
...
确认本方案后，先将 P1 与 P2 拆成可执行的详细规格/组件清单和实施计划，同时给 Studio 专项提交共享契约草案。不会直接把六页签同时分派成互不相干的前后端页面任务。
