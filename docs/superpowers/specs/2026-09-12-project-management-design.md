...
- 本轮完整稿交付复核：主线已推进至 `4e3f059529215edbbc6b6a5cf59c0f14c9e93d0f`，包含 Studio 空窗口和渐进复刻方法文档，自动化后端仍未实现；最新来源指纹见[完整稿核验记录](../../project-management/design/design-verification.json)，不改写早期研究快照。
...
- 完整细则：[功能与业务设计入口](../../project-management/design/README.md)。用户本轮明确“状态与工作流条件决定复用、工作流负责表/字段/记录 CRUD、结束节点保留环境并关联相关行”，见[业务决定](../../../.ai/decisions/2026-09-12-project-data-workflow-semantics.md)。以下摘要同步新规则；历史审查仍对应其固定旧版本。

## 1. 建议结论

项目将**同一业务的数据、自动化、运行结果和浏览器环境组织在一起**。一个项目可以有多个自动化，共享项目数据；全局浏览器配置、内核、代理和模型通过引用使用。应用设置中的“工作区”继续表示本地数据根，不与项目混用。
...
归属推荐（proposed）：从项目创建的流程属于该项目的自动化；“关联已有流程”直接引用独立 Studio 的当前文档，不转移所有权。界面在关联前说明之后编辑会更新同一文档，不在日常流程中要求理解所有权转移。一个流程至多关联一个项目自动化；已有绑定时显示去向，不能隐式复制或抢占。删除自动化或项目只解除独立流程的关联；项目所有流程则列入删除影响，由核心确认无运行/编辑占用后删除。打开 Studio 的窗口方式不改变归属。本模块不新增复制流程或转移所有权功能，独立流程与项目流程共用核心保存契约。
...
每表有系统维护的单值业务状态列，记录可为“未设置”；来源中的普通状态字段不自动映射为系统状态。工作流明确决定字段、状态及记录增删改查，也可通过受控节点新增或修改字段/列结构。兼容新增列不一概因正在运行而禁止；影响活动输入的删除/改类型等操作须校验并阻断破坏性变更。完整状态、动态写入和结构影响规则见[数据规格](../../project-management/design/data-and-state-rules.md)。

**Google Sheets**：项目级连接，一张数据表对应一个工作表绑定。本地副本是任务读取和编辑的事实源；普通业务值拉取新增记录，已有记录刷新远端定位、公式结果/错误及来源信息，保留本地普通值和业务状态。出站推送受管理且可写字段，公式不可被普通编辑覆盖，不实现任意双向合并。人工保存先提交本地并触发即时推送，工作流变化按现有五分钟目标合并周期推送，提供立即同步；具体同步状态和恢复见数据规格。

推送必须经过 `待推送 → 发送中 → 待核验 → 云端确认`。超时/响应不明先核验，不盲目重发；失败保留本地版本和队列。远端既存记录改动、缺失、身份重复或字段结构变化明确提示，不覆盖未映射列、不按行号写回。手工改业务字段不自动改系统业务状态。

完整设计建议沿用多个项目可引用同一 Sheets 的目标，替代此前单写绑定的收缩建议；本地状态独立，物理记录活动排他和远端定位/写入协调分别保护。多绑定对齐已验证稳定身份，显示字段映射重叠及同字段冲突策略；不暗中把独立本地副本当作实时共享值。Excel 同一文件多次导入仍独立。更换账号、工作表、身份或映射前处理旧队列，操作固定目标和绑定代次，绝不把旧变化投递给新来源。具体取舍仍为 proposed，见数据规格。
...
**占用期间编辑**：建议普通业务字段可提交新版本，任务原始输入不变，旧版本写入返回冲突。身份、删除、来源重绑、全量替换以及破坏活动字段契约的变更受保护；兼容新增字段按受控结构操作处理。正在占用记录的业务状态禁止人工抢改，工作流状态节点仍可在有效占用下明确修改。状态、内容和环境关联的修订分别校验，详见数据规格。

不提供外部 SQLite 文件接入和历史兼容页。完整设计将空白本地表与结果导出列为明确推荐的候选功能；无来源空白表若纳入，复用同一套本地数据能力，不另建存储或同步系统。候选身份见功能规格 DT-12/DT-13，不能因本轮其他业务确认自动记为已批准。

### 5.7 环境：浏览器现场与持久保存

- 四个区域：运行环境、等待人工、持久环境、项目默认资源。可用内部分段页签；不增加项目资源侧栏。
- 运行环境：所属任务/批次、来源浏览器配置、实际内核和代理摘要、状态、查看浏览器、诊断与清理。不能把配置卡片当运行实例。
- 临时环境默认在任务终结后清理；等待人工保留现场至明确期限；结束节点勾选“保留当前环境”时，保存登录上下文并关联约定的相关数据行。保存和关联是一项完整业务操作，部分失败不能报完整成功，也不能清理唯一有效副本。
- 持久环境：列表、搜索/状态筛选、详情、改名、备注、来源任务、保存时资源信息、引用影响、受控删除。更新名称不改身份或任务关联。
- **推荐稳定环境身份下显式更新保存内容**：后续任务从已保存来源创建受控工作副本，结束节点保留时更新所用来源或明确另存；同一环境身份的任务/维护独占，发布新内容检查来源代次。未勾选不自动回写。记录选择环境、相关行范围、已有引用冲突、收尾与恢复由[执行/环境规格](../../project-management/design/execution-and-environment.md)统一定义。
...
- 内核仍由全局浏览器配置模块管理。从配置新建环境采用本批固定的 Profile 内容；恢复已保存环境按其已确认身份与相容内核，模板改动不隐式重置保存环境。明确更换来源参数通过维护或覆盖规则校验。
...
启动时固定流程解释内容、参数、输入绑定/条件与资源选源策略；从配置新建时固定有效 Profile 内容/seed/内核与代理集合。记录值在**每个任务领取时**固定；使用记录关联环境时，也在该时点固定实际 environmentId、最新已保存内容及对应身份资源，以承接前任务明确保存的更新。池分配/显式覆盖见执行规格，不把普通配置变更悄悄带进旧现场。凭据按引用取用，预览不是任务预分配。
...
**用户已确认**：记录释放占用后，只要当前状态及字段仍满足工作流条件，就可在同批或其他批次再次领取，包括流程使用最终态且不改状态的情形。撤回此前按本批历史默认去重的建议，不设置一次性/可复用消费类型。活动排他防同时冲突；系统不替工作流修改状态，也不承诺外部网页“恰好一次”。

最大任务数和并发独立。正数限制已提交任务总数，不保证同样数量的不同记录；存在必要输入时可明确选择不限数量，直至条件实际耗尽、停止策略触发或用户停止。条件持续成立可能持续使用同一记录，启动摘要要说明。零输入/仅可选输入须有限数量，建议默认一次；任务失败后停或继续按可见运行策略执行，不用暗改状态代替。
