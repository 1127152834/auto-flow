...
## 2026-09-24 既有 End 结果恢复修复

状态：confirmed（既有 End/仅修复关联授权内的有界修复，验证进行中）；来源：用户既有 PM9 目标、execution-and-environment.md §End / XE-A12 / XE-C18、实际路由和组件反例。不是跨进程恢复执行。

- GET `/api/v1/projects/{projectId}/tasks/{taskId}/end` 读取该项目/任务最新持久 End；无记录返回 null，项目不存在404，不暴露其他项目记录。复用已有 End/Save 账本，不增迁移或执行器。
- 返回原 `operation` 与原 `outcome`（保持失败历史），另给 `saveOperationId`、当前 `associationPhase` 和原 `recordTargets`。修复更新关联阶段，不能把原失败结果改为成功。
- 修复必须提交内部保存操作 ID；外层 End ID 不能冒充保存 ID。新打开/刷新任务页及 Run 状态版本变化后重新读取持久事实。已成功保存只提供关联修复，不因工作副本清理尚未完成而再次保存。
- 界面从原 End 目标清单选择（包括本任务新建记录），不能用全体输入替代；显示记录身份和采用的关联版本。替换授权默认 false，由用户明确勾选。修复再次冲突则保留选择并使用该次响应的当前版本供下一次提交；不放松后端项目/身份/CAS/占用检查。
- 真实打包链：无授权替换导致 saved_unlinked → 新任务页明确授权/只修复 → 刷新仍显示关联完成且 Run 失败 → 记录关联恢复登录 → 修改会话并 End 更新到新代次 → 第三次恢复实际新登录状态；Profile保持不变，不抵扣I1–I3持久身份缺口。
