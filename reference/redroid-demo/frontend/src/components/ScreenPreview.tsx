...
export function ScreenPreview({ id, enabled, onInput }: { id: string; enabled: boolean; onInput: (input: Input) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    return () => { disposed = true; clearTimeout(timer); controller?.abort(); document.removeEventListener('visibilitychange', visibility); if (url) URL.revokeObjectURL(url); setSrc(''); setDimensions(''); };
  }, [id, enabled, watching]);
