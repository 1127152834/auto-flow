...
export function CreateForm({ environment, availableSlots, pending, onCreate }: {
  ...
  const selectedImage = images.find(item => item.ref === image)?.ref || images.find(item => item.cached)?.ref || images[0]?.ref || '';
  const canCreate = environment?.ready && images.some(item => item.ref === selectedImage && item.cached) && availableSlots > 0 && config.count <= availableSlots;
  return <section className="panel"><h2>创建实例</h2><p>最多 3 台独立实例。每台使用独立数据卷，复制配置会创建空白设备。</p>
    <form onSubmit={event => { event.preventDefault(); void onCreate({ ...config, image: selectedImage }); }}>
      <fieldset disabled={pending}><label>实例名称<input required maxLength={64} value={config.name} onChange={event => setConfig({ ...config, name: event.target.value })} /></label>
        <label>Android 镜像<select value={selectedImage} onChange={event => setImage(event.target.value)}>{images.length === 0 && <option value="">等待环境检查</option>}{images.map(item => <option key={item.ref} value={item.ref} disabled={!item.cached}>{item.ref}{!item.cached ? '（未缓存）' : ''}</option>)}</select></label>
