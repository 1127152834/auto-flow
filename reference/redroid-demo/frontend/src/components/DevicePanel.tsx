...
import { errorMessage, instancePath, request, type Input, type Instance, type InstanceAction } from '../api';
...
export function DevicePanel({ instance, pending, capacityFull, onAction, onInput }: {
  ...
  return <section className="panel device-panel" aria-labelledby="device-title"><div className="section-heading"><div><h2 id="device-title">{instance.name}</h2><p>{androidLabels[instance.android_status]} · Docker: {instance.docker_status}{instance.busy && ' · 操作中'}</p></div><span className="badge">{instance.width} × {instance.height}</span></div>
    ...
    <dl className="facts"><div><dt>Android 实际版本</dt><dd>{instance.android_version || '等待设备实际读取'}</dd></div><div><dt>资源</dt><dd>{instance.cpu} CPU · {instance.memory_mb} MiB · {instance.dpi} DPI</dd></div><div><dt>ADB（WSL 宿主 localhost）</dt><dd><code>{instance.adb_address || '当前无端口映射'}</code></dd></div><div><dt>镜像</dt><dd><code>{instance.image}</code></dd></div></dl>
    ...
    <div className="subsection"><h3>应用与 root 对照</h3><div className="toolbar"><button disabled={!ready} onClick={async () => {
      ...
      try { const result = await request<{ packages: string[] }>(`${instancePath(instance.id)}/packages`); setPackages(result.packages); setPackageName(result.packages[0] || ''); }
