...
export function EnvironmentPanel({ environment, error, refreshing, onRefresh }: {
  ...
  return <section className="panel environment" aria-labelledby="environment-title">
    <div className="section-heading"><div><h2 id="environment-title">运行环境</h2><p>检查实际 Docker 宿主。Windows 推荐 WSL2 Ubuntu 内的 Docker Engine。</p></div>
    ...
    {environment && <><p className={`status-line ${environment.ready ? 'success' : 'warning'}`}>
      {environment.ready ? '环境检查通过，可以创建实例' : environment.docker_available ? 'Docker 已连接，宿主条件尚未满足' : 'Docker 不可用，当前不能管理实例'}
    ...
    </div>)}</div><details><summary>镜像缓存与宿主信息</summary><ul>{environment.images.map(image => <li key={image.ref}><code>{image.ref}</code> · {image.cached ? '已缓存' : '未缓存'}{image.id && <small className="digest">{image.id}</small>}</li>)}</ul><pre>{JSON.stringify(environment.daemon, null, 2)}</pre></details></>}
  </section>;
}
