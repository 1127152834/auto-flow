...
1. **原生自动化鼠标存在偶发偏移，尚未完全解决。** Homebrew scrcpy 4.1 / SDL 3.4.16 在本次 CUA 自动化中多次点击不同位置却向 Android 注入相同坐标。对照 3.3.4 首轮完成了点击/滑动/Notes 操作，但后续重连也有坐标异常，因此不能声称降版本彻底解决。键盘操作仍可完成重启后的内容核验。需要继续区分 CUA 合成事件、窗口焦点、SDL 坐标读取与真实鼠标输入；后续用户已确认首次打开、重连、移动和缩放窗口后三组真实鼠标操作均正常（见后续输入定位）；该结果不等于自动化异常已修复。SDL 的 [macOS 26 坐标问题](https://github.com/libsdl-org/SDL/issues/16152) 和 [3.4.16 Cocoa 源码](https://github.com/libsdl-org/SDL/blob/release-3.4.16/src/video/cocoa/SDL_cocoawindow.m) 仅作为线索，未认定为同一个缺陷。
...
状态：**源码路径 confirmed；根因假设 proposed；真实鼠标验收 confirmed（用户反馈）**。

固定版源码提供了比“缩放不正确”更具体的排查方向：

- [SDL 2.32.8 Cocoa 窗口事件](https://github.com/libsdl-org/SDL/blob/release-2.32.8/src/video/cocoa/SDL_cocoawindow.m)：`Cocoa_SendMouseButtonClicks` 对当前键盘焦点窗口直接发送按钮事件；只有另一分支临时用 `locationInWindow` 更新坐标。`mouseMoved` 则负责处理移动位置。
- [SDL 2.32.8 鼠标事件](https://github.com/libsdl-org/SDL/blob/release-2.32.8/src/events/SDL_mouse.c)：按钮事件的 x/y 来自已记录的 `mouse->x/y`。
- [scrcpy 3.3.4 输入处理](https://github.com/Genymobile/scrcpy/blob/v3.3.4/app/src/input_manager.c)：点击处理使用 SDL 事件的 x/y 转换设备坐标。React 监控页没有参与这一步。

因此，如果合成点击的目标位置与 SDL 先前收到的移动位置不同，可能出现重复旧坐标；这与上一轮观察相容，但尚无本次原生 NSEvent/SDL 事件序列证明 CUA 确实触发了该条件。**不能据此断言 CUA 有缺陷，也不能认定真实鼠标受影响。** 目前不修改坐标公式、不维护 SDL 分支、不把更换输入模式当成已经验证的修复。

真实鼠标最小对照（同一台 `Mac 日常测试`，不修改屏幕分辨率）：

| 顺序 | 操作 | 记录 |
| --- | --- | --- |
| 1 | 原生窗口中点不同位置的应用图标，回到桌面，上滑打开应用列表 | 通过，用户确认 |
| 2 | 用 Mac 红色关闭按钮关窗，从监控页再次打开 | 通过，用户确认 |
| 3 | 拖动窗口到另一位置并调整大小，再点图标和滑动 | 通过，用户确认 |

2026-09-13，用户对上述三组操作明确反馈“都没问题”。状态更新为 confirmed：当前 Mac 环境的人工点击、滑动、重连、移动和缩放窗口对照通过。依据是用户实际操作反馈，并非新增 CUA 测试或事件日志；原 pending 状态 superseded。自动化输入异常及其根因假设仍保留，不扩大为所有硬件、版本或长时间运行的稳定性保证。

结论：页面 → 本机原生窗口 → 真实 Android → 输入 → 关窗保留 → 重开/设备重启后数据保留的功能闭环成立，置信度高；鼠标在自动化重连场景的稳定性待进一步核验。当前人工操作路径可以作为后续设备与工作流控制权设计的依据，自动化坐标问题不再阻塞该设计。工作流的设备操作应通过 Android 自动化接口执行；原生窗口用于人工接管，正式接入仍需设计、规格和实施计划确认。
