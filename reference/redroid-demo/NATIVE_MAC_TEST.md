...
状态：**源码路径 confirmed；根因假设 proposed；真实鼠标验收 pending**。
...
| 顺序 | 操作 | 记录 |
| --- | --- | --- |
| 1 | 原生窗口中点不同位置的应用图标，回到桌面，上滑打开应用列表 | 是否命中、是否能滑动 |
| 2 | 用 Mac 红色关闭按钮关窗，从监控页再次打开 | 重连后重复第 1 步是否准确 |
| 3 | 拖动窗口到另一位置并调整大小，再点图标和滑动 | 问题是否只随窗口位置或大小出现 |

本轮已通过监控页重新打开真实设备，API 报告 `open`；等待使用者完成物理鼠标/触控板对照。在使用者测试期间暂停 CUA 输入，避免两个操作者互相影响。若人工三步均正常，记录为“当前人工路径通过，自动化路径仍待定位”；若人工也有偏移，再按位置、缩放、焦点分别复现，不能直接归因于工具。
