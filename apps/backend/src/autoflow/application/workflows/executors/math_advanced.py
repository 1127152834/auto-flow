...
# backend/app/executors/math_advanced.py.
import asyncio
import math
import random
...
_MAX_COMBINATORIC_TERMS = 10_000
_MAX_RESULT_DECIMAL_DIGITS = 4_000
...
async def _permutation_fits_runtime_boundary(
    n: int, r: int, calc_type: str, context: ExecutionContext
) -> bool:
...
class MathLogExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        number_value = context.resolve_value(
            config.get("numberValue", config.get("value", ""))
        )
        ...
        try:
            ...
            if log_type == "ln":
                ...
                if base_num <= 0 or base_num == 1:
                    return ModuleResult(
                        success=False, error="对数底数必须大于0且不等于1"
                    )
                result = math.log(num, base_num)
                desc = f"log{base_num}({num})"
            else:
                return ModuleResult(
                    success=False, error=f"不支持的对数类型: {log_type}"
                )

            context.set_variable(result_variable, result)

            return ModuleResult(
                success=True, message=f"对数运算: {desc} = {result}", data=result
            )

        except (ValueError, TypeError) as e:
            return ModuleResult(success=False, error=f"无效的数字: {e!s}")
        except Exception as e:
            return ModuleResult(success=False, error=f"对数运算失败: {e!s}")
...
class MathTrigExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        trig_type = context.resolve_value(
            config.get("trigType", config.get("function", "sin"))
        )
        angle_unit = context.resolve_value(
            config.get("angleUnit", config.get("unit", "degree"))
        )  # degree or radian
        result_variable = config.get("resultVariable", "")
        ...
        try:
            ...
            if trig_type == "sin":
                ...
                if num < -1 or num > 1:
                    return ModuleResult(
                        success=False, error="反正弦函数的输入必须在[-1, 1]范围内"
                    )
                ...
                if num < -1 or num > 1:
                    return ModuleResult(
                        success=False, error="反余弦函数的输入必须在[-1, 1]范围内"
                    )
                ...
                return ModuleResult(
                    success=False, error=f"不支持的三角函数类型: {trig_type}"
                )
            ...
            return ModuleResult(success=False, error=f"三角函数计算失败: {e!s}")
...
class MathExpExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(
                success=True, message=f"指数运算: e^{num} = {result}", data=result
            )
            ...
            return ModuleResult(success=False, error=f"指数运算失败: {e!s}")
...
class MathGcdExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(success=False, error=f"计算最大公约数失败: {e!s}")
...
class MathLcmExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(success=False, error=f"计算最小公倍数失败: {e!s}")
...
class MathFactorialExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(
                success=True, message=f"阶乘: {num}! = {result}", data=result
            )

        except (ValueError, TypeError):
            return ModuleResult(success=False, error=f"无效的数字: {number_value}")
        except Exception as e:
            return ModuleResult(success=False, error=f"阶乘计算失败: {e!s}")
...
class MathPermutationExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        calc_type = context.resolve_value(
            config.get("calcType", "permutation")
        ...
        try:
            ...
            if calc_type not in {"permutation", "combination"}:
                return ModuleResult(
                    success=False, error=f"不支持的计算类型: {calc_type}"
                )
            if not await _permutation_fits_runtime_boundary(
                n_val, r_val, calc_type, context
            ):
                return ModuleResult(success=False, error="计算规模超过工作流安全限制")
            ...
            return ModuleResult(
                success=True, message=f"{type_text}: {desc} = {result}", data=result
            )

        except (ValueError, TypeError) as e:
            return ModuleResult(success=False, error=f"无效的数字: {e!s}")
        except Exception as e:
            return ModuleResult(success=False, error=f"计算失败: {e!s}")
...
class MathPercentageExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        calc_type = context.resolve_value(
            config.get("calcType", config.get("operation", "percent_of"))
        )
        ...
        try:
            ...
            return ModuleResult(
                success=True, message=f"百分比计算: {desc} = {result}", data=result
            )

        except (ValueError, TypeError) as e:
            return ModuleResult(success=False, error=f"无效的数字: {e!s}")
        except Exception as e:
            return ModuleResult(success=False, error=f"百分比计算失败: {e!s}")
...
class MathClampExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(success=False, error=f"范围限制失败: {e!s}")
...
class MathRandomAdvancedExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            if random_type in ("uniform", "int"):
                min_val = float(
                    context.resolve_value(
                        config.get("min", config.get("minValue", "0"))
                    )
                )
                max_val = float(
                    context.resolve_value(
                        config.get("max", config.get("maxValue", "100"))
                    )
                )
                if min_val > max_val:
                    return ModuleResult(success=False, error="最小值不能大于最大值")
                for index in range(count_val):
                    await _checkpoint(context, index)
                    if random_type == "int":
                ...
                return ModuleResult(
                    success=False, error=f"不支持的随机类型: {random_type}"
                )
            ...
            return ModuleResult(
                success=True, message=f"生成{count_val}个{type_text}随机数", data=result
            )

        except (ValueError, TypeError) as e:
            return ModuleResult(success=False, error=f"无效的参数: {e!s}")
        except Exception as e:
            return ModuleResult(success=False, error=f"生成随机数失败: {e!s}")
