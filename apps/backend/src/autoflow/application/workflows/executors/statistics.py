...
# backend/app/executors/statistics.py.
import asyncio
import statistics
...
async def _numeric_values(values: list[Any], context: ExecutionContext) -> list[float]:
    numbers: list[float] = []
...
class MedianExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            numbers = await _numeric_values(list_data, context)
            if not numbers:
            ...
            return ModuleResult(success=False, error=f"计算中位数失败: {e!s}")
...
class ModeExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(success=False, error=f"计算众数失败: {e!s}")
...
class VarianceExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(success=False, error=f"计算方差失败: {e!s}")
...
class StdevExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            return ModuleResult(success=False, error=f"计算标准差失败: {e!s}")
...
class PercentileExecutor(ModuleExecutor):
    ...
    async def execute(
        ...
        try:
            ...
            numbers = sorted(await _numeric_values(list_data, context))
            if not numbers:
                return ModuleResult(success=False, error="列表中没有数字")
            ...
            context.set_variable(result_variable, result)
            return ModuleResult(
                success=True, message=f"第{p}百分位数: {result}", data=result
            )
        except Exception as e:
            return ModuleResult(success=False, error=f"计算百分位数失败: {e!s}")
        ...
        try:
            ...
            if method == "custom":
                try:
                    ...
                    return ModuleResult(
                        success=False, error="自定义范围的最小值/最大值必须是数字"
                    )
            else:
            ...
            if min_val == max_val:
                ...
                for index, number in enumerate(numbers):
                    await _checkpoint(context, index)
                    result.append(
                        new_min + (number - min_val) / (max_val - min_val) * span
                    )

            context.set_variable(result_variable, result)
            return ModuleResult(
                success=True,
                message=f"归一化完成，范围[{new_min},{new_max}]",
                data=result,
            )
        except Exception as e:
            return ModuleResult(success=False, error=f"归一化失败: {e!s}")
    ...
    async def execute(
        ...
        try:
            ...
            if stdev == 0:
                ...
                for index, number in enumerate(numbers):
                    await _checkpoint(context, index)
                    result.append((number - mean) / stdev)

            context.set_variable(result_variable, result)
            return ModuleResult(
                success=True, message="标准化完成（Z-score）", data=result
            )
        except Exception as e:
            return ModuleResult(success=False, error=f"标准化失败: {e!s}")
