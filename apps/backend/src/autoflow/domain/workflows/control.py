...
def compile_control(document: dict[str, Any], *, check_variables: bool = True) -> list[dict[str, Any]]:
    ...
    writers = {n['config']['variableName'] for n in nodes.values() if n['type'] in {'set_variable', 'get_element_info', 'screenshot', 'android_screenshot'} and isinstance(n['config'].get('variableName'), str)}
    visited: set[str] = set()
