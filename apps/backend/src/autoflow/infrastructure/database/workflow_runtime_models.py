...
class WorkflowPreparedContentRow(Base):
    __tablename__ = "project_workflow_prepared_contents"
    __table_args__ = (
        Index(
        ...
        CheckConstraint(
            ...
            name="ck_project_workflow_prepared_source_revision",
        ),
    )
...
class WorkflowRunRow(Base):
    ...
    __table_args__ = (
        Index("ix_project_workflow_runs_prepared_status", "prepared_content_id", "status"),
        CheckConstraint(
            ...
            name="ck_project_workflow_runs_status",
        ),
        CheckConstraint(
            "status_revision >= 1 AND execution_generation >= 0 AND last_sequence >= 0",
            name="ck_project_workflow_runs_revisions",
        ),
    )
...
class WorkflowRunEventRow(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            ...
            name="ck_project_workflow_run_event_generation",
        ),
        CheckConstraint(
            "attempt IS NULL OR attempt >= 1",
            name="ck_project_workflow_run_event_attempt",
        ),
    )
...
class WorkflowRunArtifactRow(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_project_workflow_artifacts_purpose", "run_id", "purpose", "ordinal"),
    )
