...
def read_profile_environment_options() -> ProfileEnvironmentOptions:
    ...
    for options in (catalog.locales, catalog.timezones, catalog.user_agent_templates):
    ...
    for template in catalog.user_agent_templates:
        for text in (template.value, template.label):
            expanded = text.replace("{major}", "")
            if any(character in expanded for character in ("{", "}", "\r", "\n")):
                raise ValueError("User Agent templates only support the {major} placeholder")
    return catalog
