...
async def test_project_recognition_uses_real_models_files_branches_and_cleanup(tmp_path: Path, scenario: str):
    ...
    try:
        ...
        expected = {'missing': 'failed', 'stop': 'cancelled'}.get(scenario, 'succeeded')
        ...
        if scenario == 'match':
            ...
            assert not any(e.node_id in {'ocr', 'matched', 'empty'} for e in events)
            if scenario == 'stop':
