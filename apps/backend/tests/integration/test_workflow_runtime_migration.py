...
def test_empty_database_upgrades_to_one_runtime_head_with_complete_v2_schema(tmp_path):
    ...
    with sqlite3.connect(database) as connection:
        ...
        assert {
            ...
            "provenance",
            "created_at",
        } <= _table_columns(connection, "workflow_prepared_contents")
        assert {
            "id",
            "run_request_id",
            "request_digest",
        ...
        assert {
            ...
            "occurred_at",
            "payload",
        } <= _table_columns(connection, "workflow_run_events")
        assert "active_slot" not in _table_columns(connection, "workflow_runs")
    ...
    with sqlite3.connect(database) as connection:
        ...
        if start_revision == "0008_workflow_debug":
            active = connection.execute(
                ...
                "FROM workflow_runs WHERE id='run-active'"
            ).fetchone()
            ...
            assert active[0] not in {"queued", "running", "reconciling"}
                ...
                "FROM workflow_runs WHERE id='run-done'"
            ).fetchone()
...
def test_0009_upgrade_preserves_shared_facts_and_maps_legacy_provenance(tmp_path):
    ...
    with sqlite3.connect(database) as connection:
        ...
        rows = connection.execute(
            ...
            "FROM workflow_prepared_contents ORDER BY workflow_id"
        ).fetchall()
        assert len(rows) == 2
        for (
            source_revision,
        ...
        events = connection.execute(
            ...
            "FROM workflow_run_events ORDER BY run_id, sequence"
        ).fetchall()
        assert len(events) == 2
...
def test_missing_source_keeps_run_document_snapshot(tmp_path):
    ...
    with sqlite3.connect(database) as connection:
        document, provenance = connection.execute(
            "SELECT document, provenance FROM workflow_prepared_contents"
        ).fetchone()
...
def test_previous_interrupted_terminal_keeps_error_and_finished_at(tmp_path):
    ...
    with sqlite3.connect(database) as connection:
        status, error, finished_at = connection.execute(
            "SELECT status, error, completed_at FROM workflow_runs WHERE id='run-active'"
        ).fetchone()
