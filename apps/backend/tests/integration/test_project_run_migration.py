...
def test_empty_database_upgrades_to_the_single_project_run_head(tmp_path):
    ...
    assert ScriptDirectory.from_config(config).get_heads() == ["0020_merge_android_pm9"]
    database_session.migrate_database(database)

    with sqlite3.connect(database) as connection:
        assert connection.execute(
            "SELECT version_num FROM alembic_version"
        ).fetchone() == ("0020_merge_android_pm9",)
...
def test_project_run_foreign_keys_are_restrictive_and_complete(tmp_path):
    ...
    expected = {
        "project_batches": {
            ...
            ("project_workflow_prepared_contents", "prepared_content_id"),
        },
        "project_tasks": {
            ("projects", "project_id"),
