...
def test_pm1_upgrade_preserves_0005_resources(tmp_path: Path, existing: bool):
    ...
    assert ScriptDirectory.from_config(config).get_heads() == ["0020_recording_project_scope"]
    ...
    if existing:
        ...
        with sqlite3.connect(database) as connection:
            ...
            for table in (
                ...
                preserved[table] = connection.execute(
                    f"SELECT * FROM {table}"
                ).fetchall()
    ...
    with sqlite3.connect(database) as connection:
        ...
        ).fetchall() == [("0020_recording_project_scope",)]
