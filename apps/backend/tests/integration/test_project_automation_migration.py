...
@pytest.mark.parametrize('previous', [None, 'pm01_projects', '0011_workflow_runtime_contracts'])
def test_automation_upgrade_preserves_existing_rows_and_enforces_binding(tmp_path, previous):
    ...
    if previous:
        ...
        with sqlite3.connect(database) as connection:
            connection.execute(
                ...
                ('workflow-fixture', '既有流程', json.dumps({'original': True}), '{}', 7, '2026-09-15', '2026-09-15'),
            )
            connection.execute(
                ...
                ('project-fixture', '既有项目', '既有项目', '', '既有项目', '{}', 1, 'active', '2026-09-15', '2026-09-15'),
            )
    ...
    with sqlite3.connect(database) as connection:
        ...
        foreign_keys = connection.execute('PRAGMA foreign_key_list(project_automations)').fetchall()
        ...
        if previous:
            values = ('automation-fixture', 'project-fixture', 'workflow-fixture', 'A', 'a', 'a', '', 1, '{"inputs":[]}', '[]', '{}', '{}', '2026-09-15', '2026-09-15')
            ...
            with pytest.raises(sqlite3.IntegrityError):
                connection.execute("DELETE FROM workflow_documents WHERE id='workflow-fixture'")
            with pytest.raises(sqlite3.IntegrityError):
...
def test_empty_downgrade_roundtrip_and_nonempty_downgrade_preserves_data(tmp_path):
    ...
    with sqlite3.connect(database) as connection:
        connection.execute('PRAGMA foreign_keys=ON')
        connection.execute('INSERT INTO workflow_documents VALUES (?,?,?,?,?,?,?)', ('w', 'W', '{}', '{}', 1, '2026-09-15', '2026-09-15'))
        connection.execute('INSERT INTO projects (id,name,name_key,description,search_text,default_resources,management_revision,lifecycle_state,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)', ('p', 'P', 'p', '', 'p', '{}', 1, 'active', '2026-09-15', '2026-09-15'))
        connection.execute(
            ...
            ('a', 'p', 'w', 'A', 'a', 'a', '', 1, '{}', '[]', '{}', '{}', '2026-09-15', '2026-09-15'),
        )
    ...
    with pytest.raises(RuntimeError, match='configuration exists'):
        command.downgrade(config, '0011_workflow_runtime_contracts')
    with sqlite3.connect(database) as connection:
        assert connection.execute('SELECT * FROM project_automations').fetchall() == original
        assert connection.execute('SELECT version_num FROM alembic_version').fetchone() == ('pm03_project_automations',)
