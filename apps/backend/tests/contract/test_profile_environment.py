...
def wire_catalog(catalog):
    return {"locales": catalog["locales"], "timezones": catalog["timezones"], "userAgentTemplates": catalog["user_agent_templates"]}
...
def test_catalog_matches_json_and_contains_valid_profile_values(client, profile_payload):
    ...
    assert catalog == wire_catalog(json.loads(profile_environment.CATALOG_PATH.read_text()))
    ...
    created = client.post("/api/v1/profiles", json={**profile_payload, "locale": "ja-JP", "timezone": "Asia/Tokyo", "userAgent": ua})
...
def test_backend_file_is_the_live_source(client, tmp_path, monkeypatch):
    ...
    catalog = {
        ...
        "user_agent_templates": [{"value": "Catalog UA Chrome/{major}.0.0.0", "label": "目录 UA"}],
    }
