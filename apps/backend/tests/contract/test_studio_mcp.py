...
@pytest.mark.parametrize(
    "transport",
    [None, "", "stdio", "sse", "http", "HTTP", "Streamable_HTTP", "streamable-http"],
)
def test_mcp_config_preserves_wire_fields_and_extensions(transport):
    ...
    assert (
        StudioMcpConfig.model_validate(data).model_dump(
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {"success": True},
        {"success": False, "saved": True},
        {"success": 1, "saved": True},
...
def test_mcp_reload_preserves_partial_failure_without_success_fabrication():
    data = {
        "connected": [],
        "failed": [{"name": "fixture", "error": "mock does not connect"}],
        "disabled": ["off"],
        "total_servers": 2,
        "commandId": "reload-1",
        "revision": 3,
