...
from app.executors.basic import TextToSpeechExecutor
...
async def _run(payload):
    ...
    main.request_tts_sync = lambda **_values: True
    sys.modules["app.main"] = main
    context = ExecutionContext(variables=payload.get("variables", {}))
    result = await TextToSpeechExecutor().execute(payload["config"], context)
