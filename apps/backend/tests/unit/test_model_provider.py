...
def connection(kind="openai-compatible", *, preset="custom-openai-compatible", url="https://api.example/v1"):
...
async def test_openai_discovery_preserves_query_and_normalizes_context():
    ...
    result = await gateway.discover(connection(url="https://api.example/v1?region=cn"), "secret")
...
async def test_anthropic_and_gemini_protocols():
    ...
    async def handler(request):
        ...
        if request.url.host == "api.anthropic.com":
            ...
            return httpx.Response(200, json={"data": [{"id": "claude", "max_input_tokens": 200000}]})
        assert "authorization" not in request.headers
        assert request.url.params["key"] == "gk"
        assert request.url.params["pageSize"] == "1000"
        return httpx.Response(200, json={"models": [{"name": "models/gemini", "inputTokenLimit": 1000000}]})

    gateway = HttpModelProvider(transport=httpx.MockTransport(handler))
    anthropic = await gateway.discover(ProviderConnection("anthropic", "anthropic", None), "ak")
...
async def test_qwen_replaces_path_but_keeps_custom_host_and_query():
    async def handler(request):
        ...
        return httpx.Response(200, json={"output": {"models": [{
            "model": "qwen-plus", "name": "Qwen Plus", "provider": "qwen",
            "model_info": {"context_window": 32768},
        }]}})

    gateway = HttpModelProvider(transport=httpx.MockTransport(handler))
    result = await gateway.discover(connection(preset="qwen", url="https://private.example/compatible/v1?tenant=one"), "key")
...
async def test_client_security_and_timeout_options_are_explicit():
    ...
    gateway = HttpModelProvider(transport=httpx.MockTransport(handler), client_factory=factory)
    await gateway.discover(connection(), "key")
    await gateway.test_model(connection(), "key", "sample")
    assert [(item["trust_env"], item["follow_redirects"], item["timeout"]) for item in seen] == [
...
async def test_generation_payload_encoding_and_preview_limits():
    async def handler(request):
        assert request.url.raw_path.decode().split("?", 1)[0].endswith("/models/name%2Fwith%20space:generateContent")
        payload = json.loads(request.content)
        assert payload["contents"][0]["parts"][0]["text"] == "只回复 OK"
        assert payload["generationConfig"]["maxOutputTokens"] == 16
        return httpx.Response(200, json={"candidates": [{"content": {"parts": [
            {"text": "o" * 300}, {"text": "r" * 2100, "thought": True}
        ]}}]})

    gateway = HttpModelProvider(transport=httpx.MockTransport(handler))
    result = await gateway.test_model(ProviderConnection("gemini", "gemini", None), "key", "name/with space")
...
@pytest.mark.parametrize("status,code", [
    (401, "MODEL_PROVIDER_AUTH_FAILED"), (403, "MODEL_PROVIDER_AUTH_FAILED"),
    (404, "MODEL_PROVIDER_ENDPOINT_NOT_FOUND"), (429, "MODEL_PROVIDER_RATE_LIMITED"),
    (500, "MODEL_PROVIDER_REQUEST_FAILED"),
])
async def test_http_errors_are_fixed_and_redacted(status, code):
    async def handler(_request):
        return httpx.Response(status, text="secret https://user:pass@example?key=secret", headers={"retry-after": "9"})

    with pytest.raises(ModelError) as caught:
        await HttpModelProvider(transport=httpx.MockTransport(handler)).discover(connection(), "secret")
...
async def test_invalid_and_oversized_responses_are_rejected(response):
    with pytest.raises(ModelError) as caught:
        await HttpModelProvider(transport=httpx.MockTransport(lambda _request: response)).discover(connection(), "key")
...
@pytest.mark.parametrize("url", [
    "file:///tmp/models", "https://user:pass@example/v1", "https://example/v1#fragment",
    "https://example/v1?API_KEY=secret", "https://example:99999/v1",
])
...
@pytest.mark.asyncio
@pytest.mark.parametrize("kind,preset", [("anthropic", "ollama"), ("gemini", "ollama"), ("openai-compatible", None)])
