...
def test_custom_module_workflow_requires_semantic_fields_without_layout() -> None:
    payload = {
        "name": "without_layout",
        "workflow": {
            "nodes": [{"id": "value", "type": "moduleNode", "data": {"moduleType": "set_variable"}}]
        },
    }
    assert CustomModuleDraft.from_payload(payload).definition["workflow"] == {
        **payload["workflow"], "edges": [], "variables": []
    }
    ...
    assert invalid.value.details == {"path": "workflow.nodes.0.data"}
