...
function DialogContent({ className, style, busy: contentBusy, onEscapeKeyDown, onPointerDownOutside, children, ...props }: ContentProps) {
  ...
  return <OverlayHostContext.Provider value={frame.scope}><DialogPrimitive.Portal>
    <DialogPrimitive.Overlay data-slot="modal-overlay" style={frame.overlayStyle} className="autoflow-dialog-overlay fixed inset-0 z-[50] bg-ink/35" />
    <DialogPrimitive.Content data-overlay-depth={frame.scope.depth} style={{ ...style, ...frame.contentStyle }} className={cn('autoflow-dialog-content fixed left-1/2 top-1/2 z-[50] grid w-[min(92vw,32rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-card border border-line bg-surface p-6 shadow-xl focus:outline-none motion-safe:transition-all', className)} onEscapeKeyDown={(event) => { if (busy || frame.hasOpenPopup()) { event.preventDefault(); return }; onEscapeKeyDown?.(event) }} onPointerDownOutside={(event) => { if (busy) { event.preventDefault(); return }; onPointerDownOutside?.(event) }} {...props}>{children}<div data-overlay-host ref={frame.hostRef} style={{ display: 'contents' }} /></DialogPrimitive.Content>
