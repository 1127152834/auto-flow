...
export function Select({ ref, value, options, onValueChange, onBlur, id, name, size = 'md', disabled, readOnly, loading, errorMessage, onRetry, className, placeholder = '请选择', clearable = true, ...aria }: ChoiceProps & { ref?: Ref<HTMLButtonElement> }) {
  const [open, setOpen] = useState(false)
  const trigger = useRef<HTMLButtonElement>(null)
  const canChange = () => !readOnly && !disabled && !trigger.current?.matches(':disabled') && !trigger.current?.closest('[inert]')
  ...
  return <div className={cn('grid min-w-0 gap-2', className)}>
    <div className="flex min-w-0 items-center gap-1">
      <SelectPrimitive.Root value={encodeValue(value)} onValueChange={key => { if (canChange()) onValueChange(decodeValue(key)); else setOpen(false) }} disabled={disabled}
        open={open && !readOnly} onOpenChange={next => setOpen(next && canChange())}>
        <SelectPrimitive.Trigger data-choice-value={value ?? undefined} {...aria} id={id} ref={trigger} onBlur={onBlur} aria-readonly={readOnly || undefined} aria-invalid={aria['aria-invalid'] || Boolean(errorMessage) || undefined} aria-describedby={description} aria-busy={loading || undefined}
      ...
      {clearable && value !== null && !readOnly && !disabled ? <IconButton aria-label="清除选择" size={size} variant="ghost" onClick={() => { onValueChange(null); trigger.current?.focus() }}><X size={14} /></IconButton> : null}
    </div>
