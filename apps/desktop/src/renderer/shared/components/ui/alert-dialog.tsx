...
function AlertDialogContent({ className, style, children, onEscapeKeyDown, ...props }: ComponentPropsWithoutRef<typeof AlertDialogPrimitive.Content>) {
  ...
  return <OverlayHostContext.Provider value={frame.scope}><AlertDialogPrimitive.Portal>
    <AlertDialogPrimitive.Overlay data-slot="modal-overlay" style={frame.overlayStyle} className="autoflow-dialog-overlay fixed inset-0 bg-[var(--color-overlay)]" />
    <AlertDialogPrimitive.Content data-overlay-depth={frame.scope.depth} onEscapeKeyDown={event => { if (frame.hasOpenPopup()) { event.preventDefault(); return }; onEscapeKeyDown?.(event) }} style={{ ...style, ...frame.contentStyle }} className={cn('autoflow-dialog-content fixed left-1/2 top-1/2 grid w-[min(92vw,30rem)] -translate-x-1/2 -translate-y-1/2 max-h-[90dvh] overflow-y-auto gap-4 rounded-modal border border-line bg-surface p-6 shadow-modal', className)} {...props}>
