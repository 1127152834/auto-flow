...
export function BatchLauncher(props: BatchLauncherProps) {
  ...
  const checks = [{ label: '运行条件', value: props.validationLoading ? '正在检查' : props.validationError ? '检查失败' : props.readOnly ? '只读项目' : Boolean(props.validation?.runnable) ? '检查完成' : issueText || '需要配置', accepted: Boolean(props.validation?.runnable), status: props.validation?.runnable ? '可以启动' : '需要处理' }, ...(hasInputs ? [{ label: '项目数据', value: preview.loading ? '正在预检数据输入' : preview.error ? '预检失败' : inputsReady ? '已找到完整输入组' : '当前不能领取完整输入组', accepted: inputsReady, status: inputsReady ? '可以领取' : '需要处理' }] : [])]
