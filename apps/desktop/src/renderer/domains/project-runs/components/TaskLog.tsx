...
const levelLabels: Record<string, string> = { debug: '调试', info: '信息', success: '成功', warning: '警告', error: '错误' }
...
const visibleMessage = (level: string, message: string, isUserLog: boolean) => level === 'error' && !isUserLog ? '节点执行失败，请查看异常与证据。' : message
const clock = (value: string | null) => value ? new Date(value).toLocaleTimeString('zh-CN', { hour12: false }) : '未执行'
const contextLabel = (value: unknown) => {
  if (!value || typeof value !== 'object') return ''
  const context = value as { scopes?: unknown; loops?: unknown }
  const scopes = Array.isArray(context.scopes) ? context.scopes.map(scope => scope?.name || scope?.id).filter(Boolean) : []
  const loops = Array.isArray(context.loops) ? context.loops.map(loop => Number.isInteger(loop?.iteration) ? `第 ${loop.iteration} 轮` : '').filter(Boolean) : []
  return [...scopes, ...loops].join(' / ')
}
...
export function TaskLog({ attempts, logs, nodeNames, selectedNode, level, query, loading, error, onNodeChange, onLevelChange, onQueryChange, onLoadMore, onLoadMoreAttempts, onRetry }: { attempts?: Attempts; logs?: Logs; nodeNames?: Record<string, string>; selectedNode: string | null; level: string | null; query: string; loading?: boolean; error?: string; onNodeChange(value: string | null): void; onLevelChange(value: string | null): void; onQueryChange(value: string): void; onLoadMore(): void; onLoadMoreAttempts(): void; onRetry(): void }) {
  ...
  return <div className="grid min-w-0 gap-4 lg:grid-cols-[20rem_minmax(0,1fr)]">
    ...
    <section className="min-w-0 rounded-card border border-line bg-surface p-4">
      ...
      <TableScroll label="任务日志" className="mt-4 rounded-control border border-line"><Table><TableHeader><TableRow><TableHead>时间</TableHead><TableHead>级别</TableHead><TableHead>日志内容</TableHead></TableRow></TableHeader><TableBody>{logs?.items.map(item => <TableRow key={item.eventId}><TableCell><time dateTime={item.occurredAt}>{new Date(item.occurredAt).toLocaleTimeString('zh-CN', { hour12: false })}</time></TableCell><TableCell><TableStatus tone={item.level === 'error' ? 'danger' : item.level === 'warning' ? 'warning' : item.level === 'success' ? 'success' : 'neutral'}>{levelLabels[item.level] ?? item.level}</TableStatus></TableCell><TableCell className="max-w-xl whitespace-normal break-words">{visibleMessage(item.level, item.message, item.isUserLog)}{contextLabel(item.executionContext) ? <span className="mt-1 block text-xs text-muted">{contextLabel(item.executionContext)}</span> : null}</TableCell></TableRow>)}</TableBody></Table></TableScroll>
