...
export function TaskDetailPage({ workspaceKey, instanceId, projectId, taskId, tab, client, disabled, onNavigate }: TaskDetailPageProps) {
  ...
  return <>
    ...
    <TaskEndPanel workspaceKey={workspaceKey} instanceId={instanceId} projectId={projectId} taskId={taskId} runId={detail.data.run.runId} executionGeneration={detail.data.run.executionGeneration} inputs={detail.data.inputSnapshot.inputs} client={client} disabled={disabled} environmentCleaned={detail.data.cleanup.status === 'succeeded'} />
