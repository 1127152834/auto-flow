...
function Detail({ workspaceKey, instanceId, projectId, client, disabled, readOnly, batchId, onNavigate }: RunPageContext & { batchId: string }) {
  ...
  if (!detail.data) return <section className="rounded-card border border-line bg-surface p-5"><p role={detail.error ? 'alert' : 'status'}>{detail.error ? `批次读取失败：${presentRunFailure(detail.error)}` : '正在读取批次…'}</p><Button onClick={back}>返回批次列表</Button>{detail.error ? <Button disabled={disabled} onClick={() => void detail.refetch()}>重试读取</Button> : null}</section>
  const batch = detail.data.batch, forceEligible = ['stopping', 'reconciling'].includes(batch.status)
