...
function Directory({ workspaceKey, instanceId, projectId, client, disabled, readOnly, onOpen, onCreate }: AutomationDirectoryPageProps) {
  ...
  return <AutomationDirectory items={result.data?.items ?? []} total={result.data?.total ?? 0} page={query.page} pageSize={query.pageSize} query={query.query} sort={query.sort} loading={result.isLoading} refreshing={result.isFetching || disabled} readOnly={readOnly} errorMessage={result.error ? safeProjectError(result.error) : undefined}
