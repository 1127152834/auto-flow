...
function Detail({ projectDefaults, workspaceKey, instanceId, projectId, automationId, client, disabled, readOnly, onCreated, registerLeaveGuard }: AutomationDetailPageProps) {
  ...
  if (!baseline) return <section className="rounded-card border border-line bg-surface p-6" role={result.error ? 'alert' : 'status'}>{result.error ? <><p>无法读取自动化：{result.error.message}</p><Button onClick={() => void result.refetch()}>重试读取</Button></> : '正在读取自动化…'}</section>
  ...
  return <section className="grid min-w-0 gap-3">
    {readErrors.length > 0 ? <div role="alert" className="rounded-control border border-warning/40 bg-surface p-3 text-sm"><p>部分资料读取失败，当前草稿已保留。{readErrors.map(error => error?.message).join('；')}</p><Button size="sm" onClick={() => { void workflows.refetch(); void profiles.refetch(); void proxies.refetch(); void models.refetch(); void tables.refetch(); if (automationId) void result.refetch(); if (recordTable) void records.refetch() }}>重新读取资料</Button></div> : null}
    ...
    <AutomationEditor validationNotice={automationId && validation.data && !validation.data.runnable ? <details className="mx-5 border-b border-line py-2 text-sm"><summary className="cursor-pointer text-muted">查看运行条件 · 尚未满足</summary><ul className="mb-0 mt-2 pl-5">{validation.data.issues.map((issue, index) => <li key={`${issue.code}:${index}`}>{issue.message}</li>)}</ul><p className="mb-0 text-muted">可以继续维护配置，保存配置不代表开始运行。</p></details> : null} initialValue={baseline.value} resetKey={baseline.resetKey} isNew={!automationId} disabled={disabled || readOnly} saving={command.busy} recovering={command.recovering} error={command.error} serverErrors={command.fields}
      workflowOptions={(workflows.data?.items ?? []).map(workflow => ({ id: workflow.workflowId, name: workflow.name, revision: workflow.revision, updatedAt: workflow.updatedAt, runnable: workflow.validation.runnable, validationMessage: workflow.validation.issues.map(issue => issue.message).join('；') }))}
