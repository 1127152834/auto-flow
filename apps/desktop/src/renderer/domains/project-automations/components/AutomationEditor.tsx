...
export function AutomationEditor({ initialValue, resetKey, workflowOptions, isNew = false, disabled = false, saving = false, recovering = false, error, serverErrors = {}, validationNotice, environmentOptions, renderInputPlan, onSubmit, onCancel, onDraftStateChange, onOpenStudio, onStartRun }: AutomationEditorProps) {
  ...
  return <section className="rounded-card border border-line bg-surface" aria-label="自动化配置编辑器">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b border-line px-5 py-4">
      ...
      <div className="flex gap-2">{onOpenStudio ? <Button disabled={locked || !value.workflowId} onClick={onOpenStudio}>打开 Studio</Button> : null}{onStartRun ? <Button variant="primary" disabled={locked || dirty} onClick={onStartRun}>启动运行</Button> : null}</div>
    </header>
    {validationNotice}
    ...
    <Tabs value={activeTab} onValueChange={tab => setActiveTab(tab as Tab)} className="px-5 pt-2">
      ...
      <TabsContent forceMount value="overview" hidden={activeTab !== 'overview'} data-tab-panel="overview" className="min-h-[22rem] py-5">
        <div className="grid gap-6">
          ...
          <div className="grid gap-2 md:grid-cols-[12rem_minmax(0,1fr)] md:items-start"><span className="pt-2 text-sm font-medium">关联工作流</span><div>{isNew ? <Select aria-label="关联工作流" value={value.workflowId || null} options={workflowOptions.map(option => ({ value: option.id, label: option.name }))} clearable={false} disabled={locked} errorMessage={errors.workflowId} onValueChange={workflowId => workflowId && change('workflowId', workflowId)}/> : <div className="flex flex-wrap items-center justify-between gap-3 rounded-control border border-line bg-surface-subtle p-3"><span className="flex min-w-0 flex-wrap items-center gap-3"><FlowArrow size={28} className="shrink-0 text-clay" aria-hidden/><strong className="break-words">{workflow?.name ?? '关联工作流暂不可用'}</strong><small className="text-muted">{workflow?.runnable === true ? '可以运行' : workflow?.runnable === false ? '不可运行' : '未能读取校验状态'}</small>{workflow?.updatedAt && Number.isFinite(Date.parse(workflow.updatedAt)) ? <small className="flex items-center gap-1 text-muted"><Clock aria-hidden/>最近保存 {new Intl.DateTimeFormat('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(workflow.updatedAt))}</small> : null}</span>{onOpenStudio ? <Button size="sm" disabled={locked} onClick={onOpenStudio}>打开 Studio</Button> : null}</div>}<p className="mb-0 mt-2 text-xs text-muted">关联建立后不能通过普通编辑替换工作流</p></div></div>
      ...
      <TabsContent forceMount value="resources" hidden={activeTab !== 'resources'} data-tab-panel="resources" className="min-h-[22rem] py-5"><EnvironmentPolicyEditor value={value.environmentPolicy} onChange={next => change('environmentPolicy', next)} disabled={locked} errors={fieldErrors('environmentPolicy.')} {...environmentOptions}/></TabsContent>
      <TabsContent forceMount value="run" hidden={activeTab !== 'run'} data-tab-panel="run" className="min-h-[22rem] py-5"><RunPolicyEditor value={value.runPolicy} onChange={next => change('runPolicy', { ...next, maxTasks: next.maxTasks ?? value.runPolicy.maxTasks })} disabled={locked} errors={fieldErrors('runPolicy.')} resetKey={resetKey} onDraftStateChange={setRunDraft}/></TabsContent>
    </Tabs>
    {activeTab !== 'resources' ? <aside aria-label="配置摘要" className="mx-5 flex flex-wrap items-center gap-4 border-t border-line py-4 text-sm"><strong className="mr-4">配置摘要</strong><span className="flex items-center gap-2 border-l border-line pl-4"><SlidersHorizontal size={21} aria-hidden/>{value.parameterSchema.length} 个参数</span><span className="flex items-center gap-2 border-l border-line pl-4"><Browser size={21} aria-hidden/>{value.environmentPolicy.source === 'newFromProfile' ? '临时浏览器环境' : '已保存的环境策略'}</span><span className="flex items-center gap-2 border-l border-line pl-4"><FileText size={21} aria-hidden/>最多 {value.runPolicy.maxTasks} 个任务</span><span className="flex items-center gap-2 border-l border-line pl-4"><Lightning size={21} aria-hidden/>并发 1</span></aside> : null}
