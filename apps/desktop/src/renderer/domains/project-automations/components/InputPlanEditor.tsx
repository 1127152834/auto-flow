...
export type InputTableOption = { id: string; name: string; datasetGeneration: string; fields: FieldOption[]; statuses: Schema['DataStatusView'][]; slotDefinitions: Schema['TableSlotDefinition'][]; records?: { label: string; ref: RecordRef }[] }
...
const tableOptions = (tables: InputTableOption[]) => tables.map(table => ({ value: table.id, label: table.name }))
...
export function InputPlanEditor({ value, onChange, tables, disabled = false, errors = {}, resetKey = '', onDraftStateChange, onLoadRecords }: InputPlanEditorProps) {
  ...
  return <section className="grid gap-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="m-0 text-base font-semibold">项目数据输入</h3><p className="mt-1 text-sm text-warning">{value.inputs.length ? '项目数据执行能力暂未开放；当前输入配置仍可保存。' : '当前自动化不读取项目数据，可以直接使用参数启动。'}</p></div><Button size="sm" disabled={disabled || !tables.length} onClick={add}>添加数据输入</Button></div>
    {value.inputs.map((input, index) => {
      ...
      return <article key={input.inputId} className="grid min-w-0 gap-4 rounded-control border border-line bg-surface p-4" tabIndex={inputErrors.length ? -1 : undefined} aria-invalid={inputErrors.length ? true : undefined}>
        ...
        <div className="grid min-w-0 gap-3 sm:grid-cols-2">
          <Select aria-label={`数据表 ${input.alias}`} value={input.tableId} options={tableOptions(tables)} clearable={false} disabled={disabled || Boolean(reference) || input.mode === 'related'} errorMessage={tableError} onValueChange={tableId => {
        ...
        {input.mode === 'fixedRecord' ? <div className="grid gap-2"><div className="flex flex-wrap gap-2"><Select className="min-w-64 flex-1" aria-label={`固定记录 ${input.alias}`} value={input.fixedRecord ? recordKey(input.fixedRecord) : null} options={(table?.records ?? []).map(record => ({ value: recordKey(record.ref), label: record.label }))} disabled={disabled} errorMessage={errors[`${input.inputId}.fixedRecord`]} onValueChange={key => replace(index, { ...input, fixedRecord: table?.records?.find(record => recordKey(record.ref) === key)?.ref ?? null })}/>{onLoadRecords ? <Button disabled={disabled} onClick={() => onLoadRecords(input.tableId)}>读取{table?.name ?? '数据表'}记录</Button> : null}</div></div> : null}
        {input.mode === 'related' ? <fieldset className="grid gap-3 rounded-control bg-surface-subtle p-3"><legend className="text-sm font-medium">关联输入</legend>
          <div className="grid gap-2 sm:grid-cols-2"><Select aria-label={`来源输入 ${input.alias}`} value={input.relation?.sourceInputId ?? null} options={sourceCandidates.map(item => ({ value: item.inputId, label: item.alias || '未命名输入', disabled: !relatedCandidates.includes(item) }))} clearable={false} disabled={disabled} onValueChange={sourceInputId => { if (!sourceInputId) return; const candidate = sourceCandidates.find(item => item.inputId === sourceInputId); if (!candidate) return; const relation = relationFor(relationType, sourceInputId) ?? firstRelationFor(candidate); if (relation) replace(index, { ...input, relation }) }}/><Select aria-label={`关联方式 ${input.alias}`} value={relationType} options={[{ value: 'sameRecord', label: '同一记录', disabled: !capabilities.sameRecord }, { value: 'fieldEquals', label: '字段相等', disabled: !capabilities.fieldEquals }, { value: 'recordSlot', label: '记录槽', disabled: !capabilities.recordSlot }]} clearable={false} disabled={disabled} onValueChange={type => { if (!type || !sourceInput) return; const relation = relationFor(type, sourceInput.inputId); if (relation) replace(index, { ...input, relation }) }}/></div>
          {input.relation?.type === 'fieldEquals' ? <div className="grid gap-2 sm:grid-cols-2"><Select aria-label={`来源字段 ${input.alias}`} value={input.relation.sourceFieldRef.fieldId} options={sourceFields.map(field => ({ value: field.ref.fieldId, label: field.name }))} clearable={false} disabled={disabled} onValueChange={fieldId => { const field = sourceFields.find(item => item.ref.fieldId === fieldId), target = table?.fields.find(item => item.type === field?.type); if (field && target && input.relation?.type === 'fieldEquals') replace(index, { ...input, relation: { ...input.relation, sourceFieldRef: field.ref, targetFieldRef: target.ref } }) }}/><Select aria-label={`目标字段 ${input.alias}`} value={input.relation.targetFieldRef.fieldId} options={(table?.fields ?? []).filter(field => field.type === relationSourceFieldType).map(field => ({ value: field.ref.fieldId, label: field.name }))} clearable={false} disabled={disabled} onValueChange={fieldId => { const field = table?.fields.find(item => item.ref.fieldId === fieldId); if (field && input.relation?.type === 'fieldEquals') replace(index, { ...input, relation: { ...input.relation, targetFieldRef: field.ref } }) }}/></div> : null}
          {input.relation?.type === 'recordSlot' ? <Select aria-label={`记录槽 ${input.alias}`} value={input.relation.slotId} options={slots.map(slot => ({ value: slot.slotId, label: slot.name }))} clearable={false} disabled={disabled} onValueChange={slotId => { if (slotId && input.relation?.type === 'recordSlot') replace(index, { ...input, relation: { ...input.relation, slotId } }) }}/> : null}
          {!slots.length ? <p className="m-0 text-xs text-muted">来源表没有指向当前表的记录槽，不能选择记录槽关系。</p> : null}
        </fieldset> : null}
        <fieldset className="grid gap-3"><legend className="text-sm font-medium">字段映射</legend>{input.fieldBindings.map((binding, bindingIndex) => <div className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]" key={binding.inputFieldId}><Input aria-label={`字段别名 ${binding.inputFieldAlias}`} value={binding.inputFieldAlias} disabled={disabled} onChange={event => replace(index, { ...input, fieldBindings: input.fieldBindings.map((item, itemIndex) => itemIndex === bindingIndex ? { ...item, inputFieldAlias: event.target.value } : item) })}/><Select aria-label={`映射字段 ${binding.inputFieldAlias}`} value={binding.fieldRef.fieldId} options={(table?.fields ?? []).map(field => ({ value: field.ref.fieldId, label: field.name }))} clearable={false} disabled={disabled} onValueChange={fieldId => { const field = table?.fields.find(item => item.ref.fieldId === fieldId); if (field) replace(index, { ...input, fieldBindings: input.fieldBindings.map((item, itemIndex) => itemIndex === bindingIndex ? { ...item, fieldRef: field.ref } : item) }) }}/><Button size="sm" variant="ghost" disabled={disabled} onClick={() => replace(index, { ...input, fieldBindings: input.fieldBindings.filter((_, itemIndex) => itemIndex !== bindingIndex) })}>移除映射</Button></div>)}<Button className="justify-self-start" size="sm" disabled={disabled || !table?.fields.length} aria-label={`添加字段映射 ${input.alias}`} onClick={() => { const field = table?.fields.find(candidate => !input.fieldBindings.some(binding => binding.fieldRef.fieldId === candidate.ref.fieldId)); if (field) replace(index, { ...input, fieldBindings: [...input.fieldBindings, { inputFieldId: crypto.randomUUID(), inputFieldAlias: field.name, fieldRef: field.ref }] }) }}>添加字段映射</Button></fieldset>
        {table ? <div tabIndex={filterDrafts[input.inputId] ? -1 : undefined} aria-invalid={filterDrafts[input.inputId] ? true : undefined}>
          {!boundFields.length ? <p className="mb-2 text-sm text-warning">字段筛选和字段排序需要先添加字段映射；状态条件和系统字段排序仍可使用。</p> : null}
