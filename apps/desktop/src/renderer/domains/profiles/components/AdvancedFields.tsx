...
export function AdvancedFields() {
  ...
  return <section aria-labelledby="profile-advanced-fields" className="grid gap-5">
    ...
    <Disclosure summary="查看填写说明" className="rounded-control border border-line bg-surface-subtle p-4 text-sm text-ink">
    ...
    <FormField label="扩展目录（每行一个）" htmlFor="profile-extension-paths" hint="只填写已解压扩展的文件夹路径。">{(a11y) => <>
      <Textarea {...a11y} rows={4} placeholder={'例如：\nD:\\AutoFlow\\extensions\\my-extension'} {...register('extensionPathsText')} />
    </>}</FormField>
    <FormField label="高级参数（每行一个）" htmlFor="profile-expert-args" error={errors.expertArgsText?.message} hint="每一行会作为一个独立 Chromium 启动参数。">{(a11y) => <>
      <Textarea {...a11y} rows={5} placeholder={'例如：\n--disable-notifications\n--window-position=40,40'} {...register('expertArgsText')} />
    </>}</FormField>
  </section>
}
