...
it('offers all server options despite current defaults and supports custom input', async () => {
  ...
  render(<Harness />)
  expect(choiceValue(screen.getByLabelText('浏览器语言'))).toBe('zh-CN')
  ...
  await chooseOption(user, screen.getByLabelText('浏览器语言'), '__custom__')
  await chooseOption(user, screen.getByLabelText('浏览器时区'), '__custom__')
  ...
  expect(choiceValue(screen.getByLabelText('浏览器语言'))).toBe('de-CH-1901')
  await chooseOption(user, screen.getByLabelText('浏览器语言'), '')
...
it('keeps unlisted saved values while the catalog loads or fails, and offers retry', async () => {
  ...
  expect(choiceValue(screen.getByLabelText('浏览器语言'))).toBe(initial.locale)
  rerender(<Harness initial={initial} error="连接失败" retry={retry} />)
  expect(screen.getByRole('alert')).toHaveTextContent('连接失败')
  expect(choiceValue(screen.getByLabelText('浏览器时区'))).toBe(initial.timezone)
...
it('applies viewport presets and supports custom integer dimensions', async () => {
  const user = userEvent.setup()
  render(<Harness />)
  await chooseOption(user, screen.getByLabelText('视口预设'), '1920x1080')
...
it('moves an unspecified viewport explicitly to a preset or custom size', async () => {
  ...
  expect(choiceValue(screen.getByLabelText('视口预设'))).toBe('')
  await chooseOption(user, screen.getByLabelText('视口预设'), '1920x1080')
...
it('offers a kernel-major UA preset and binds environment preferences', async () => {
  const user = userEvent.setup()
  render(<Harness />)
  screen.getByLabelText('User Agent').focus(); await user.keyboard('{ArrowDown}')
  expect(screen.getByRole('option', {name:'后端 UA · Chromium 146'})).toBeInTheDocument()
  await user.keyboard('{Escape}')
  expect(choiceValue(screen.getByLabelText('User Agent'))).toBe('')
  await chooseOption(user, screen.getByLabelText('User Agent'), 'Server UA Chrome/146.0.0.0')
  expect(screen.getByLabelText('当前 User Agent')).toHaveTextContent('Server UA Chrome/146.0.0.0')
  await chooseOption(user, screen.getByLabelText('色彩模式'), 'dark')
  await chooseOption(user, screen.getByLabelText('人类行为预设'), 'careful')
  await chooseOption(user, screen.getByLabelText('User Agent'), '__custom__')
  ...
  expect(choiceValue(screen.getByLabelText('User Agent'))).toBe('Custom UA')
  await chooseOption(user, screen.getByLabelText('User Agent'), '')
...
it('refreshes UA choices on kernel changes without overwriting saved or custom UA values', async () => {
  ...
  await user.keyboard('{Escape}')
  expect(choiceValue(screen.getByLabelText('User Agent'))).toBe('Server UA Chrome/146.0.0.0')
  expect(screen.getByText(/当前 User Agent 的 Chromium 146 与所选内核不一致/)).toBeInTheDocument()
  await chooseOption(user, screen.getByLabelText('User Agent'), 'Server UA Chrome/147.0.0.0')
  ...
  expect(choiceValue(screen.getByLabelText('User Agent'))).toBe('Server UA Chrome/147.0.0.0')
})
