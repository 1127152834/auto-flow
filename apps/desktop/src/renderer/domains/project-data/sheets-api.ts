...
export function createSheetsApi(client: StreamingApiClient, desktop: Partial<GoogleSheetsBridge>, projectId: string) {
  ...
  return {
    ...
    connect: async (accountLabel: string, authorizationToken: string, key: string, current: () => boolean) =>
      (await command.submit(`${base}/sheets/connections`, { accountLabel, authorizationToken }, key, 'createSheetsConnection', current)),
    disconnect: async (connectionId: string, body: Schema['SheetsConnectionDelete'], key: string, current: () => boolean) =>
      (await command.submit(`${base}/sheets/connections/${encodeURIComponent(connectionId)}`, body, key, 'createSheetsConnection', current)),
    lookupConnection: async (key: string, current: () => boolean) =>
      (await command.lookup(key, 'createSheetsConnection', current)),
    ...
    putBinding: async (tableId: string, body: SheetsBindingWrite, key: string, current: () => boolean) =>
      (await command.submit(`${table(tableId)}/sheets/binding`, body, key, 'bindSheets', current)),
    lookupBinding: async (key: string, current: () => boolean) =>
      (await command.lookup(key, 'bindSheets', current)),
    ...
    pull: async (tableId: string, expectedTableRevision: number, key: string, current: () => boolean) =>
      (await command.submit(`${table(tableId)}/sync/pull`, { expectedTableRevision }, key, 'pullSheets', current)),
    lookupPull: async (key: string, current: () => boolean) => (await command.lookup(key, 'pullSheets', current)),
    push: async (tableId: string, mode: 'due' | 'allPending', expectedBindingEpoch: number, key: string, current: () => boolean) =>
      (await command.submit(`${table(tableId)}/sync/push`, { mode, expectedBindingEpoch }, key, 'pushSheets', current)),
    lookupPush: async (key: string, current: () => boolean) => (await command.lookup(key, 'pushSheets', current)),
    ...
    reconcile: async (tableId: string, syncOperationId: string, expectedStatusRevision: number, key: string, current: () => boolean) =>
      (await command.submit(`${table(tableId)}/sync-operations/${encodeURIComponent(syncOperationId)}/reconcile`, { expectedStatusRevision }, key, 'reconcileSheets', current)),
    lookupReconcile: async (key: string, current: () => boolean) => (await command.lookup(key, 'reconcileSheets', current)),
