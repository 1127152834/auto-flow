...
it('shows real table facts and keeps open separate from edit', async () => {
  ...
  render(<DataTableDirectory items={[table]} onRetry={vi.fn()} onCreate={vi.fn()} onOpen={onOpen} onEdit={onEdit} />)
  ...
  expect(onOpen).toHaveBeenCalledWith('t1'); expect(onEdit).toHaveBeenCalledWith('t1')
})

it('distinguishes loading, empty, filtered empty, and stale error states', async () => {
  const retry = vi.fn(); const props = { items: [] as DataTableView[], onRetry: retry, onCreate: vi.fn(), onOpen: vi.fn(), onEdit: vi.fn() }
...
it('keeps archived tables openable while hiding mutating actions', async () => {
  const onOpen = vi.fn()
  render(<DataTableDirectory items={[table]} readonly onRetry={vi.fn()} onCreate={vi.fn()} onOpen={onOpen} onEdit={vi.fn()} />)
