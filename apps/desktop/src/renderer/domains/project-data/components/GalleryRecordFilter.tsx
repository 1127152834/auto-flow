...
type SimpleFilter = { compare?: Extract<FilterDraft,{type:'compare'}>; status?: Extract<FilterDraft,{type:'status'}> }
...
function simpleFilter(filter:FilterDraft):SimpleFilter|null {
  ...
  return compare.length<=1&&status.length<=1&&status.every(item=>item.operator==='eq')&&compare.length+status.length===items.length?{compare:compare[0],status:status[0]}:null
}
