...
import { Select as SharedSelect } from '../../../shared/components/ui/select'
...
function Select({ missingLabel, ...props }: ComponentProps<typeof SharedSelect> & { missingLabel?: string }) {
  const value = props.value == null ? null : String(props.value)
  const options = value && missingLabel && !props.options.some(option => option.value === value)
    ? [{ value, label: missingLabel, disabled: true }, ...props.options]
    : props.options
  return <SharedSelect {...props} options={options} />
}
...
export function GalleryRecordFilter({fields,statuses,filter,disabled=false,dirty=false,error=null,onChange}:{fields:Field[];statuses:Status[];filter:FilterDraft;disabled?:boolean;dirty?:boolean;error?:RecordQueryError|null;onChange(filter:FilterDraft):void}) {
  ...
  if(advanced)return <div className="grid gap-3"><div className="flex items-center justify-between"><span className="text-base font-medium">高级条件</span><div className="flex items-center gap-2">{dirty?<span className="rounded-control bg-clay-soft px-2 py-1 text-xs text-clay">尚未应用</span>:null}<Button type="button" size="sm" variant="ghost" disabled={disabled||simple===null} onClick={()=>setAdvanced(false)}>使用简单条件</Button></div></div>{simple===null?<p className="text-sm text-muted">请先在高级条件中调整为一个字段条件和一个业务状态条件。</p>:null}<section aria-label="高级条件编辑器"><RecordFilterDraftEditor fields={fields} statuses={statuses} filter={filter} disabled={disabled} error={error} onChange={onChange}/></section></div>
  ...
  const basicValue=current.compare&&field&&!isNull&&(field.type==='number'||field.type==='string'&&!/[\r\n]/.test(current.compare.value.text))&&current.compare.value.presence==='value'
  return <div data-testid="gallery-simple-filter" className="grid gap-4 [&_[data-af-control]]:text-base">
    <section className="grid gap-3"><div className="flex items-center justify-between"><h3 className="m-0 text-base font-semibold">字段条件</h3><div className="flex items-center gap-2">{dirty?<span className="rounded-control bg-clay-soft px-2 py-1 text-xs text-clay">尚未应用</span>:null}<Button type="button" size="sm" variant="ghost" onClick={()=>setAdvanced(true)}>高级条件</Button></div></div><div className="grid grid-cols-2 gap-2"><Select missingLabel="字段已失效" aria-label="字段" value={current.compare?.fieldId??null} disabled={disabled} options={fields.map(item=>({value:item.ref.fieldId,label:item.name}))} onValueChange={fieldId=>{const next=fields.find(item=>item.ref.fieldId===fieldId);onChange(combine({...current,compare:next?{type:'compare',fieldId:next.ref.fieldId,operator:'eq',value:emptyValue(next.type)}:undefined}))}}/><Select aria-label="字段运算符" value={current.compare?.operator??null} disabled={disabled||!field} clearable={false} options={(field?operatorsFor(field.type):[]).map(value=>({value,label:labels[value]}))} onValueChange={operator=>operator&&current.compare&&onChange(combine({...current,compare:{...current.compare,operator,value:operator==='isNull'||operator==='isNotNull'?scalarDraft(undefined):current.compare.value}}))}/></div>{basicValue?<div className="grid gap-1"><Input className="text-base" aria-label="比较值" inputMode={field.type==='number'?'decimal':undefined} value={current.compare!.value.text} disabled={disabled} aria-invalid={Boolean(fieldError)} onChange={event=>onChange(combine({...current,compare:{...current.compare!,value:{...current.compare!.value,text:event.target.value}}}))}/>{fieldError?<p role="alert" className="text-xs text-danger">{fieldError.message}</p>:null}</div>:current.compare&&field&&!isNull?<ScalarValueEditor compact presenceDisplay="contextual" id="simple-filter-value" label="比较值" type={field.type} draft={current.compare.value} disabled={disabled} error={fieldError?.message} errorTarget={fieldError?.control} onChange={value=>onChange(combine({...current,compare:{...current.compare!,value}}))}/>:null}{fieldError&&!field?<p role="alert" className="text-xs text-danger">{fieldError.message}</p>:null}</section>
    <div className="h-px bg-line" />
    <section className="grid gap-2"><h3 className="m-0 text-base font-semibold">业务状态</h3><Select missingLabel="状态不可用" aria-label="业务状态" value={current.status?.statusId??null} disabled={disabled} options={statuses.map(item=>({value:item.statusId,label:item.name}))} onValueChange={statusId=>onChange(combine({...current,status:statusId?{type:'status',operator:'eq',statusId}:undefined}))}/>{statusError?<p role="alert" className="text-xs text-danger">{statusError.message}</p>:null}</section>
    <p className="flex items-center gap-2 text-sm text-muted"><Info className="shrink-0"/>两项条件同时满足时显示；业务状态仅属于本项目。</p>{error&&!fieldError&&!statusError?<p role="alert" className="text-xs text-danger">{error.message}</p>:null}
  </div>
}
