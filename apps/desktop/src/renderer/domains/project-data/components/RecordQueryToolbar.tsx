...
export type RecordQueryToolbarProps = {
  ...
  disabled?: boolean; readonly?: boolean; exportDisabled?: boolean; queryError?: string; totalCount?: number; selectionCount: number; onClearSelection(): void; onCreate(): void; onBatchStatus(): void; onExport(): void
...
export function RecordQueryToolbar({ fields, statuses, query, visibleFieldIds, quickSearch, disabled=false, readonly=false, exportDisabled=false, queryError, totalCount, selectionCount, onClearSelection, onCreate, onBatchStatus, onExport, onApplyQuery, onApplyColumns, onApplySearch, resetKey }: RecordQueryToolbarProps) {
  ...
  return <section aria-label="记录查询工具" className="grid min-w-0 gap-3">
    <div role="toolbar" aria-label="记录工具" className="flex min-w-0 flex-wrap items-center gap-2">
      {totalCount!==undefined?<div className="mr-auto flex items-baseline gap-2 whitespace-nowrap"><strong className="text-xl">{totalCount.toLocaleString()} 条</strong><span className="text-muted">本地记录</span></div>:<span className="mr-auto"/>}
      <form role="search" className="flex min-w-0 max-w-md flex-1 gap-2" onSubmit={submitSearch}><Select className="w-32 max-w-full" aria-label="搜索字段" value={search.fieldId} disabled={!enabled||!canSearch} clearable={false} options={textFields.map(field=>({value:field.ref.fieldId,label:field.name}))} onValueChange={fieldId=>{setSubmitError(false);setSearch(value=>({...value,fieldId}))}}/><SearchInput className="min-w-32 flex-1" aria-label="文本搜索" disabled={!enabled||!canSearch} value={search.keyword} clearLabel="清除文本搜索" onChange={event=>{setSubmitError(false);setSearch(value=>({...value,keyword:event.target.value}))}} onClear={()=>{const next={fieldId:search.fieldId,keyword:''};if(same(next,quickSearch)){setSubmitError(false);setSearch(next);appliedSearchRef.current=next}else applySearch(next)}}/><IconButton type="submit" variant="secondary" aria-label="搜索记录" disabled={!enabled||!canSearch}><MagnifyingGlass/></IconButton></form>
      <Panel kind="filter" applied={appliedFilter} open={panel==='filter'} onOpenChange={value=>value?open('filter'):setPanel(current=>current==='filter'?null:current)} trigger="筛选" label="记录筛选" icon={<Funnel />} disabled={!enabled} footer={<><Button className="mr-auto" variant="ghost" disabled={!enabled||(draft.filter.type==='all'&&draft.filter.items.length===0)} onClick={()=>{setDraft(value=>({...value,filter:{type:'all',items:[]}}));setError(null)}}>清空条件</Button><Button variant="ghost" onClick={close}>取消</Button><Button variant="primary" disabled={!enabled} onClick={()=>applyQuery('filter')}>应用筛选</Button></>}><div className="grid gap-4">{filterDirty?<span className="justify-self-end rounded-full bg-clay-soft px-2 py-1 text-xs text-clay">尚未应用</span>:null}<GalleryRecordFilter fields={fields} statuses={statuses} filter={draft.filter} disabled={!enabled} error={error} onChange={filter=>{setDraft(value=>({...value,filter}));setError(null)}}/>{showQueryError?<p ref={errorRef} tabIndex={-1} role="alert" className="text-sm text-danger">{queryErrorMessage}</p>:null}</div></Panel>
