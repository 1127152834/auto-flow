...
function Panel({ open, onOpenChange, kind, trigger, label, icon, applied, disabled, footer, children }: { open: boolean; onOpenChange(open:boolean):void; kind:Exclude<QueryPanel,null>; trigger:string; label:string; icon:ReactNode; applied:boolean; disabled?:boolean; footer:ReactNode; children:ReactNode }) {
  return <Popover open={open} onOpenChange={onOpenChange}><PopoverTrigger asChild><Button type="button" variant="secondary" aria-label={trigger} disabled={disabled} data-query-applied={applied} data-query-panel-trigger data-record-action={kind}>{icon}{trigger}{applied ? <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-clay" /> : null}</Button></PopoverTrigger><PopoverContent sticky="always" align="center" aria-label={label} data-query-panel-size={kind} className={`af-table-query-panel ${panelWidth[kind]} overflow-hidden p-0`} onInteractOutside={event=>{if((event.target as Element).closest('[data-query-panel-trigger]'))event.preventDefault()}}><div className="flex max-h-[min(var(--radix-popover-content-available-height),calc(100dvh-1.5rem))] flex-col"><header className="flex shrink-0 items-center justify-between border-b border-line px-3 py-2"><h2 className={`m-0 font-semibold ${'text-sm'}`}>{label}</h2><Button type="button" variant="ghost" className="h-8 w-8 p-0" aria-label={`关闭${label}`} onClick={()=>onOpenChange(false)}><X /></Button></header><div data-query-panel-body className="min-h-0 flex-1 overflow-y-auto p-3">{children}</div><footer data-query-panel-footer className="flex shrink-0 justify-end gap-2 border-t border-line px-3 py-2">{footer}</footer></div></PopoverContent></Popover>
}
...
export function RecordQueryToolbar({ fields, statuses, query, visibleFieldIds, quickSearch, queryLocked=false, createLabel='新增记录', createDisabled=false, disabled=false, readonly=false, exportDisabled=false, queryError, totalCount, selectionCount, onClearSelection, onCreate, onBatchStatus, onExport, onReimport, onApplyQuery, onApplyColumns, onApplySearch, resetKey }: RecordQueryToolbarProps) {
  ...
  return <section aria-label="记录查询工具" className="grid min-w-0 gap-3">
    <TableToolbar label="记录工具">
      {totalCount!==undefined?<div className="mr-auto flex items-baseline gap-2 whitespace-nowrap"><strong className="text-sm font-semibold">{totalCount.toLocaleString()} 条</strong><span className="text-muted">本地记录</span></div>:<span className="mr-auto"/>}
      <form role="search" className="af-table-search" onSubmit={submitSearch}><Select className="w-32 max-w-full" aria-label="搜索字段" value={search.fieldId} disabled={!enabled||!canSearch} clearable={false} options={textFields.map(field=>({value:field.ref.fieldId,label:field.name}))} onValueChange={fieldId=>{setSubmitError(false);setSearch(value=>({...value,fieldId}))}}/><SearchInput leadingIcon={false} className="min-w-0" aria-label="文本搜索" placeholder="搜索记录内容" disabled={!enabled||!canSearch} value={search.keyword} clearLabel="清除文本搜索" onChange={event=>{setSubmitError(false);setSearch(value=>({...value,keyword:event.target.value}))}} onClear={()=>{const next={fieldId:search.fieldId,keyword:''};if(same(next,quickSearch)){setSubmitError(false);setSearch(next);appliedSearchRef.current=next}else applySearch(next)}}/><IconButton type="submit" variant="secondary" className="h-8 w-8" aria-label="搜索记录" disabled={!enabled||!canSearch}><MagnifyingGlass/></IconButton></form>
