...
import { ArrowsDownUp, Columns, DotsThree, DownloadSimple, Funnel, MagnifyingGlass, Plus, Stack, X } from '@phosphor-icons/react'
...
import { RecordSortDraftEditor } from './RecordFilterEditor'
...
export type RecordQueryToolbarProps = {
  ...
  queryLocked?: boolean; createLabel?: string; createDisabled?: boolean; disabled?: boolean; readonly?: boolean; exportDisabled?: boolean; queryError?: string; totalCount?: number; selectionCount: number; onClearSelection(): void; onCreate(): void; onBatchStatus(): void; onExport(): void; onReimport?(): void
...
function Panel({ open, onOpenChange, kind, trigger, label, icon, applied, disabled, footer, children }: { open: boolean; onOpenChange(open:boolean):void; kind:Exclude<QueryPanel,null>; trigger:string; label:string; icon:ReactNode; applied:boolean; disabled?:boolean; footer:ReactNode; children:ReactNode }) {
  return <Popover open={open} onOpenChange={onOpenChange}><PopoverTrigger asChild><Button type="button" variant="secondary" aria-label={trigger} disabled={disabled} data-query-applied={applied} data-query-panel-trigger data-record-action={kind}>{icon}{trigger}{applied ? <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-clay" /> : null}</Button></PopoverTrigger><PopoverContent sticky="always" align="center" aria-label={label} data-query-panel-size={kind} className={`${panelWidth[kind]} overflow-hidden p-0`} onInteractOutside={event=>{if((event.target as Element).closest('[data-query-panel-trigger]'))event.preventDefault()}}><div className="flex max-h-[min(var(--radix-popover-content-available-height),calc(100dvh-1.5rem))] flex-col"><header className="flex shrink-0 items-center justify-between border-b border-line px-4 py-3"><h2 className={`m-0 font-semibold ${kind==='filter'?'text-xl':'text-base'}`}>{label}</h2><Button type="button" variant="ghost" className="h-8 w-8 p-0" aria-label={`关闭${label}`} onClick={()=>onOpenChange(false)}><X /></Button></header><div data-query-panel-body className="min-h-0 flex-1 overflow-y-auto p-4">{children}</div><footer data-query-panel-footer className="flex shrink-0 justify-end gap-2 border-t border-line px-4 py-3 [&_button]:h-10 [&_button]:text-base">{footer}</footer></div></PopoverContent></Popover>
}

export function RecordQueryToolbar({ fields, statuses, query, visibleFieldIds, quickSearch, queryLocked=false, createLabel='新增记录', createDisabled=false, disabled=false, readonly=false, exportDisabled=false, queryError, totalCount, selectionCount, onClearSelection, onCreate, onBatchStatus, onExport, onReimport, onApplyQuery, onApplyColumns, onApplySearch, resetKey }: RecordQueryToolbarProps) {
  ...
  const enabled=!disabled&&!queryLocked, canSearch=textFields.length>0
  ...
  const filterDirty=!same(draft.filter,recordQueryDraft(query).filter)
  return <section aria-label="记录查询工具" className="grid min-w-0 gap-3">
    <div role="toolbar" aria-label="记录工具" className="flex min-w-0 flex-wrap items-center gap-2 [&_[data-record-action]]:h-12 [&_[data-record-action]]:text-base">
      {totalCount!==undefined?<div className="mr-auto flex items-baseline gap-2 whitespace-nowrap"><strong className="text-[28px] leading-none">{totalCount.toLocaleString()} 条</strong><span className="text-muted">本地记录</span></div>:<span className="mr-auto"/>}
      <form role="search" className="flex min-w-[min(100%,20rem)] max-w-md flex-1 gap-2 [&_[data-af-control]]:h-12 [&_[data-af-control]]:text-base" onSubmit={submitSearch}><Select className="w-32 max-w-full" aria-label="搜索字段" value={search.fieldId} disabled={!enabled||!canSearch} clearable={false} options={textFields.map(field=>({value:field.ref.fieldId,label:field.name}))} onValueChange={fieldId=>{setSubmitError(false);setSearch(value=>({...value,fieldId}))}}/><SearchInput className="min-w-32 flex-1" aria-label="文本搜索" placeholder="搜索记录内容" disabled={!enabled||!canSearch} value={search.keyword} clearLabel="清除文本搜索" onChange={event=>{setSubmitError(false);setSearch(value=>({...value,keyword:event.target.value}))}} onClear={()=>{const next={fieldId:search.fieldId,keyword:''};if(same(next,quickSearch)){setSubmitError(false);setSearch(next);appliedSearchRef.current=next}else applySearch(next)}}/><IconButton type="submit" variant="secondary" className="h-12 w-12 text-base" aria-label="搜索记录" disabled={!enabled||!canSearch}><MagnifyingGlass/></IconButton></form>
      <Panel kind="filter" applied={appliedFilter} open={panel==='filter'} onOpenChange={value=>value?open('filter'):setPanel(current=>current==='filter'?null:current)} trigger="筛选" label="记录筛选" icon={<Funnel />} disabled={!enabled} footer={<><Button className="mr-auto" variant="ghost" disabled={!enabled||(draft.filter.type==='all'&&draft.filter.items.length===0)} onClick={()=>{setDraft(value=>({...value,filter:{type:'all',items:[]}}));setError(null)}}>清空条件</Button><Button variant="secondary" onClick={close}>取消</Button><Button variant="primary" disabled={!enabled} onClick={()=>applyQuery('filter')}>应用筛选</Button></>}><div className="grid gap-4"><GalleryRecordFilter dirty={filterDirty} fields={fields} statuses={statuses} filter={draft.filter} disabled={!enabled} error={error} onChange={filter=>{setDraft(value=>({...value,filter}));setError(null)}}/>{showQueryError?<p ref={errorRef} tabIndex={-1} role="alert" className="text-sm text-danger">{queryErrorMessage}</p>:null}</div></Panel>
      <Panel kind="sort" applied={appliedSort} open={panel==='sort'} onOpenChange={value=>value?open('sort'):setPanel(current=>current==='sort'?null:current)} trigger="排序" label="记录排序" icon={<ArrowsDownUp />} disabled={!enabled} footer={<><Button variant="secondary" onClick={close}>取消</Button><Button variant="primary" disabled={!enabled} onClick={()=>applyQuery('sort')}>应用排序</Button></>}><div className="grid gap-4"><RecordSortDraftEditor fields={fields} orderBy={draft.orderBy} disabled={!enabled} error={error} onChange={orderBy=>{setDraft(value=>({...value,orderBy}));setError(null)}}/>{showQueryError?<p ref={errorRef} tabIndex={-1} role="alert" className="text-sm text-danger">{queryErrorMessage}</p>:null}</div></Panel>
      ...
      <DropdownMenu><DropdownMenuTrigger asChild><IconButton variant="secondary" aria-label="更多操作" data-record-action="more" disabled={!enabled}><DotsThree/></IconButton></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled={!enabled||readonly||selectionCount===0} onSelect={onBatchStatus}><Stack className="mr-2"/>批量设置状态</DropdownMenuItem><DropdownMenuSeparator/>{onReimport&&!readonly?<DropdownMenuItem disabled={!enabled} onSelect={onReimport}>重新导入 Excel</DropdownMenuItem>:null}<DropdownMenuItem disabled={!enabled||exportDisabled} onSelect={onExport}><DownloadSimple className="mr-2"/>导出 Excel</DropdownMenuItem></DropdownMenuContent></DropdownMenu>{!readonly?<Button variant="primary" data-record-action="create" disabled={disabled||createDisabled} onClick={onCreate}><Plus />{createLabel}</Button>:null}
    </div>
