...
function Panel({ open, onOpenChange, kind, trigger, label, icon, disabled, children }: { open: boolean; onOpenChange(open:boolean):void; kind:Exclude<QueryPanel,null>; trigger:string; label:string; icon:ReactNode; disabled?:boolean; children:ReactNode }) {
  return <Popover open={open} onOpenChange={onOpenChange}><PopoverTrigger asChild><Button type="button" variant="secondary" disabled={disabled} data-query-panel-trigger data-record-action={kind}>{icon}{trigger}</Button></PopoverTrigger><PopoverContent aria-label={label} data-query-panel-size={kind} className={`${panelWidth[kind]} overflow-hidden p-0`} onInteractOutside={event=>{if((event.target as Element).closest('[data-query-panel-trigger]'))event.preventDefault()}}><div className="flex max-h-[var(--radix-popover-content-available-height)] flex-col"><header className="flex shrink-0 items-center justify-between border-b border-line px-4 py-3"><h2 className="m-0 text-base font-semibold">{label}</h2><Button type="button" variant="ghost" className="h-8 w-8 p-0" aria-label={`关闭${label}`} onClick={()=>onOpenChange(false)}><X /></Button></header><div className="min-h-0 overflow-y-auto p-4">{children}</div></div></PopoverContent></Popover>
}
