...
function Panel({ open, onOpenChange, trigger, label, disabled, children }: { open: boolean; onOpenChange(open:boolean):void; trigger:string; label:string; disabled?:boolean; children:ReactNode }) {
  return <Popover open={open} onOpenChange={onOpenChange}><PopoverTrigger asChild><Button type="button" variant="ghost" disabled={disabled} data-query-panel-trigger>{trigger}</Button></PopoverTrigger><PopoverContent aria-label={label} onInteractOutside={event=>{if((event.target as Element).closest('[data-query-panel-trigger]'))event.preventDefault()}}>{children}</PopoverContent></Popover>
}
...
export function RecordQueryToolbar({ fields, statuses, query, visibleFieldIds, quickSearch, disabled=false, readonly=false, exportDisabled=false, queryError, selectionCount, onClearSelection, onCreate, onBatchStatus, onExport, onApplyQuery, onApplyColumns, onApplySearch, resetKey }: RecordQueryToolbarProps) {
  ...
  return <section aria-label="记录查询工具" className="grid min-w-0 gap-3">
    <div role="toolbar" aria-label="记录工具" className="flex min-w-0 flex-wrap items-center gap-2">
      ...
      <Panel open={panel==='filter'} onOpenChange={value=>value?open('filter'):setPanel(current=>current==='filter'?null:current)} trigger="筛选" label="记录筛选" disabled={!enabled}><div className="grid gap-4"><RecordFilterDraftEditor fields={fields} statuses={statuses} filter={draft.filter} disabled={!enabled} error={error} onChange={filter=>{setDraft(value=>({...value,filter}));setError(null)}}/>{showQueryError?<p ref={errorRef} tabIndex={-1} role="alert" className="text-sm text-danger">{queryErrorMessage}</p>:null}<div className="flex justify-end gap-2"><Button variant="ghost" onClick={close}>取消</Button><Button variant="primary" disabled={!enabled} onClick={()=>applyQuery('filter')}>应用筛选</Button></div></div></Panel>
      <Panel open={panel==='sort'} onOpenChange={value=>value?open('sort'):setPanel(current=>current==='sort'?null:current)} trigger="排序" label="记录排序" disabled={!enabled}><div className="grid gap-4"><RecordSortDraftEditor fields={fields} orderBy={draft.orderBy} disabled={!enabled} error={error} onChange={orderBy=>{setDraft(value=>({...value,orderBy}));setError(null)}}/>{showQueryError?<p ref={errorRef} tabIndex={-1} role="alert" className="text-sm text-danger">{queryErrorMessage}</p>:null}<div className="flex justify-end gap-2"><Button variant="ghost" onClick={close}>取消</Button><Button variant="primary" disabled={!enabled} onClick={()=>applyQuery('sort')}>应用排序</Button></div></div></Panel>
      <Panel open={panel==='columns'} onOpenChange={value=>value?open('columns'):setPanel(current=>current==='columns'?null:current)} trigger="显示列" label="显示列" disabled={!enabled}><div className="grid gap-3">{fields.map(field=><label key={field.ref.fieldId} className="flex items-center gap-2 text-sm"><Checkbox aria-label={field.name} checked={columns.includes(field.ref.fieldId)} disabled={!enabled} onCheckedChange={checked=>setColumns(value=>checked===true?[...value,field.ref.fieldId]:value.filter(id=>id!==field.ref.fieldId))}/><span>{field.name}</span></label>)}<div className="flex justify-end gap-2"><Button variant="ghost" onClick={close}>取消</Button><Button variant="primary" disabled={!enabled} onClick={()=>{const applied=visibleFieldIds??fields.map(field=>field.ref.fieldId);if(!same(columns,applied))onApplyColumns(columns);close()}}>应用显示列</Button></div></div></Panel>
      {!readonly?<Button disabled={!enabled} onClick={onCreate}>新增记录</Button>:null}<Button disabled={!enabled||readonly||selectionCount===0} onClick={onBatchStatus}>批量设置状态</Button><Button disabled={!enabled||exportDisabled} onClick={onExport}>导出 Excel</Button>
    </div>
