...
export function RecordGridCellEditor({ field, rowNumber, cell, active, disabled, error, onActivate, onChange, onMove, onPasteGrid, onSave, onUndo }: Props) {
  ...
  return <div ref={root} role="gridcell" aria-label={label} aria-readonly={locked || undefined} aria-invalid={Boolean(error) || undefined} aria-describedby={error ? errorId : undefined}
    ...
    className={`af-table-grid-cell relative min-h-8 min-w-0 rounded-sm border px-1 py-0.5 text-sm ${error ? 'border-danger' : active ? 'border-clay' : 'border-transparent'} focus:outline-2 focus:outline-focus`}
    ...
    </div> : <span className={`block min-w-0 truncate py-1 ${cell.presence === 'missing' ? 'text-muted' : 'text-ink'}`} title={value}>{cell.presence === 'missing' && (!field.writable || field.formula) ? '不可填写' : value}</span>}
