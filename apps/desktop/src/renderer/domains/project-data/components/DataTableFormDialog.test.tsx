...
it('renders the gallery create-table structure without changing the data limits', () => {
  ...
  expect(dialog).toHaveClass('[&>header_h2]:text-[28px]')
  expect(dialog.querySelector('header h2')).toBe(screen.getByRole('heading',{name:'新建数据表'}))
  expect(screen.getByRole('button',{name:'取消'})).toHaveClass('border-control-border')
  expect(screen.getByLabelText('数据表名称')).toHaveClass('h-12','text-base')
