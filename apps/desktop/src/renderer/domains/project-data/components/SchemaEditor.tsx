...
export function SchemaEditor(props: Props) {
  ...
  return <section aria-label="字段目录" className="grid min-w-0 grid-cols-1 gap-4">
    <div className="min-w-0 rounded-control border border-line bg-surface p-4">
      <header className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-2xl font-semibold">字段与校验</h3><p className="mt-1 text-base text-muted">定义业务字段与填写规则，业务状态单独管理。</p></div><Button className="h-12 px-5 text-base" variant="primary" disabled={frozen} onClick={() => setDrawer({ id: null, session: crypto.randomUUID() })}><Plus size={22} />新增字段</Button></header>
      ...
      <div className="overflow-x-auto rounded-control border border-line"><Table aria-label="字段与校验" className="min-w-[680px] table-fixed text-base [&_td]:border-r [&_td]:border-line [&_td:last-child]:border-r-0 [&_th]:border-r [&_th]:border-line [&_th:last-child]:border-r-0 [&_td]:px-4 [&_td]:py-4 [&_th]:px-4">
