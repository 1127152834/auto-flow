  ...
  return <>
    <Modal open={open} onOpenChange={next=>{if(!next)requestClose()}} closeDisabled={busy} size="small" className="max-w-[32.5rem] [&>header]:border-0 [&>header_h2]:text-[28px] [&>header_p]:text-base [&>footer]:border-0 [&>footer_button]:h-12 [&>footer_button]:min-w-28 [&>footer_button]:text-base" title={mode==='create'?'新建数据表':'编辑数据表'} description={mode==='create'?'先给数据一个用途，来源可以稍后配置。':'修改数据表名称和用途说明。'} footer={<><Button type="button" variant="secondary" disabled={busy} onClick={requestClose}>取消</Button>{recoveryPending?<Button type="button" variant="primary" disabled={busy||!onRecover} onClick={recover}>{recovering?'正在核对…':'核对保存结果'}</Button>:<Button type="submit" form="data-table-form" variant="primary" disabled={busy||readonly}>{busy?'正在保存…':mode==='create'?'创建数据表':'保存修改'}</Button>}</>}>
      <form id="data-table-form" className="grid gap-5" onSubmit={submit}>
        ...
        <div className="grid gap-2"><label className="text-base font-medium text-ink" htmlFor="data-table-name">数据表名称 <span aria-hidden="true" className="text-clay">*</span></label><Input className="h-12 text-base" id="data-table-name" aria-label="数据表名称" autoFocus readOnly={frozen||readonly} aria-invalid={form.formState.errors.name?true:undefined} aria-describedby={form.formState.errors.name?'data-table-name-error':undefined} {...form.register('name')}/>{form.formState.errors.name?<p id="data-table-name-error" role="alert" className="text-xs text-clay">{form.formState.errors.name.message}</p>:null}</div>
        <FormField className="[&>label]:text-base [&>p]:text-sm" label="用途说明（可选）" htmlFor="data-table-description" error={form.formState.errors.description?.message} hint="最多 1000 个字符"><Textarea className="text-base" rows={4} readOnly={frozen||readonly} {...form.register('description')}/></FormField>
