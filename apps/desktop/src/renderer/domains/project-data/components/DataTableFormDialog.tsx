  ...
  return <>
    <Modal open={open} onOpenChange={next=>{if(!next)requestClose()}} closeDisabled={busy} variant="form" size="small" title={mode==='create'?'新建数据表':'编辑数据表'} description="填写数据表名称和说明。" footer={<><Button type="button" variant="ghost" disabled={busy} onClick={requestClose}>取消</Button>{recoveryPending?<Button type="button" variant="primary" disabled={busy||!onRecover} onClick={recover}>{recovering?'正在核对…':'核对保存结果'}</Button>:<Button type="submit" form="data-table-form" variant="primary" disabled={busy||readonly}>{busy?'正在保存…':mode==='create'?'创建数据表':'保存修改'}</Button>}</>}>
      <form id="data-table-form" className="grid gap-5" onSubmit={submit}>
        ...
        <FormField label="数据表名称" htmlFor="data-table-name" error={form.formState.errors.name?.message} hint="1–120 个字符"><Input autoFocus readOnly={frozen||readonly} {...form.register('name')}/></FormField>
        <FormField label="数据表描述" htmlFor="data-table-description" error={form.formState.errors.description?.message} hint="最多 1000 个字符"><Textarea rows={5} readOnly={frozen||readonly} {...form.register('description')}/></FormField>
      </form>
    </Modal>
