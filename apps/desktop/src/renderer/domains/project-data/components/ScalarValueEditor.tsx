...
export function ScalarValueEditor({ id, label, type, draft, onChange, disabled = false, readOnly = false, allowMissing = false }: ScalarValueEditorProps) {
  ...
  return <fieldset className="grid min-w-0 gap-3">
    ...
    <FormField label="值状态" htmlFor={`${id}-presence`}>
    ...
    {draft.presence === 'value' && type === 'string' ? <FormField label={label} htmlFor={id}>
      <Textarea value={draft.text} disabled={disabled} readOnly={readOnly} onChange={event => update({ text: event.target.value })} />
    </FormField> : null}
    {draft.presence === 'value' && type === 'number' ? <FormField label={label} htmlFor={id}>
    ...
    {draft.presence === 'value' && type === 'boolean' ? <FormField label={label} htmlFor={id}>
    ...
    {draft.presence === 'value' && type === 'date' ? <>
      ...
      <FormField label={label} htmlFor={id}><Input value={draft.text} disabled={disabled} readOnly={readOnly} placeholder={draft.precision === 'date' ? 'YYYY-MM-DD' : 'YYYY-MM-DDTHH:mm:ss'} onChange={event => update({ text: event.target.value })} /></FormField>
      {draft.precision === 'datetime' ? <FormField label="时区偏移" htmlFor={`${id}-offset`} hint="留空、Z 或 ±HH:MM"><Input aria-label={`${label}时区偏移`} value={draft.offset} disabled={disabled} readOnly={readOnly} placeholder="+08:00" onChange={event => update({ offset: event.target.value })} /></FormField> : null}
    </> : null}
  </fieldset>
}
