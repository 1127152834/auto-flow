...
export function RecordFieldsView({ fields, record, identityFieldId, openExternalLink = window.autoflow?.openExternalLink }: {
  ...
  return <section aria-label="基本信息" className="min-w-0 rounded-card border border-line bg-surface p-6">
    ...
    <dl className="m-0 grid min-w-0 gap-0">
      <div className="grid min-w-0 gap-2 border-b border-line py-5 sm:grid-cols-[180px_minmax(0,1fr)]"><dt className="text-base text-muted">{fields.find(field => field.ref.fieldId === identityFieldId)?.name ?? '记录身份'}</dt><dd className="m-0 min-w-0 break-all text-base">{{ text: '文本', integer: '整数', uuid: 'UUID' }[key.type]} · {key.value}<span className="ml-2 text-xs text-muted">只读</span></dd></div>
