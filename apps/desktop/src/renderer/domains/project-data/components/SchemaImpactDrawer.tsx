...
export function SchemaImpactDrawer({ open, tableName, sourceLabel, changes, impact, busy = false, recoveryPending = false, error, onOpenChange, onConfirm }: {
  ...
  return <Modal open={open} onOpenChange={next => { if (!busy) onOpenChange(next) }} placement="drawer" variant="form" className="w-[min(94vw,32rem)]" title="保存字段前核对影响" description={`${tableName} · ${changes.length} 项待保存修改`}
    ...
    <div className="grid gap-7 text-base">
      <section><h3 className="mb-3 text-lg font-semibold">本次修改</h3><div className="overflow-hidden rounded-control border border-line"><table className="w-full table-fixed text-left"><tbody>{changes.map(change => <tr key={change.id} className="border-b border-line last:border-0"><th className="break-words p-3 font-medium">{change.name}<span className="ml-2 text-sm font-normal text-muted">{change.key}</span></th><td className="whitespace-pre-wrap border-l border-line p-3 align-top [overflow-wrap:anywhere]">{change.summary}</td></tr>)}</tbody></table></div></section>
