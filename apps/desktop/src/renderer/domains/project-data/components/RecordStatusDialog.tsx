...
export function RecordStatusDialog(props: RecordStatusDialogProps) {
  ...
  if(presentation==='inline')return open?<div aria-label="业务状态编辑" className="grid gap-3">{content}<div className="grid gap-2">{save}<Button type="button" disabled={busy||recoveryPending||readonly||unavailable} onClick={()=>{selection.current=null;setDraft({selected:null,baseline:baseline.current});live.current.onDirtyChange(null!==baseline.current);void run(false)}}>清空状态</Button><Button type="button" variant="ghost" disabled={busy||recoveryPending} className="justify-self-end" onClick={reset}>取消</Button></div></div>:null
