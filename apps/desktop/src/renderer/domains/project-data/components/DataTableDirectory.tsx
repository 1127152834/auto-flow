...
export function DataTableDirectory({ toolbar, items, loading = false, error, readonly = false, hasFilters = false, onRetry, onCreate, onImportExcel, onOpen, onEdit }: Props) {
  return <section aria-label="数据表目录" className="grid gap-4">
    ...
    {loading && items.length === 0 ? <div role="status" className="grid gap-3" aria-label="正在加载数据表"><Skeleton className="h-28" /><Skeleton className="h-28" /><span className="sr-only">正在加载数据表</span></div> : items.length === 0 && !error ? <div className="rounded-card border border-dashed border-line bg-surface px-6 py-14 text-center"><p className="m-0 font-medium text-ink">{hasFilters ? '没有匹配的数据表' : '还没有数据表'}</p><p className="mb-0 mt-2 text-sm text-muted">{hasFilters ? '调整筛选条件后重试。' : '创建第一张本地数据表以开始录入记录。'}</p></div> : <div className="grid gap-3 md:grid-cols-2">{items.map(table => { const SourceIcon = sourceIcon[table.sourceKind]; return <article key={table.tableId} className="min-w-0 rounded-card border border-line bg-surface p-4 shadow-card"><div className="flex min-w-0 items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-control bg-clay-soft text-clay" aria-hidden="true"><SourceIcon size={22} /></span><div className="min-w-0 flex-1"><div className="flex min-w-0 flex-wrap items-start justify-between gap-2"><h2 className="m-0 min-w-0 break-all text-base font-semibold text-ink">{table.name}</h2><Badge className="shrink-0">{sourceLabels[table.sourceKind]}</Badge></div><p className="mt-1 line-clamp-2 break-all text-sm text-muted">{table.description || '暂无描述'}</p></div></div><div className="mt-4 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted"><span>{table.recordCount} 条记录</span><time dateTime={table.updatedAt}>更新于 {updated(table.updatedAt)}</time></div><div className="mt-4 flex flex-wrap justify-end gap-2"><Button type="button" variant="ghost" aria-label={`打开${table.name}`} onClick={() => onOpen(table.tableId)}>打开</Button>{readonly ? null : <Button type="button" variant="ghost" aria-label={`编辑${table.name}`} onClick={() => onEdit(table.tableId)}><PencilSimple />编辑</Button>}</div></article> })}</div>}
  </section>
}
