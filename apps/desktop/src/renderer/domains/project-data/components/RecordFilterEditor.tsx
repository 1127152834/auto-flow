...
function FilterNode({ node, fields, statuses, disabled, depth, path, error, onChange, onRemove }: { node: FilterDraft; fields: Field[]; statuses: Status[]; disabled: boolean; depth: number; path: string; error: RecordQueryError | null; onChange(node: FilterDraft): void; onRemove?(): void }) {
  ...
  return <fieldset className="grid min-w-0 gap-2 rounded-control border border-line p-3"><legend className="px-1 text-sm font-medium">字段条件</legend><div className="grid min-w-0 grid-cols-2 gap-2"><Select aria-label={`${path}字段`} value={node.fieldId || null} disabled={disabled} options={fields.map(value=>({value:value.ref.fieldId,label:value.name}))} onValueChange={value=>{const next=fields.find(item=>item.ref.fieldId===value);onChange(compare(next))}}/><Select aria-label={`${path}运算符`} value={node.operator} clearable={false} disabled={disabled} options={operatorsFor(type).map(value=>({value,label:operatorLabels[value]}))} onValueChange={value=>value&&onChange({...node,operator:value,value:value==='isNull'||value==='isNotNull'?scalarDraft(undefined):node.value})}/></div>{isNull?null:<ScalarValueEditor compact id={`${path}-value`} label="比较值" type={type} draft={node.value} onChange={value=>onChange({...node,value})} disabled={disabled} error={error?.path===path?error.message:undefined} errorTarget={error?.path===path?error.control:undefined}/>} <div className="flex flex-wrap gap-2">{wrap}{remove}</div>{error?.path===path&&!error.control?<p role="alert" className="text-xs text-danger">{error.message}</p>:null}</fieldset>
}
