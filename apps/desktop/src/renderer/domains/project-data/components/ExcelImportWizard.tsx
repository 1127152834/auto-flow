...
export function ExcelImportWizard({ open, mode, sessionKey, scopeKey, contextKey, api, files, table, existingFields = [], readonly, disabled, onClose, onCompleted, onDirtyChange, onBusyChange }: ExcelImportWizardProps) {
  ...
  useLayoutEffect(() => { epoch.current += 1 }, [scopeKey, sessionKey, contextKey, open])
  ...
  const toReview = async (value: MappingResult) => {
    ...
    if (mode === 'replace' && table) { const ticket = epoch.current; try { const value = await api.replaceImpact(table.tableId); if (ticket === epoch.current) setImpact(value) } catch (cause) { if (ticket === epoch.current) setImpactError(cause instanceof Error ? cause.message : '无法评估替换影响') } }
  }
  const submit = () => {
