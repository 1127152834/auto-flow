...
it('previews edit, displays blockers, invalidates impact after draft changes', async () => {
  ...
  await user.click(screen.getByRole('button',{name:'预检影响'})); expect(await screen.findByText('已检查')).toBeVisible()
  ...
  await user.click(screen.getByRole('button',{name:'预检影响'})); await screen.findByText('已检查')
...
it('blocks confirmation and preserves dirty values through same-session refresh', async () => {
  ...
  await user.click(screen.getByRole('button',{name:'预检影响'})); expect(await screen.findByText('不能修改')).toBeVisible()
