...
export function RecordEditorForm({ id, mode, presentation='dialog', sessionKey, fields, initialRecord, initialSubmittedValues, identityFieldId, submissionEpoch=0, saving=false, recoveryPending=false, readonly=false, error, errorActions, footerClassName, externalActions=false, onCancel, onSubmitAttempt, onSubmit, onRecover, onDirtyChange, onDraftSummaryChange, onValidationErrorChange, onSavingChange }: RecordEditorFormProps) {
  ...
  const identityType=context.initialRecord?.ref.recordKey.type==='text'?'文本':context.initialRecord?.ref.recordKey.type==='integer'?'整数':'UUID'
  return <form id={id} aria-label={mode==='create'?'新建记录表单':'编辑记录表单'} className={`grid min-w-0 ${presentation==='page'?'gap-0':'gap-5'}`} noValidate onSubmit={submit}>
    ...
    {presentation==='page'?(context.initialRecord?<div className="grid min-w-0 items-start gap-3 border-b border-line py-3 lg:grid-cols-[250px_minmax(0,1fr)]"><span className="pt-3 text-base font-medium">{context.fields.find(field=>field.ref.fieldId===context.identityFieldId)?.name??'记录身份'} <span className="text-danger">*</span></span><div className="grid gap-1"><div className="relative"><Input aria-label="记录身份" className="h-11 pr-11 text-base" value={`${identityType} · ${context.initialRecord.ref.recordKey.value}`} readOnly/><Lock aria-hidden="true" className="pointer-events-none absolute right-3 top-3 text-muted" size={20}/></div><span className="text-sm text-muted">只读</span></div></div>:!context.identityFieldId?<div className="grid min-w-0 items-start gap-3 border-b border-line py-3 lg:grid-cols-[250px_minmax(0,1fr)]"><span className="text-base font-medium">记录身份</span><p className="m-0 text-base text-muted">保存时自动生成</p></div>:null):context.initialRecord?<p className="m-0 break-all rounded-control border border-line bg-surface px-3 py-2 text-sm text-muted">记录身份：{context.initialRecord.ref.recordKey.type} · {context.initialRecord.ref.recordKey.value}</p>:null}
