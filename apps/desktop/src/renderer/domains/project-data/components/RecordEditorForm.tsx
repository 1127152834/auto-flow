...
export function RecordEditorForm({ id, mode, presentation='dialog', sessionKey, fields, initialRecord, initialSubmittedValues, identityFieldId, submissionEpoch=0, saving=false, recoveryPending=false, readonly=false, error, errorActions, footerClassName, externalActions=false, onCancel, onSubmitAttempt, onSubmit, onRecover, onDirtyChange, onSavingChange }: RecordEditorFormProps) {
  ...
  return <form id={id} aria-label={mode==='create'?'新建记录表单':'编辑记录表单'} className="grid min-w-0 gap-5" noValidate onSubmit={submit}>
    {context.initialRecord?<p className="m-0 break-all rounded-control border border-line bg-surface px-3 py-2 text-sm text-muted">记录身份：{presentation==='page'?`${identityType} · ${context.initialRecord.ref.recordKey.value}（只读）`:`${context.initialRecord.ref.recordKey.type} · ${context.initialRecord.ref.recordKey.value}`}</p>:null}
