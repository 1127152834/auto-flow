...
export function RecordEditPage({ mode, editorForm, footer, conflictView, identity, statusName = '未设置', loading, error, disabled, onBack, onRetry, onStatus }: {
  mode: 'create' | 'edit'; editorForm?: ReactNode; footer?: ReactNode; conflictView?: ReactNode; identity?: string; statusName?: string; loading?: boolean; error?: string | null; disabled?: boolean; onBack(): void; onRetry?(): void; onStatus?(): void
}) {
  return <section aria-label={mode === 'create' ? '新增记录' : '编辑记录'} className="grid min-w-0 gap-5" data-record-page={mode}>
    ...
    {loading && !editorForm ? <div role="status" aria-label="正在加载记录表单"><Skeleton className="h-64" /></div> : editorForm ? <div className={`grid min-w-0 items-start gap-6 ${conflictView ? 'lg:grid-cols-2' : 'lg:grid-cols-[minmax(0,1fr)_18rem]'}`}>
      <section aria-label="基本信息" className="min-w-0 rounded-card border border-line bg-surface p-6"><h2 className="mb-5 text-base font-semibold">{conflictView ? '我的修改' : '基本信息'}</h2>{identity ? <div className="mb-5 border-b border-line pb-4 text-sm"><p className="mb-1 text-muted">记录身份 · 只读</p><p className="break-all">{identity}</p></div> : null}{editorForm}</section>
      {conflictView ?? <aside className="grid min-w-0 gap-5"><section aria-label="业务状态" className="rounded-card border border-line bg-surface p-5"><h2 className="mb-4 text-base font-semibold">业务状态</h2><p className="text-sm">{mode === 'create' ? '未设置' : statusName}</p><p className="mt-3 text-sm leading-6 text-muted">{mode === 'create' ? '新记录的业务状态默认为未设置。创建后可在记录详情中设置。' : '保存字段内容不会修改业务状态。请在记录详情中单独设置。'}</p>{mode === 'edit' && onStatus ? <Button className="mt-3" size="sm" variant="ghost" onClick={onStatus} disabled={disabled}>前往设置业务状态</Button> : null}</section>
