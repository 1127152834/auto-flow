...
it('keeps Gallery metadata in the status card and deletion behind more', async () => {
 const remove=vi.fn()
 render(<RecordDetailPage title="温室记录" updatedAt="2026-09-13T00:00:00Z" fieldsView={<p>业务值</p>} statusForm={<p>状态表单</p>} onBack={vi.fn()} onEdit={vi.fn()} onDelete={remove}/>)
 ...
 expect(within(status).queryByText(/记录编号|001/)).not.toBeInTheDocument()
