...
it('create and edit pages compose one shared form without a modal or automatic status editing', () => {
  ...
  expect(screen.queryByRole('dialog')).not.toBeInTheDocument()
  view.rerender(<RecordEditPage {...props} mode="edit" statusName="可使用" identity="文本 · 001" />)
  expect(screen.getByText('文本 · 001')).toBeVisible()
  expect(screen.getByText('可使用')).toBeVisible()
  expect(screen.getByText(/未修改的异常单元格/)).toBeVisible()
})
