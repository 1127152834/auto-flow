...
export function ProjectsWorkspace({ route, workspaceKey, instanceId, client, disabled, onNavigate, registerLeaveGuard }: ProjectsWorkspaceProps) {
  ...
  return <>
    ...
    {route.projectId && project ? <ProjectOverviewPage detailContext={route.automationId || route.automationCreate ? { name: route.automationCreate ? '新建自动化' : automationContext.data?.name ?? '自动化配置', label: '返回自动化目录', onBack: () => onNavigate({ projectId: project.projectId, tab: 'automations' }) } : route.tab === 'runs' && (route.batchId || route.taskId || route.manualItemId) ? { name: route.manualItemId ? '人工事项' : route.taskId ? '任务详情' : '批次详情', label: route.manualItemId ? '返回等待人工' : '返回运行记录', onBack: () => onNavigate({ projectId: project.projectId, tab: 'runs', ...(route.manualItemId ? { runView: 'manual' } : { runView: 'batches' }) }) } : route.tab === 'environments' && route.environmentId ? { name: '环境详情', label: '返回环境', onBack: () => onNavigate({ projectId: project.projectId, tab: 'environments' }) } : undefined} tableBackLabel={route.record ? "返回记录列表" : "返回数据表"} onTableBack={() => onNavigate({ projectId: project.projectId, tab: "data", ...(route.record ? { tableId: route.tableId, dataTab: "records" } : {}) })} tableName={tableContext.data?.name} tableDetail={Boolean(route.tableId)} project={project} tab={route.tab} disabled={disabled} onBack={() => onNavigate({ tab: 'overview' })} onEdit={() => { if (!disabled && project.lifecycleState === 'active') setEditor({ project, draftSession: `edit:${project.projectId}:${Date.now()}` }) }} onTabChange={tab => onNavigate({ projectId: project.projectId, tab })}>
