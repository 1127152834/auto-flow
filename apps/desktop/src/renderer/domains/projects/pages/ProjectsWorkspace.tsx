...
export function ProjectsWorkspace({ route, workspaceKey, instanceId, client, disabled, onNavigate, registerLeaveGuard }: ProjectsWorkspaceProps) {
  ...
  const openStudio = (workflowId?: string, automationId?: string) => {
    ...
    void window.autoflow.openAutomationStudio({ workspaceKey, instanceId, projectId: project.projectId, ...(automationId ? { automationId } : {}), ...(workflowId ? { workflowId } : {}) }).catch(error => notify({ title: safeProjectError(error), tone: 'error' }))
  }
  return <>
    <Toaster />
