...
export function ProjectHeader({ project, density = 'default', disabled, onEdit }: { project: ProjectView; density?: HeaderDensity; disabled: boolean; onBack(): void; onEdit(): void }) {
  ...
  return <header data-project-header-density={density} className="flex min-w-0 flex-wrap items-start justify-between gap-5">
    <div className="flex min-w-0 flex-1 items-start gap-5">
      ...
      <div className="min-w-0"><div className="flex min-w-0 flex-wrap items-center gap-2">{title}{project.lifecycleState !== 'active' ? <span className="shrink-0 rounded-control bg-surface-subtle px-2 py-1 text-xs text-muted">{project.lifecycleState === 'archived' ? '已归档' : '处理中'}</span> : null}</div><p className="mb-0 mt-2 break-all text-base text-muted">{project.description || '暂无项目描述'}</p>{density === 'default' && project.updatedAt ? <p className="mb-0 mt-1 text-sm text-muted">最近修改：{new Intl.DateTimeFormat('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }).format(new Date(project.updatedAt))}</p> : null}</div>
    </div>
