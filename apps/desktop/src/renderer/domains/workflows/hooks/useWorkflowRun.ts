...
export function useWorkflowRun(api: WorkflowRunApi, connected: boolean) {
  ...
  const start = useCallback(async (content: WorkflowContent, profileId: string, debug?: RunStart['debug']) => {
    if (!online.current || busyRef.current || activeRef.current || pendingStart.current || !verified.current) return
    await submit({ ...structuredClone(content), profileId, runId: crypto.randomUUID(), mode: 'run', ...(debug ? { mode: 'debug' as const, debug } : {}) })
  }, [submit])
