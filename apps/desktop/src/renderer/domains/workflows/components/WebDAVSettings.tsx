...
export function WebDAVSettings() {
  ...
  useEffect(() => {
    setLoading(true)
    studioFetch(`${getBackendBaseUrl()}/api/local-workflows/webdav-config`)
      .then((r) => r.json())
  ...
  const handleSave = async () => {
    ...
    try {
      const r = await studioFetch(`${getBackendBaseUrl()}/api/local-workflows/webdav-config`, {
  ...
  const handleTest = async () => {
    ...
    try {
      const r = await studioFetch(`${getBackendBaseUrl()}/api/local-workflows/webdav-test`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(cfg),
      })
      const d = await r.json()
