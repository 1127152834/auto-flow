...
import { DatePicker, TimePicker } from '../controls/date-time-picker'
...
export function TaskCreateDialog({ open, onClose }: TaskCreateDialogProps) {
  ...
  const handleWorkflowChange = (filename: string) => {
    ...
    localWorkflowApi.getSelfHeal(filename).then(r => {
      ...
      if (!r.success || !r.data?.success) setError(r.error || '读取配置失败')
      else setSelfHeal(r.data.enabled === true)
    }).finally(() => { if (request === workflowRequest.current) setSelfHealBusy(false) })
  }
  const handleToggleSelfHeal = async (value: boolean) => {
    const request = ++workflowRequest.current
    setSelfHealBusy(true)
    try {
      const response = await localWorkflowApi.setSelfHeal(workflowId, value)
      if (request !== workflowRequest.current) return
      if (!response.success || !response.data?.success) setError(response.error || '保存配置失败')
      else setSelfHeal(value)
    } finally { if (request === workflowRequest.current) setSelfHealBusy(false) }
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 animate-fade-in" style={{ zIndex: 2147483646 }} onClick={handleClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col animate-scale-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          ...
          >
            <X aria-label="关闭" className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
          ...
          {workflowId && (
            <div className="flex items-start justify-between gap-3 p-3 rounded-lg border border-[hsl(var(--brand-500)/0.25)] bg-[hsl(var(--brand-50)/0.5)]">
              ...
              <Switch aria-label="自愈固化" disabled={selfHealBusy} checked={selfHeal} onCheckedChange={handleToggleSelfHeal} />
            </div>
          )}
