...
import { useScheduledTaskStore, type ScheduledTask } from '../../hooks/stores/scheduledTaskStore'
...
export function ScheduledTasksPage({ onClose }: ScheduledTasksPageProps = {}) {
  ...
  return (
    ...
    <div className="h-full flex-1 min-h-0 flex flex-col bg-[hsl(var(--background))]">
      ...
      {error && <div role="alert" className="mx-5 mt-3 rounded border border-red-300 p-3 text-sm text-red-700">{error}</div>}
                ...
                <div className="flex gap-1.5 pt-3 border-t border-[hsl(var(--border))]">
                  {task.is_running ? (
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => { void handleStop(task).catch(() => { /* Store exposes the failure above. */ }) }}
                      className="flex-1"
                    >
                    ...
                    <Button
                      variant="success"
                      size="sm"
                      onClick={() => { void handleExecute(task).catch(() => { /* Store exposes the failure above. */ }) }}
                  ...
                  <Button
                    ...
                    size="sm"
                    aria-label="删除任务"
                    onClick={() => { void handleDelete(task).catch(() => { /* Store exposes the failure above. */ }) }}
