...
export function Toolbar() {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'F5') {
        e.preventDefault()
        window.dispatchEvent(new CustomEvent(e.shiftKey ? 'hotkey:stop' : 'hotkey:run'))
        return
      }
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 's') {
        ...
        queueMicrotask(() => void handleSave())
        return
      }
      // 如果焦点在输入框中，不处理其他快捷键
  ...
  const handleClipboardImageConfirm = async (name: string) => {
    ...
    try {
      const response = await studioFetch(`${API_BASE}/api/system/save-clipboard-image`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const doScreenshot = useCallback(async () => {
    ...
    try {
      const response = await studioFetch(`${API_BASE}/api/system/screenshot-tool`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleScreenshotNameConfirm = async (newName: string) => {
    ...
    try {
      // 调用API重命名截图
      const response = await studioFetch(`${API_BASE}/api/image-assets/${screenshotAsset.id}/rename?newName=${encodeURIComponent(newName + '.png')}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' }
      })
  ...
  const handleExportPlaywright = useCallback(async () => {
    ...
    try {
      ...
      // 调用导出 API
      const API_BASE = getBackendBaseUrl()
      const response = await studioFetch(`${API_BASE}/api/workflows/${currentWorkflowId}/export-playwright`)
      const data = await response.json()
          ...
          <DropdownMenuContent align="end" className="w-44">
            ...
            <DropdownMenuItem
              ...
              onMouseEnter={() => {
                ...
                import('./documentation/index').catch(() => {})
              }}
            >
