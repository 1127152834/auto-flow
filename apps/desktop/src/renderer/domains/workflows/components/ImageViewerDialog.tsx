// Source: WebRPA@5ccb900e, components/workflow/ImageViewerDialog.tsx; see SOURCE.md for license and adaptation boundaries.
...
export function ImageViewerDialog({ imageUrl, autoClose, displayTime, onClose }: ImageViewerDialogProps) {
  ...
  const convertImage = useCallback(async (url: string): Promise<string | null> => {
    ...
    try {
      const response = await studioFetch(`${getBackendUrl()}/api/system/convert-image`, {
        method: 'POST',
