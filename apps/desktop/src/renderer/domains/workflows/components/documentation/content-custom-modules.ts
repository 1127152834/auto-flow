// Source: WebRPA@5ccb900e, components/workflow/documentation/content-custom-modules.ts; see SOURCE.md for license and adaptation boundaries.
export const customModulesGuideContent = `# 自定义模块

> 当前 Studio 教学范围为保留节点及前端配置交互。Mock 用于交互演示，不执行真实网页、模型或系统操作；文中的执行行为需正式后端支持。

自定义模块允许你扩展AutoFlow的功能，创建可复用的模块。
