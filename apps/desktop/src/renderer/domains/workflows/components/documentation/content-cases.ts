// Source: WebRPA@5ccb900e, components/workflow/documentation/content-cases.ts; see SOURCE.md for license and adaptation boundaries.
...
3. 获取元素信息 -> 获取所有商品选择器
4. 遍历列表 -> 遍历每个商品
   ├─ 获取元素信息 -> 商品名称 -> 数据表格列名：名称
   ├─ 获取元素信息 -> 商品价格 -> 数据表格列名：价格
   └─ 获取元素信息 -> 商品链接 -> 数据表格列名：链接
\`\`\`
...
// 保存到数据表格
设置变量（数据表格列名：情感）：{分析数据[sentiment]}
设置变量（数据表格列名：分数）：{分析数据[score]}
设置变量（数据表格列名：摘要）：{分析数据[summary]}
\`\`\`
