...
export function ManualHandoffPanel({ run, api, connected, onRefresh, onStop }: { run: RunRead; api: WorkflowRunApi; connected: boolean; onRefresh(): void; onStop(): void }) {
  ...
  return <section aria-label="人工处理安卓" className="border-b border-line bg-surface-subtle px-5 py-4">
    <div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold">{run.state === 'resuming' ? '正在收回控制权' : '等待人工处理'} · {run.targetName}</h2><p className="mt-1 text-sm">{handoff.prompt}</p></div><span className="text-sm text-muted">剩余 {remaining} 秒</span></div>
    <p role="status" className="mt-2 text-sm text-muted">{handoff.state === 'closed' ? '窗口已关闭，工作流仍在等待。' : handoff.state === 'open' ? '请在 Mac 原生窗口操作，完成后回到这里继续。' : '设备仍由当前工作流占用，其他任务不会向它输入。'}</p>
    ...
    <div className="mt-3 flex gap-2">
      ...
      <Button variant="primary" disabled={unavailable || Boolean(retry.current)} onClick={() => void control('continue')}>完成并继续</Button>
      {retry.current && <Button disabled={pending || !connected} onClick={() => void control(retry.current!.action)}>按原编号重试</Button>}
      <Button disabled={!connected} onClick={onStop}>停止运行</Button>
    </div>
  </section>
}
