// Source: WebRPA@5ccb900e, components/workflow/config-panels/QQModuleConfigs.tsx; see SOURCE.md for license and adaptation boundaries.
...
import { QQContactSelect } from '../controls/qq-contact-select'
...
function QRCodeDialog({ 
  ...
  const loadQRCode = useCallback(async () => {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        const response = await studioFetch(testUrl, { method: 'GET' })
  ...
  const refreshQRCode = useCallback(async () => {
    ...
    try {
      const API_BASE = getBackendBaseUrl()
      const response = await studioFetch(`${API_BASE}/api/system/napcat/refresh-qrcode`, {
        method: 'POST'
      })
  ...
  useEffect(() => {
    ...
    const checkLogin = async () => {
      try {
        const API_BASE = getBackendBaseUrl()
        const response = await studioFetch(`${API_BASE}/api/system/napcat/status`)
...
function QQServiceStatus({ apiUrl }: { apiUrl: string }) {
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      const response = await studioFetch(`${url}/get_login_info`, {
        method: 'POST',
        headers,
        body: JSON.stringify({})
      })
...
function NapCatServiceManager() {
  ...
  const startService = async () => {
    ...
    try {
      const API_BASE = getBackendBaseUrl()
      const response = await studioFetch(`${API_BASE}/api/system/napcat/start`, {
        method: 'POST',
  ...
  const stopService = async () => {
    ...
    try {
      const API_BASE = getBackendBaseUrl()
      const response = await studioFetch(`${API_BASE}/api/system/napcat/stop`, {
        method: 'POST'
      })
      const result = await response.json()
      if (result.success) {
