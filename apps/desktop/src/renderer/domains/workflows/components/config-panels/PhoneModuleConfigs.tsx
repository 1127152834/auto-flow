// Source: WebRPA@5ccb900e, components/workflow/config-panels/PhoneModuleConfigs.tsx; see SOURCE.md for license and adaptation boundaries.
...
import { PhoneCoordinateInput } from '../controls/phone-coordinate-input'
...
export function PhoneInputTextConfig({ data, onChange }: { data: NodeData; onChange: (key: string, value: unknown) => void }) {
  ...
  const checkAdbKeyboardStatus = async () => {
    try {
      ...
      const response = await studioFetch(`${getBackendUrl()}/api/phone/check-adbkeyboard`)
  ...
  const installAdbKeyboard = async () => {
    try {
      ...
      const response = await studioFetch(`${getBackendUrl()}/api/phone/install-adbkeyboard`, {
        method: 'POST',
...
export function PhoneSetClipboardConfig({ data, onChange }: { data: NodeData; onChange: (key: string, value: unknown) => void }) {
  ...
  const checkClipperStatus = async () => {
    try {
      ...
      const response = await studioFetch(`${getBackendUrl()}/api/phone/check-clipper`)
      const result = await response.json()
  ...
  const installClipper = async () => {
    try {
      ...
      const response = await studioFetch(`${getBackendUrl()}/api/phone/install-clipper`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      })
      const result = await response.json()
