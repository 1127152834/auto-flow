// Source: WebRPA@5ccb900e, components/workflow/config-panels/TriggerModuleConfigs.tsx; see SOURCE.md for license and adaptation boundaries.
...
import { InputDialog, ConfirmDialog, AlertDialog } from '../controls/custom-dialogs'
...
export function GestureTriggerConfig({
  ...
  const loadCustomGestures = () => {
    studioFetch(`${getBackendUrl()}/api/triggers/gesture/custom`)
      .then(res => res.json())
      .then(result => {
        if (result.success) {
  ...
  // 检查手势识别状态
  const checkStatus = () => {
    studioFetch(`${getBackendUrl()}/api/triggers/gesture/status`)
      .then(res => res.json())
      .then(result => {
        if (result.success) {
  ...
  const handleStartRecording = async () => {
    ...
    try {
      // 直接录制手势（不需要先启动识别服务）
      const response = await studioFetch(`${getBackendUrl()}/api/triggers/gesture/record`, {
        method: 'POST',
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await studioFetch(`${getBackendUrl()}/api/triggers/gesture/custom/${deleteGestureName}`, {
        method: 'DELETE'
      })
