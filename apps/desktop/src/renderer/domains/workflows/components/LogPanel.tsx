// Source: WebRPA@5ccb900e, components/workflow/LogPanel.tsx; see SOURCE.md for license and adaptation boundaries.
import { useWorkflowStore, type DataRow } from '../editor-store'
            ...
            <ScrollArea className="h-full p-2">
              {variables.length === 0 ? (
                ...
                <div className="flex flex-col h-full">
                  <TableScroll label="全局变量" className="flex-1">
                    <Table className="text-xs">
                      <TableHeader>
                        <TableRow className="bg-muted/50">
                          <TableHead className="border px-2 py-1.5 text-left font-medium w-32">变量名</TableHead>
                          <TableHead className="border px-2 py-1.5 text-left font-medium">值</TableHead>
                          <TableHead className="border px-2 py-1.5 text-left font-medium w-20">类型</TableHead>
                          <TableHead className="border px-2 py-1.5 w-12">操作</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {variables.map((v) => (
                          <TableRow key={v.name} className="hover:bg-muted/30">
                            <TableCell className="border px-2 py-1 cursor-pointer hover:bg-muted/50" onClick={() => startEditVarName(v.name)}>
                            ...
                            <TableCell className="border px-2 py-1 cursor-pointer hover:bg-muted/50" onClick={() => startEditVar(v.name, v.value, v.type)}>
                              {editingVar === v.name ? (
                            ...
                            <TableCell className="border px-2 py-1 text-muted-foreground">{v.type}</TableCell>
