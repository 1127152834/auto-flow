...
function VariableRow({ variable, names, onChange, onRename, onDelete, onEditStart, onEditEnd, disabled }: {
  ...
  return <div className="space-y-3 rounded-card border border-line bg-surface-subtle p-3" aria-label={`变量 ${variable.name}`}>
    ...
    <FormField htmlFor={`variable-${variable.name}-name`} label="变量名" error={nameError ? `${nameError}尚未重命名，原名称仍为 ${variable.name}。` : undefined}><Input value={name} data-pending-variable-rename={Boolean(nameError)} disabled={disabled} onFocus={onEditStart} onChange={(event) => setName(event.target.value)} onBlur={rename} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur() }} /></FormField>
