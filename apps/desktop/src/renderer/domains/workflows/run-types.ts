...
export const isRunActive = (run: Pick<RunSummary, 'state'>) => ['starting', 'running', 'pausing', 'paused', 'failed_paused', 'finishing', 'stopping', 'waiting_manual', 'resuming'].includes(run.state)
export const runStateLabel: Record<RunSummary['state'], string> = {
  pausing: '正在等待暂停', paused: '已暂停', failed_paused: '失败现场待检查', waiting_manual: '等待人工', resuming: '正在收回控制权', starting: '正在启动', running: '运行中', finishing: '正在清理', stopping: '正在停止',
