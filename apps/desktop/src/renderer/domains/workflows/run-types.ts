...
export const isRunActive = (run: Pick<RunSummary, 'state'>) => ['starting', 'running', 'finishing', 'stopping'].includes(run.state)
export const runStateLabel: Record<RunSummary['state'], string> = {
  starting: '正在启动', running: '运行中', finishing: '正在清理', stopping: '正在停止',
