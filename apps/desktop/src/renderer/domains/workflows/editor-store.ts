...
const LOG_PREFS_KEY = 'autoflow.studio.mock.editor.logPrefs'
...
export interface ErrorPolicy {
  ...
  maxRetries?: number | string  // 最大重试次数；字符串保留未完成输入或变量引用
  interval?: number | string  // 每次重试间隔（秒）；字符串保留未完成输入或变量引用
...
export const useWorkflowStore = create<WorkflowState>((set, get) => ({
  ...
  setVerboseLog: (enabled) => {
    ...
    // 同步到后端
    import('./events').then(({ socketService }) => {
  ...
  undo: () => {
    // Source mutations snapshot the BEFORE state. Capture the live state for redo before stepping back.
    get().pushHistory()
    const state = get()
  ...
  restoreSnapshot: (snapshot, options) => {
    ...
    set({
      ...
      selectedNodeId: null,
      hasUnsavedChanges: true,
      ...(options?.resetHistory ? { history: [structuredClone(restored)], historyIndex: 0 } : {}),
    })
  },
