...
export function runRecord(overrides: Partial<RunRead> = {}): RunRead {
  return { executionCount: 0, artifactCount: 0,
    target: { kind: 'browser', profileId: 'profile-1' }, targetName: '真实配置',
...
export function runApi(record: RunRead | null = null): WorkflowRunApi {
  return {
    ...
    start: vi.fn(async request => runRecord({ ...request, target: request.target ?? { kind: 'browser', profileId: request.profileId ?? 'profile-1' }, workflowId: request.document.id, name: request.document.name })),
    handoff: vi.fn(async () => record ?? runRecord()),
