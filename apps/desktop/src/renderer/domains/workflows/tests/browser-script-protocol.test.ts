...
describe.each(['memory','http'] as const)('browser script tests over %s',mode=>{
 ...
 it('cancels idempotently and releases the mutually exclusive browser slot',async()=>{
  ...
  for(const path of ['/workflows/other/execute','/recorder/start','/element-picker/start'])expect((await request(path,path==='/element-picker/start'?{sessionId:'blocked-picker'}:{})).status).toBe(409)
