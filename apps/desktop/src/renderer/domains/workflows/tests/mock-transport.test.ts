...
describe('source-compatible mock HTTP boundary',()=>{
  ...
  it('feeds the non-destructive recorder protocol and requires an available browser',async()=>{
    expect((await request('/recorder/start',{sessionId:'recording',commandId:'start-before-browser'})).status).toBe(409)
    await request('/browser/open',{url:'about:blank'});await request('/recorder/start',{sessionId:'recording',commandId:'start-recording'})
    ...
    expect((await(await request('/recorder/stop',{sessionId:'recording',commandId:'stop-recording',afterSeq:1})).json()).data.events).toHaveLength(1)
