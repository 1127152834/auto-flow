...
it('pauses and resumes recording from the visible controls',async()=>{
 await browserApi.open();render(<Editor recorder/>);await click('开始录制')
 addMockRecordingEvent({type:'input',selector:'#name',value:'暂停前'})
 await click('暂停录制');expect(screen.getByText('已暂停')).toBeTruthy()
 expect(()=>addMockRecordingEvent({type:'click',selector:'#ignored'})).toThrow('录制已暂停')
 await click('恢复录制');addMockRecordingEvent({type:'click',selector:'#captured'})
 await click('停止录制')
 expect(screen.getByText(/暂停前/)).toBeTruthy();expect(screen.getByText(/#captured/)).toBeTruthy()
})
