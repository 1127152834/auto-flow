...
const operations = [
  { name: 'config', call: () => mcpApi.config(), value: config },
  { name: 'status', call: () => mcpApi.status(), value: status },
  { name: 'save', call: () => mcpApi.save(config), value: { success: true, saved: true } },
  { name: 'reload', call: () => mcpApi.reload(), value: { connected: [], failed: [], disabled: [], total_servers: 0 } },
]
let restore = () => {}
let restoreNext = () => {}
...
it('keeps a new server draft and blocks sending it after the connection changes', async () => {
  restore = configureStudioConnection('http://mcp.fixture', async input => Response.json(String(input).endsWith('/status') ? status : config))
  ...
  const next = vi.fn(async () => Response.json({ success: true, saved: true }))
...
it('a failed reload read disables writes against the stale configuration', async () => {
  ...
  restore = configureStudioConnection('http://mcp.fixture', async input => {
    if (String(input).endsWith('/reload')) { fail = true; return Response.json({ connected: [], failed: [], disabled: [], total_servers: 0 }) }
    if (String(input).endsWith('/status')) return Response.json(status)
    return fail ? Response.json({ error: '读取失败' }, { status: 503 }) : Response.json(config)
  })
  render(<MCPConfigPanel />)
  await screen.findByText('fixture')
...
it('a late enable-state save cannot change the visible confirmed config after reconnection', async () => {
  let resolve!: (response: Response) => void
  restore = configureStudioConnection('http://mcp.fixture', async (input, init) => init?.method === 'PUT' ? new Promise(done => { resolve = done }) : Response.json(String(input).endsWith('/status') ? status : config))
  render(<MCPConfigPanel />)
  fireEvent.click(await screen.findByRole('button', { name: '禁用' }))
  await act(async () => {
    restoreNext = configureStudioConnection('http://next.fixture', async () => Response.json({}))
    resolve(Response.json({ success: true, saved: true }))
  })
...
it('explicit rereading after a connection change replaces stale metadata and enables the new config', async () => {
  ...
  render(<MCPConfigPanel />)
  await screen.findByText('fixture')
  await act(async () => { restoreNext = configureStudioConnection('http://next.fixture', async input => Response.json(String(input).endsWith('/status') ? status : { mcpServers: { next: { command: 'node' } } })) })
