...
describe.each(['memory', 'http'] as const)('shared protocol assertions: %s', mode => {
  ...
  it('replays recording events without consuming them and keeps stop retries idempotent', async () => {
    ...
    await request('/recorder/start', json({ sessionId: 'record-a', commandId: 'start-a' }))
    ...
    const stop = await (await request('/recorder/stop', json({ sessionId: 'record-a', commandId: 'stop-a', afterSeq: 1 }))).json()
    expect(stop.data.events).toMatchObject([{ sequence: 2, selector: '#tail' }])
    expect(await (await request('/recorder/stop', json({ sessionId: 'record-a', commandId: 'stop-a', afterSeq: 1 }))).json()).toEqual(stop)
    ...
    await request('/recorder/start', json({ sessionId: 'record-b', commandId: 'start-b' }))
    expect((await request('/recorder/start', json({ sessionId: 'record-a', commandId: 'start-a-again' }))).status).toBe(409)
    expect((await request('/recorder/stop', json({ sessionId: 'record-a', commandId: 'stop-old' }))).status).toBe(409)
  })
