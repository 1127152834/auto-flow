...
it('loads full JSON results through the authenticated API and presents them read-only', async () => {
  const api = runApi()
  const record = runRecord({ artifacts: [{ purpose: 'result', eventSeq: 0, id: 'artifact-1', nodeId: 'node-1', kind: 'json', name: '提取结果', mimeType: 'application/json', relativePath: 'value.json', outputPath: null, preview: 'short preview' }] })
...
it('allows collapsing logs and stops following when the user scrolls up to inspect older events', async () => {
  ...
  expect(screen.getByLabelText('日志内容')).not.toBeVisible()
...
it('renders a PNG from a fetched object URL and releases it when the result is collapsed', async () => {
  ...
  const api = runApi()
  const record = runRecord({ artifacts: [{ purpose: 'result', eventSeq: 0, id: 'png-1', nodeId: 'node-1', kind: 'image', name: '网页截图', mimeType: 'image/png', relativePath: 'shot.png', outputPath: null, preview: '' }] })
  render(<RunPanel run={record} events={[]} history={[]} nextOffset={null} sameDocument connected message={null} api={api} onSelect={vi.fn()} onLocate={vi.fn()} onMore={vi.fn()} />)
  await userEvent.click(screen.getByRole('tab', { name: /只读结果/ }))
  expect(api.artifact).not.toHaveBeenCalled()
...
it('preserves unsubmitted runtime edits when the diagnostic panel is collapsed', async () => {
  const api = runApi()
  render(<RunPanel run={runRecord({ mode: 'debug', state: 'paused', debug: { pauseId: 'p1', controlRevision: 1 } })} events={[]} history={[]} nextOffset={null} sameDocument connected message={null} api={api} onSelect={vi.fn()} onLocate={vi.fn()} onMore={vi.fn()} />)
  fireEvent.change(screen.getByLabelText('修改运行变量'), { target: { value: '{"new":' } })
  await userEvent.click(screen.getByRole('button', { name: '收起运行记录' }))
  expect(screen.getByLabelText('修改运行变量')).not.toBeVisible()
  await userEvent.click(screen.getByRole('button', { name: '展开运行记录' }))
  expect(screen.getByLabelText('修改运行变量')).toHaveValue('{"new":')
