...
it('new documents persist node ownership while later navigations reuse the instance',()=>{
 ...
 expect(document.nodes.map((node:{data:{browserEnvironment:{source:string}}})=>node.data.browserEnvironment.source)).toEqual(['newFromProfile','current'])
...
it('edits node template proxy and installed kernel without changing global selection',async()=>{
 ...
 await waitFor(()=>expect(screen.getByLabelText('浏览器模板')).toBeTruthy())
 await chooseOption(user,screen.getByLabelText('浏览器模板'),'模板一')
 ...
 expect(useWorkflowStore.getState().nodes[0].data.browserEnvironment).toEqual({source:'newFromProfile',profileId:'one',proxy:{mode:'fixed',proxyId:'proxy-one'},kernel:{edition:'public',version:'145'}})
 ...
 await chooseOption(user,screen.getByLabelText('环境来源'),'使用当前实例')
 expect(screen.queryByLabelText('浏览器模板')).toBeNull()
 expect(useWorkflowStore.getState().nodes[0].data.browserEnvironment).toEqual({source:'current'})
})
