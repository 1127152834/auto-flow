...
it.each(['selected', 'similar', 'exception'] as const)('deduplicates %s polling failures, preserves the target and resumes after recovery', async mode => {
  const poll = vi.spyOn(elementPickerApi, 'getSelected').mockResolvedValue({ success: true, data: { selected: false } })
  if (mode === 'selected') poll.mockResolvedValue({ success: false, error: '拾取服务暂时不可用' })
  if (mode === 'similar') vi.mocked(elementPickerApi.getSimilar).mockResolvedValue({ success: false, error: '拾取服务暂时不可用' })
  if (mode === 'exception') poll.mockRejectedValue(new Error('拾取服务暂时不可用'))
  render(<ConfigPanel selectedNodeId={nodeId} />); await start()
  await act(async () => vi.advanceTimersByTimeAsync(1500))
  const errors = () => store.getState().logs.filter(log => log.level === 'error' && log.message.includes('拾取服务暂时不可用'))
  expect(errors()).toHaveLength(1)
  expect(store.getState().nodes[0].data.selector).toBe('#before')
  expect(screen.getByTitle('停止选择')).toBeTruthy()
  if (mode === 'selected') expect(elementPickerApi.getSimilar).not.toHaveBeenCalled()
  poll.mockResolvedValue({ success: true, data: { selected: false } })
  vi.mocked(elementPickerApi.getSimilar).mockResolvedValue({ success: true, data: { selected: false } })
  await act(async () => vi.advanceTimersByTimeAsync(500))
  expect(store.getState().logs.at(-1)).toMatchObject({ level: 'info', message: '拾取结果读取已恢复' })
  poll.mockResolvedValueOnce({ success: false, error: '拾取服务暂时不可用' })
  await act(async () => vi.advanceTimersByTimeAsync(500))
  expect(errors()).toHaveLength(2)
  poll.mockResolvedValue(selected)
  await act(async () => vi.advanceTimersByTimeAsync(500))
  expect(store.getState().nodes[0].data.selector).toBe('#picked')
})
