...
it.each([false,true])('applies a similar selector and its index variable in one undo with existing=%s', async existing => {
  if (existing) store.getState().addVariable({name:'index',value:99,type:'number',scope:'global'})
  vi.spyOn(elementPickerApi, 'getSelected').mockResolvedValue({success:true,data:{selected:false}})
  vi.mocked(elementPickerApi.getSimilar).mockResolvedValue({success:true,data:{selected:true,similar:{pattern:'.item-{index}',count:4,minIndex:1,maxIndex:4}}})
  render(<ConfigPanel selectedNodeId={nodeId} />);await start()
  await act(async () => vi.advanceTimersByTimeAsync(500))
  ...
  expect(store.getState().variables).toEqual([{name:'index',value:1,type:'number',scope:'global'}])
  act(() => store.getState().undo())
  expect(store.getState().nodes[0].data.selector).toBe('#before')
  expect(store.getState().variables).toEqual(existing?[{name:'index',value:99,type:'number',scope:'global'}]:[])
  act(() => store.getState().redo())
  expect(store.getState().nodes[0].data.selector).toBe('.item-{index}')
  expect(store.getState().variables[0].value).toBe(1)
})
it('naming a new similar index does not rename references to an existing variable', async () => {
  store.getState().addVariable({name:'index',value:99,type:'number',scope:'global'})
  store.getState().updateNodeData(nodeId,{name:'引用 {index}'})
  vi.spyOn(elementPickerApi, 'getSelected').mockResolvedValue({success:true,data:{selected:false}})
  vi.mocked(elementPickerApi.getSimilar).mockResolvedValue({success:true,data:{selected:true,similar:{pattern:'.item-{index}',count:4,minIndex:1,maxIndex:4}}})
  render(<ConfigPanel selectedNodeId={nodeId} />);await start()
  await act(async () => vi.advanceTimersByTimeAsync(500))
  const input=screen.getByPlaceholderText('index')
  fireEvent.focus(input);fireEvent.change(input,{target:{value:'rowIndex'}});fireEvent.blur(input)
  await act(async () => vi.advanceTimersByTimeAsync(300))
  expect(screen.queryByText('变量重命名')).toBeNull()
  await act(async () => fireEvent.click(screen.getByText('确认使用')))
  expect(store.getState().nodes[0].data.name).toBe('引用 {index}')
  expect(store.getState().variables).toEqual([{name:'index',value:99,type:'number',scope:'global'},{name:'rowIndex',value:1,type:'number',scope:'global'}])
})
it('reports a similar picker business error and recovers on the next poll', async () => {
  vi.spyOn(elementPickerApi, 'getSelected').mockResolvedValue({success:true,data:{selected:false}})
  vi.mocked(elementPickerApi.getSimilar).mockResolvedValueOnce({success:false,error:'相似元素读取失败'})
    .mockResolvedValue({success:true,data:{selected:true,similar:{pattern:'.item-{index}',count:4,minIndex:1,maxIndex:4}}})
  render(<ConfigPanel selectedNodeId={nodeId} />); await start()
  await act(async () => vi.advanceTimersByTimeAsync(500))
  expect(store.getState().logs.some(log=>log.level==='error'&&log.message.includes('相似元素读取失败'))).toBe(true)
  expect(store.getState().nodes[0].data.selector).toBe('#before')
  await act(async () => vi.advanceTimersByTimeAsync(500))
