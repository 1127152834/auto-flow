// Source: WebRPA@5ccb900e, lib/globalTooltip.ts; adapted lifecycle, accessibility and AutoFlow colors.
/**
...
function show(target: Element, text: string) {
  ...
  if (arrowEl) {
    if (placement === 'top') {
      ...
      arrowEl.style.borderTop = '5px solid hsl(var(--brand-700))'
      arrowEl.style.borderBottom = ''
    } else {
      arrowEl.style.bottom = '100%'
      arrowEl.style.top = ''
      arrowEl.style.borderBottom = '5px solid hsl(var(--brand-500))'
  ...
  requestAnimationFrame(() => {
    if (!installed || tipEl !== el || !target.isConnected) return
...
function migrateTitle(el: Element) {
  ...
  if (html.hasAttribute('title')) {
    ...
    if (t != null && t !== '') {
      ...
      html.setAttribute('data-tip-migrated', '')
    }
    // Preserve accessible names of icon-only controls when moving the native title.
    const ownedLabel = html.getAttribute('data-tip-label')
    if (ownedLabel !== null && html.getAttribute('aria-label') === ownedLabel) {
      if (t) { html.setAttribute('aria-label', t); html.setAttribute('data-tip-label', t) }
      else { html.removeAttribute('aria-label'); html.removeAttribute('data-tip-label') }
    } else if (t && !html.textContent?.trim() && !html.hasAttribute('aria-label')) {
      html.setAttribute('aria-label', t)
      html.setAttribute('data-tip-label', t)
    }
    if (!t) html.removeAttribute('data-tip')
...
export function installGlobalTooltip() {
  ...
  return () => {
    ...
    arrowEl = null
    textNode = null
    document.querySelectorAll('[data-tip-migrated]').forEach(el => {
      if (!el.hasAttribute('title')) el.setAttribute('title', el.getAttribute('data-tip') || '')
      el.removeAttribute('data-tip')
      el.removeAttribute('data-tip-migrated')
      if (el.getAttribute('aria-label') === el.getAttribute('data-tip-label')) el.removeAttribute('aria-label')
