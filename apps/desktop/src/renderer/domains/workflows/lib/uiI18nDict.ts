// Source: WebRPA@5ccb900e, lib/uiI18nDict.ts; see SOURCE.md for license and adaptation boundaries.
...
// AutoFlow mock transport and fixture surface (no real browser or model execution).
Object.assign(PHRASES, {
  'Mock 单文件上限 2 MiB；真实后端接入后使用文件存储': 'Mock files are limited to 2 MiB; production uses backend file storage',
  '字节数': 'Bytes',
  '回复': 'Reply',
  '已接收到当前画布上下文。本轮只验证对话、流式消息和会话管理，没有调用真实模型，也没有自动修改工作流。': 'Canvas context received. This mock verifies chat, streaming and session management without calling a model or editing the workflow.',
  '会话不存在': 'Session not found',
  'Mock 连接可用；未连接真实模型': 'Mock connection ready; no model connected',
  '附件': 'Attachment', '，未解析文件正文': ', contents not parsed',
  '语音转写，未识别音频': 'transcription fixture; audio not recognized',
  '请先开启元素拾取': 'Start element picking first',
  '已模拟': 'Simulated', '的事件；未执行网页动作': 'events; no browser action executed',
  'Mock 模式禁止访问外部服务': 'External services are disabled in mock mode',
  '请求不是合法 JSON': 'Request is not valid JSON', '命令 ID 冲突': 'Command ID conflict',
  '磁盘写入失败，草稿未保存': 'Disk write failed; draft not saved',
  '工作流缺少 nodes': 'Workflow is missing nodes',
  '导出：本文件用于校验下载交互，并非可运行脚本': 'export: download fixture, not an executable script',
  '浏览器正被运行、录制或拾取占用': 'browser is occupied by a run, recording or picker',
  '起点不存在': 'Start node not found', '没有活跃运行': 'No active run',
  '请先打开空闲的': 'Open an available', 'Mock 浏览器': 'Mock browser',
  '定位结果（未查询网页）': 'locator result (no page query)',
  '尚未实现': 'not implemented', '不包含远程协作': 'does not include remote collaboration',
  '已发送': 'Sent', '事件': 'event',
  '普通模拟浏览；在录制或拾取面板开始后，操作会产生对应事件。': 'Mock browsing. Start recording or picking to emit interaction events.',
  '正在采集 Mock 事件': 'Capturing mock events', '普通 Mock 浏览': 'Mock browsing',
  '此处仅产生测试事件，不会访问输入的网址。': 'This fixture produces test events without visiting the entered URL.',
  '导航': 'Navigate', '提交测试按钮': 'Submit test button',
})
