...
/** Applies every paused-variable edit as one identified command. */
export async function sendDebugVariables(workflowId:string,request:DebugVariablesRequest):Promise<ApiResponse<DebugVariablesReceipt>> {
  if(!workflowId || !isDebugVariablesRequest(request))return {success:false,httpStatus:422,error:'变量修改内容或暂停身份无效'}
  const origin=getBackendBaseUrl(),revision=getStudioTransportRevision()
  const current=()=>revision===getStudioTransportRevision()
  ...
  try{
    ...
    if(!current())return unknown()
    if(response.status>=500)throw new Error('服务响应需要查询确认')
    if(isDebugVariablesReceipt(result,workflowId,request)){
      if(response.ok && result.success)return {success:true,data:result}
      if(!result.success)return {success:false,httpStatus:response.status,error:result.error || '变量修改被拒绝',data:result}
    }
    if(!response.ok && result && typeof result==='object' && !Array.isArray(result) && (!('commandId' in result) || result.commandId===request.commandId)){
      return {success:false,httpStatus:response.status,error:parseApiWireError(result)?.message || ('error' in result && typeof result.error==='string'?result.error:`HTTP ${response.status}`)}
    }
    throw new Error('变量修改确认身份或结构错误')
  }catch{
    if(!current())return unknown()
    try{
      const response=await studioFetch(scopeStudioUrl(`${origin}/api/events/commands/${encodeURIComponent(request.commandId)}`),{signal:AbortSignal.timeout(10000)})
      ...
      if(typeof status!=='number' || !Number.isInteger(status) || status<200 || status>599 || (result.success && status>=400))return unknown()
      return {success:result.success,httpStatus:status,data:result,...(!result.success?{error:result.error || '变量修改被拒绝'}:{})}
    }catch{return unknown()}
  }
}
