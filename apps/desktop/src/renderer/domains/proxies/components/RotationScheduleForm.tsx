...
export function RotationScheduleForm({ value, disabled, canSave, onDirtyChange, onSave, onClear }: {
  ...
  return <div className="grid gap-3">
    ...
    <fieldset disabled={disabled || !canSave} className="grid gap-3 sm:grid-cols-2">
      <label className="grid gap-2 text-sm">轮换模式<Select clearable={false} aria-label="轮换模式" value={mode} onValueChange={value => setMode((value ?? 'same_city') as typeof mode)} options={Object.entries(rotationModes).map(([value, label]) => ({ value, label }))} /></label>
      <label className="grid gap-2 text-sm">轮换周期<Select clearable={false} aria-label="轮换周期" value={String(minutes)} onValueChange={value => setMinutes(Number(value ?? 5))} options={intervals.map(item => ({ value: String(item), label: `每 ${item} 分钟${![5,10,30,60].includes(item) ? '（当前远程值）' : ''}`, disabled: ![5,10,30,60].includes(item) }))} /></label>
    </fieldset>
