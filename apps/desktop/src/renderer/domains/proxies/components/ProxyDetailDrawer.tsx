...
import { CircleNotch, Copy, Pulse, X } from '@phosphor-icons/react'
import { capability, type IpAllowlist, type ProxyMetadataDraft, type ProxyReferences, type ProxyView, type ProxyApi } from '../api'
...
import { ProxyRemoteControls } from './ProxyRemoteControls'
...
export function ProxyDetailDrawer({ open, proxy, references, probing, actionBusy = false, retryAfterSeconds = 0, canCopyCredentials = false, copying, onOpenChange, onProbe, onUpdateMetadata, onCopyCredentials, api, onRemoteChanged }: ProxyDetailDrawerProps) {
  const [tab, setTab] = useState('overview')
  const [dirty, setDirty] = useState(false)
  const [remoteSubmitting, setRemoteSubmitting] = useState(false)
  const [leave, setLeave] = useState<string>()
  const navigate = (next: string) => { if (remoteSubmitting) return; if (dirty) setLeave(next); else if (next === 'close') onOpenChange(false); else setTab(next) }
  ...
  return (
    <Dialog open={open} onOpenChange={value => { if (!value) navigate('close') }} busy={actionBusy || remoteSubmitting}>
      <DialogContent className="left-auto right-0 top-0 h-dvh w-[min(100vw,43rem)] max-w-none translate-x-0 translate-y-0 content-start overflow-y-auto rounded-none border-y-0 border-r-0 p-0 motion-safe:data-[state=open]:animate-in motion-safe:data-[state=open]:slide-in-from-right-2" aria-describedby="proxy-detail-description">
        <header className="sticky top-0 z-10 flex items-start justify-between border-b border-line bg-surface/95 px-6 py-5 backdrop-blur">
          ...
          <Button className="h-9 w-9 shrink-0 px-0" variant="ghost" aria-label="关闭代理详情" onClick={() => navigate('close')} disabled={actionBusy || remoteSubmitting}><X size={18} /></Button>
        </header>
        <Tabs value={tab} onValueChange={navigate} className="p-6">
          ...
          <TabsContent value="rotation" className="grid gap-4">
            <ProxyRemoteControls api={api} proxy={proxy} onChanged={onRemoteChanged} onDirtyChange={setDirty} onSubmittingChange={setRemoteSubmitting} />
          </TabsContent>
        ...
        <AlertDialog open={Boolean(leave)} onOpenChange={value => { if (!value) setLeave(undefined) }}><AlertDialogContent><AlertDialogTitle>放弃未保存的轮换设置？</AlertDialogTitle><AlertDialogDescription>尚未提交的修改将丢失。已经受理的远程操作会继续执行。</AlertDialogDescription><div className="flex justify-end gap-2"><AlertDialogCancel asChild><Button>继续编辑</Button></AlertDialogCancel><AlertDialogAction asChild><Button variant="primary" onClick={() => { setDirty(false); if (leave === 'close') onOpenChange(false); else if (leave) setTab(leave); setLeave(undefined) }}>放弃修改</Button></AlertDialogAction></div></AlertDialogContent></AlertDialog>
