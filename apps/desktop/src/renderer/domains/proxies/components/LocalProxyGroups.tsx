      ...
      {!page.items.length ? (
        ...
        <div className="overflow-x-auto">
          <Table className="w-full min-w-[680px] text-left text-sm">
            <TableHeader className="bg-surface-subtle text-xs text-muted"><TableRow>{['组名称', '成员', '健康概况', '关联配置', '描述', '操作'].map((label) => <TableHead className="px-4 py-3 font-medium" key={label}>{label}</TableHead>)}</TableRow></TableHeader>
      ...
      <DialogContent className="max-h-[90dvh] w-[min(94vw,48rem)] max-w-none overflow-y-auto" aria-describedby="proxy-group-editor-description">
        ...
        <form className="grid gap-5" onSubmit={(event) => void submit(event, false)}>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField label="组名称" htmlFor="proxy-group-name">{(a11y) => <><Input {...a11y} value={name} maxLength={120} onChange={(event) => setName(event.target.value)} /></>}</FormField>
            <FormField label="描述" htmlFor="proxy-group-description">{(a11y) => <><Input {...a11y} value={description} maxLength={1000} onChange={(event) => setDescription(event.target.value)} /></>}</FormField>
          </div>
          <section className="grid gap-3">
            <div><h3 className="text-sm font-semibold text-ink">选择成员</h3><p className="mt-1 text-xs text-muted">远端缺失的代理不会出现在可选列表中。</p></div>
            <div className="grid gap-2 sm:grid-cols-2"><SearchInput onClear={() => setQuery('')} aria-label="搜索组成员" value={query} placeholder="搜索名称、城市或运营商" onChange={(event) => setQuery(event.target.value)} /><Select clearable={false} aria-label="筛选成员健康" value={health} onValueChange={(value) => setHealth((value ?? ''))} options={[{ value: "", label: "全部健康状态" }, { value: "healthy", label: "健康" }, { value: "unhealthy", label: "异常" }, { value: "untested", label: "未检测" }]} /></div>
            <ScrollArea aria-label="可选组成员" className="h-48 rounded-control border border-line">
              {candidates.map((proxy) => <label className="flex cursor-pointer items-center gap-3 border-b border-line px-4 py-3 last:border-b-0 hover:bg-surface-hover" key={proxy.id}><Checkbox checked={memberIds.includes(proxy.id)} onCheckedChange={(checked) => toggle(proxy.id, checked === true)} /><span className="min-w-0 flex-1 truncate text-sm text-ink">{proxy.name_override || proxy.name}</span><HealthPill health={proxy.health} /></label>)}
          ...
          {error ? <div className="rounded-control bg-danger-soft px-4 py-3 text-sm text-danger" role="alert"><p>{error}</p>{riskRequired ? <Button className="mt-3" type="button" variant="primary" disabled={retryAfterSeconds > 0} onClick={(event) => void submit(event, true)}>{retryAfterSeconds > 0 ? `${retryAfterSeconds} 秒后可重试` : '了解风险并保存'}</Button> : null}</div> : null}
