...
it('releases a definitively rejected pull so its invalid reference can be corrected', async () => {
  ...
  await userEvent.click(screen.getByRole('button', { name: '开始拉取' }))
  await vi.waitFor(() => expect(pullImage).toHaveBeenCalledTimes(2))
  expect(pullImage.mock.calls[1][0]).toEqual({ reference: 'redroid/redroid:13', requestId: expect.any(String), allowUnknownDiskEstimate: false })
  expect(pullImage.mock.calls[1][0].requestId).not.toBe(pullImage.mock.calls[0][0].requestId)
})

it('freezes disk confirmation for a lost response and resets it when the reference changes', async () => {
  const pullImage = vi.fn().mockRejectedValueOnce(new Error('连接中断')).mockResolvedValueOnce({ operationId: 'accepted', requestId: 'second', state: 'succeeded', stageLabel: '已完成' })
  renderManager({ pullImage })
  const form = await screen.findByRole('form', { name: '拉取镜像' })
  const input = within(form).getByLabelText('拉取镜像引用')
  const confirmation = within(form).getByRole('checkbox', { name: /最终占用未知/ })
  ...
  await userEvent.click(screen.getByRole('button', { name: '按原编号重试' }))
  await vi.waitFor(() => expect(pullImage).toHaveBeenCalledTimes(2))
  expect(pullImage.mock.calls[0][0]).toEqual(pullImage.mock.calls[1][0])
  expect(pullImage.mock.calls[0][0]).toEqual({ reference: 'redroid/redroid:13', requestId: expect.any(String), allowUnknownDiskEstimate: true })
  await userEvent.clear(input)
  await userEvent.type(input, 'redroid/redroid:14')
  expect(confirmation).not.toBeChecked()
})

it.each(['ANDROID_DISK_SPACE_INSUFFICIENT', 'ANDROID_DISK_PROBE_FAILED', 'ANDROID_DISK_ESTIMATE_UNKNOWN'])('releases a definitive pull preflight rejection %s', async (code) => {
  const pullImage = vi.fn().mockRejectedValueOnce(new ApiClientError('磁盘预检被拒绝', 409, code)).mockResolvedValueOnce({ operationId: 'accepted', requestId: 'second', state: 'succeeded', stageLabel: '已完成' })
  renderManager({ pullImage })
  const form = await screen.findByRole('form', { name: '拉取镜像' })
  const input = within(form).getByLabelText('拉取镜像引用')
  await userEvent.type(input, 'redroid/redroid:13')
  await userEvent.click(screen.getByRole('button', { name: '开始拉取' }))
  expect(await screen.findByRole('alert')).toHaveTextContent('磁盘预检被拒绝')
  expect(input).toBeEnabled()
  await userEvent.click(screen.getByRole('button', { name: '开始拉取' }))
  await vi.waitFor(() => expect(pullImage).toHaveBeenCalledTimes(2))
  expect(pullImage.mock.calls[1][0].requestId).not.toBe(pullImage.mock.calls[0][0].requestId)
