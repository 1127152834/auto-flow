    ...
    <div className="grid items-start gap-5 lg:grid-cols-[2fr_1fr]">
      <fieldset disabled={busy || uncertain || disabled} className="divide-y divide-line rounded-card border border-line bg-surface px-6">
        ...
        <section className="space-y-5 py-4"><h2 className="flex items-center gap-3 text-lg font-semibold"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-clay text-sm text-white">2</span>运行配置</h2>
          <div className={row}><label htmlFor="device-image" className="text-sm">系统镜像</label><Select id="device-image" {...form.register('imageId', { required: '请选择镜像' })}><option value="">选择已缓存镜像</option>{environment.images?.map(image => <option key={image.id} value={image.id}>{image.name}</option>)}</Select></div>
          <p className="rounded-control bg-surface-subtle p-3 text-xs text-muted">软件渲染 · ARM64 · Root 能力尚未在此实例验证</p>
          <div className={row}><label htmlFor="device-resolution" className="text-sm">分辨率</label><Select id="device-resolution" value={`${values.width}x${values.height}`} onChange={event => { const [width, height] = event.target.value.split('x').map(Number); form.setValue('width', width, { shouldDirty: true }); form.setValue('height', height, { shouldDirty: true }) }}>{[...new Set(['540x960', '720x1280', '1080x1920', `${source?.width ?? 720}x${source?.height ?? 1280}`])].map(size => <option key={size} value={size}>{size.replace('x', ' × ')}</option>)}</Select></div>
