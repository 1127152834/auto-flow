...
export function ImageManager({ api }: Props) {
  ...
  const pull = () => {
    ...
    return run('pull', async () => { const next = await api.pullImage(request); setOperation(next); if (next.state === 'succeeded') pullRequest.current = null; setMessage(`拉取操作已接受：${request.requestId}`) })
  }
