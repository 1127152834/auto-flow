...
export function DeviceSelect({ devices, value, onChange, disabled }: { devices: AndroidDevice[]; value: string; onChange(value: string): void; disabled: boolean }) {
  return <><label htmlFor="android-device" className="text-xs font-medium">安卓设备</label><Select id="android-device" value={value} onChange={event => onChange(event.target.value)} disabled={disabled} className="h-9 w-52"><option value="">{devices.length ? '选择安卓设备' : '请先创建安卓设备'}</option>{devices.map(device => <option key={device.deviceId} value={device.deviceId} disabled={!canOpen(device)}>{device.name} · {deviceStatus(device)}</option>)}</Select></>
}
