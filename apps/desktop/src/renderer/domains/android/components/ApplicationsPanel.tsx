...
export function ApplicationsPanel({ api, apps, session, onSession, onRefresh }: Props) {
  ...
  const canWrite = session?.access === 'manual' && session.state === 'connected' && session.endpoint === 'embedded'
  ...
  return <section aria-label="应用管理" className="rounded-card border border-line bg-surface p-5">
    ...
    {error && <div role="alert" className="mt-3 text-sm text-danger">{error}<button type="button" onClick={refreshUnknown}>刷新应用状态</button></div>}
