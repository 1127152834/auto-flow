...
export function RuntimeDiagnostics({ api }: { api: DiagnosticsApi }) {
  ...
  return <section aria-label="安卓运行环境诊断" aria-busy={checking} className="space-y-5 rounded-card border border-line bg-surface p-5"><header><div className="flex items-start justify-between gap-3"><div><h2 className="font-semibold">运行环境诊断</h2><p className="mt-1 text-sm text-muted">{environment.data.message ?? '尚未采集'} · {environment.data.runtimeId ?? 'unknown'}</p></div><button type="button" disabled={checking || checkPending} onClick={() => void refresh()}>{checking ? '检查中…' : checkPending ? '请先核实' : '重新检查'}</button></div><p className="mt-2 text-xs text-muted">检查时间：<time dateTime={environment.data.checkedAt}>{environment.data.checkedAt}</time>{stale && <span className="ml-2 text-danger">快照可能陈旧</span>}</p></header><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{Object.entries(checks).map(([name, check]) => <div key={name} className="rounded-control border border-line p-3"><div className="flex justify-between gap-3 text-sm"><strong>{labels[name] ?? name}</strong><span>{statusLabels[check.status]}</span></div><p className="mt-1 text-xs text-muted">{check.message}</p>{check.action && <p className="mt-1 text-xs text-muted">处理：{check.action}</p>}</div>)}</div><div className="grid gap-2 sm:grid-cols-2">{([
    ['宿主工作区可用空间', environment.data.hostWorkspaceFreeBytes],
    ['Linux VM Docker 可用空间', environment.data.vmDockerFreeBytes],
  ] as const).map(([label, bytes]) => <div role="group" aria-label={label} key={label} className="rounded-control border border-line p-3"><h3 className="text-sm font-medium">{label}</h3><p className="mt-1 text-sm" title={bytes == null ? undefined : `${bytes} 字节`}>{bytes == null ? '未知' : `${(bytes / 1024 ** 3).toFixed(2)} GiB`}</p></div>)}</div><div className="rounded-control bg-surface-subtle p-3 text-sm"><h3 className="font-medium">能力</h3><p className="mt-2">管理：{String(capabilities.data.management)} · 控制：{String(capabilities.data.control)} · 镜像：{String(capabilities.data.images)}</p>{Object.values(reasons).map(reason => <p key={reason} className="mt-1 text-xs text-muted">{reason}</p>)}</div>{checkError && <p role="alert" className="text-sm text-danger">{checkError}</p>}{checkPending && checkRequestId && <p className="text-sm text-muted">本次检查结果未知，保留旧快照直到核实。<button type="button" disabled={checking} onClick={() => void verifyCheck()}>核实原检查请求</button></p>}</section>
}
