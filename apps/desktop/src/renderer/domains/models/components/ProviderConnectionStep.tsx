...
export function ProviderConnectionStep({ draft, preset, hasStoredKey, error, disabled = false, onChange }: { draft: ProviderDraft; preset: ProviderPreset; hasStoredKey: boolean; error?: string; disabled?: boolean; onChange(patch: Partial<ProviderDraft>): void }) {
  ...
  return <section className="grid gap-5">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <FormField label="接口协议" htmlFor="provider-kind"><Select disabled={disabled} value={draft.providerKind} onChange={(event) => onChange({ providerKind: event.target.value as ProviderKind })}>{kinds.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select></FormField>
    </div>
