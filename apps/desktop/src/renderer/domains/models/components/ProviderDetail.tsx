import { CircleNotch, DotsThree, PencilSimple, Pulse } from '@phosphor-icons/react'
  ...
  return <section aria-label="供应商详情" className="grid gap-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div className="flex min-w-0 items-start gap-4"><ProviderLogo presetId={provider.presetId} name={provider.name} size="large" /><div className="min-w-0">
        <div className="flex flex-wrap items-center gap-3"><h2 className="m-0 break-words text-xl font-semibold">{provider.name}</h2><Badge className={status === '连接正常' ? 'bg-sage-soft text-sage-strong' : status === '连接异常' ? 'bg-red-50 text-red-700' : 'bg-surface-subtle text-muted'}>{status}</Badge></div>
      ...
      <div className="flex gap-2"><Button onClick={onTest} disabled={busy}>{testing ? <CircleNotch className="animate-spin" size={17} /> : <Pulse size={17} />}测试连接</Button><Button onClick={onEdit} disabled={busy}><PencilSimple size={17} />编辑</Button>
        <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" aria-label="供应商更多操作" className="w-10 p-0"><DotsThree size={23} /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem disabled={crudBusy} onSelect={onToggle}>{provider.enabled ? '停用供应商' : '启用供应商'}</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem disabled={crudBusy} className="text-red-700" onSelect={onDelete}>删除供应商</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
      </div>
    </div>
