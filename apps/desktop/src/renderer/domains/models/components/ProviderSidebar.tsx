...
export function ProviderSidebar({ providers, selectedId, search, onSearch, onSelect, onAdd }: {
  ...
  return <aside aria-label="供应商" className="border-b border-line bg-surface-subtle p-4 min-[920px]:border-b-0 min-[920px]:border-r">
    ...
    <div className="relative mb-4"><MagnifyingGlass size={17} className="pointer-events-none absolute left-3 top-3 text-muted" /><Input aria-label="搜索供应商" placeholder="搜索供应商" value={search} onChange={event => onSearch(event.target.value)} className="pl-9" /></div>
    <div className="grid gap-1 max-[919px]:grid-cols-2 max-[600px]:grid-cols-1">
      {visible.map(provider => <button key={provider.id} type="button" onClick={() => onSelect(provider.id)} aria-pressed={selectedId === provider.id} className={`flex min-w-0 items-center gap-3 rounded-control border-l-[3px] p-3 text-left transition-colors ${selectedId === provider.id ? 'border-clay bg-clay-soft/60' : 'border-transparent hover:bg-surface-hover'}`}>
        ...
        <span aria-label={!provider.enabled ? '已停用' : provider.connectionStatus === 'connected' ? '连接正常' : provider.connectionStatus === 'failed' ? '连接异常' : '未测试'} className={`h-1.5 w-1.5 shrink-0 rounded-full ${!provider.enabled ? 'bg-line-strong' : provider.connectionStatus === 'connected' ? 'bg-sage' : provider.connectionStatus === 'failed' ? 'bg-red-600' : 'bg-amber-600'}`} />
      </button>)}
    </div>
