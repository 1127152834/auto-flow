...
export function FeedbackCard({ feedback }: { feedback: ModelFeedback }) {
  ...
  return <div role={success ? 'status' : 'alert'} className={`flex items-start gap-3 rounded-control border p-4 text-sm ${success ? 'border-sage/30 bg-sage-soft text-sage-strong' : 'border-red-200 bg-red-50 text-red-800'}`}>
