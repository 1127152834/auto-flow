...
function ProviderWizardSession({ open, instanceId, onOpenChange, provider, api, onSaved }: { open: boolean; instanceId: string; onOpenChange(open: boolean): void; provider?: ModelProviderRead | null; api: ModelApi; onSaved(provider: ModelProviderRead): void }) {
  ...
  const footer = step === 'catalog' ? <><Button onClick={() => close(false)}>取消</Button><Button variant="primary" onClick={() => setStep('connection')}>下一步<CaretRight size={15} /></Button></> : step === 'connection' ? <>{!editing && <Button variant="ghost" onClick={() => setStep('catalog')}><ArrowLeft size={15} />上一步</Button>}<span className="grow" /><Button onClick={() => close(false)}>取消</Button>{editing ? <Button variant="primary" disabled={!valid || busy} onClick={() => save.mutate()}>{save.isPending && <Spinner  />}{connectionChanged ? '测试并保存' : '保存修改'}</Button> : <Button variant="primary" disabled={!valid || busy} onClick={() => preview.mutate()}>{preview.isPending ? <Spinner  /> : <ShieldCheck />}测试连接</Button>}</> : <><Button variant="ghost" onClick={() => { setStep('connection'); setDiscovery(null); setSelected(new Set()) }}><ArrowLeft />修改连接</Button><span className="grow" /><Button onClick={() => close(false)}>取消</Button><Button variant="primary" disabled={busy || !discovery} onClick={() => save.mutate()}>{save.isPending && <Spinner  />}保存供应商{selected.size ? `和 ${selected.size} 个模型` : ''}</Button></>
  ...
  return <Modal open={open} onOpenChange={close} closeDisabled={busy} size="large" title={editing ? '编辑模型供应商' : '添加供应商'} description={editing ? '修改基础信息可直接保存；连接发生变化时先验证。' : '选择供应商、验证连接，然后按需添加模型。'} footer={footer}>
    <div className="grid gap-6">
      <div aria-label="新增供应商步骤" className="grid grid-cols-3 gap-2">{(['catalog', 'connection', 'models'] as Step[]).map((value, index) => <span key={value} aria-current={step === value ? 'step' : undefined} className={`rounded-lg px-3 py-2 text-center text-sm ${step === value ? 'bg-clay text-on-accent' : 'bg-surface-subtle text-muted'}`}>{index + 1}. {['选择供应商', '连接信息', '选择模型'][index]}</span>)}</div>
      ...
      {step === 'models' && save.error && <div role="alert" className="rounded-card border border-danger/30 bg-danger-soft p-4 text-sm text-danger"><strong className="block">保存失败，可以重试</strong>{message(save.error)}</div>}
