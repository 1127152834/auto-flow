...
async function createStatus(name) { await click('业务状态','[role=tab]'); await click('新建状态'); await input('#status-name',name); await click('创建状态'); await visible(name); await click('数据记录','[role=tab]') }
...
async function applyNameFilter(value) { await click('筛选'); await select('字段','姓名'); await input('[aria-label=比较值]',value); await select('字段运算符','开头是'); await click('应用筛选'); await wait(300) }
...
async function click(text,selector='button') { if(selector==='button'&&['导出 Excel','重新导入 Excel','批量设置状态'].includes(text)) { await click('更多操作'); selector='[role=menuitem]'; }  const expression=`(()=>{const e=[...document.querySelectorAll(${JSON.stringify(selector)})].find(x=>(x.textContent.trim()===${JSON.stringify(text)}||x.getAttribute('aria-label')===${JSON.stringify(text)})&&x.getClientRects().length&&!x.disabled&&x.getAttribute('aria-disabled')!=='true');if(!e)return null;e.scrollIntoView({block:'center',inline:'center',behavior:'instant'});const r=e.getBoundingClientRect(),x=r.x+r.width/2,y=r.y+r.height/2,hit=document.elementFromPoint(x,y);return hit&&(hit===e||e.contains(hit))?{x,y}:null})()`; const point=await waitFor(renderer,expression,`clickable ${text}`,7000); await renderer.command('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',clickCount:1}); await renderer.command('Input.dispatchMouseEvent',{type:'mouseReleased',...point,button:'left',clickCount:1}); await wait(150) }
