...
const sourceDigest = await digest(original), checks = []
...
try {
  ...
  tableId=largeTable.tableId; await openDetail('一万行性能表'); await waitFor(renderer,`document.querySelectorAll('tbody tr').length===50`,'first 50-row page',30000)
  const first=await renderer.evaluate(`document.querySelector('tbody tr')?.innerText`); const pageStarted=performance.now(); await click('下一页'); await waitFor(renderer,`document.querySelectorAll('tbody tr').length===50&&document.querySelector('tbody tr')?.innerText!==${JSON.stringify(first)}`,'second 50-row page'); const pageMs=Math.round(performance.now()-pageStarted)
  ...
  const report = { result: 'passed', checkedAt: new Date().toISOString(), scope: 'PM2 detail flows; system open/save panel results injected, renderer UI, IPC proof, HTTP, SQLite and workbook I/O real', checks, excluded: ['manual native file-panel interaction'] }
...
async function setPicker(input, save) { await native.evaluate(`qaElectron.dialog.showOpenDialog=async()=>({canceled:false,filePaths:[${JSON.stringify(input)}]});qaElectron.dialog.showSaveDialog=async()=>({canceled:false,filePath:${JSON.stringify(save)}});true`) }
...
async function click(text,selector='button') { const expression=`(()=>{const e=[...document.querySelectorAll(${JSON.stringify(selector)})].find(x=>(x.textContent.trim()===${JSON.stringify(text)}||x.getAttribute('aria-label')===${JSON.stringify(text)})&&x.getClientRects().length&&!x.disabled&&x.getAttribute('aria-disabled')!=='true');if(!e)return null;e.scrollIntoView({block:'center',inline:'center',behavior:'instant'});const r=e.getBoundingClientRect(),x=r.x+r.width/2,y=r.y+r.height/2,hit=document.elementFromPoint(x,y);return hit&&(hit===e||e.contains(hit))?{x,y}:null})()`; const point=await waitFor(renderer,expression,`clickable ${text}`,7000); await renderer.command('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',clickCount:1}); await renderer.command('Input.dispatchMouseEvent',{type:'mouseReleased',...point,button:'left',clickCount:1}); await wait(150) }
...
async function select(label,option) { await click(label,`[aria-label=${JSON.stringify(label)}]`); const point=await renderer.evaluate(`(()=>{const e=[...document.querySelectorAll('[role=option]')].find(x=>x.textContent.includes(${JSON.stringify(option)})&&x.getClientRects().length);if(!e)return null;const r=e.getBoundingClientRect();return{x:r.x+r.width/2,y:r.y+r.height/2}})()`); assert.ok(point,`option missing: ${option}`); await renderer.command('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',clickCount:1}); await renderer.command('Input.dispatchMouseEvent',{type:'mouseReleased',...point,button:'left',clickCount:1}); await wait(120) }
async function selectAt(label,index,option) { const point=await waitFor(renderer,`(()=>{const e=[...document.querySelectorAll('[aria-label=${label}]')][${index}];if(!e)return null;e.scrollIntoView({block:'center',behavior:'instant'});const r=e.getBoundingClientRect(),x=r.x+r.width/2,y=r.y+r.height/2;return e.contains(document.elementFromPoint(x,y))?{x,y}:null})()`,`select ${label} ${index}`); await renderer.command('Input.dispatchMouseEvent',{type:'mousePressed',...point,button:'left',clickCount:1}); await renderer.command('Input.dispatchMouseEvent',{type:'mouseReleased',...point,button:'left',clickCount:1}); await wait(120); const choice=await waitFor(renderer,`(()=>{const e=[...document.querySelectorAll('[role=option]')].find(x=>x.textContent.includes(${JSON.stringify(option)})&&x.getClientRects().length);if(!e)return null;const r=e.getBoundingClientRect();return{x:r.x+r.width/2,y:r.y+r.height/2}})()`,`option ${option}`); await renderer.command('Input.dispatchMouseEvent',{type:'mousePressed',...choice,button:'left',clickCount:1}); await renderer.command('Input.dispatchMouseEvent',{type:'mouseReleased',...choice,button:'left',clickCount:1}); await wait(120) }
...
async function workflowClosed(title,timeout) { await waitFor(renderer,`!document.querySelector('[role=dialog]')`,`${title} closes after success`,timeout) }
