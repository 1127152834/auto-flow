...
try {
  ...
  await closeDialog(); await click('导出 Excel'); await click('选择保存位置'); await visible('目标文件已存在，请选择新的文件名'); checkpoint('a repeated save target was rejected instead of overwriting the first export')
...
async function visible(text,timeout=15000) { if(nativeMode&&text==='目标文件已存在，请选择新的文件名')timeout=Math.max(timeout,180000); if(text==='返回数据表')return waitFor(renderer,"!!document.querySelector('[aria-label=返回数据表]')",text,timeout); return waitFor(renderer,`document.body?.innerText.includes(${JSON.stringify(text)})`,text,timeout) }
