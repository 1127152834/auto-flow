...
const evidenceDir = await mkdtemp(join(await mkdir(evidenceRoot, { recursive: true }).then(() => evidenceRoot), 'formal-api-trigger-electron-'))
const userData = await mkdtemp(join(tmpdir(), 'autoflow-studio-b6-api-trigger-'))
const workflowName = 'B6 API触发器正式闭环'
const checks = []
...
try {
  ...
  const profile = await api(runtime, '/v1/profiles', { method: 'POST', body: {
    name: 'B6 API触发验收配置', description: '临时工作区；API触发器正式验收', startUrl: 'about:blank', locale: 'zh-CN', timezone: 'Asia/Shanghai', geoip: false,
  ...
  const triggerId = await addNode(studio, 'API触发器', 'api_trigger')
  await setInput(studio, 'input[placeholder="如: https://api.example.com/status"]', fixture.url)
  await setInput(studio, 'input[placeholder="如: data.status 或 $.result.code"]', 'status')
  await setInput(studio, 'input[placeholder="如: success 或 200"]', 'ready')
  await setInput(studio, 'input[placeholder="如: api_response"]', 'api_response')
  const printId = await addNode(studio, '打印日志', 'print_log')
  await setInput(studio, 'input[placeholder="要打印的日志信息"]', 'API触发条件已满足')
  ...
  checkpoint('通过正式 Studio 真实 UI 配置 API 地址、JSONPath 条件、响应变量并连接后续日志节点')
  ...
  assert.ok(saved)
  assert.deepEqual(saved.nodes.map(node => node.data.moduleType), ['api_trigger', 'print_log'])
  assert.equal(saved.edges.length, 1)
  checkpoint('正式保存接口写入 API 触发器配置、响应变量、后续日志节点和连线')

  await click(studio, '运行 (F5)', '[aria-label="运行 (F5)"]')
  await click(studio, '运行 (F5)', '[role="menuitem"]')
  const run = await waitForValue(async () => (await api(runtime, `/workflow-runs?documentId=${encodeURIComponent(saved.id)}&cursor=0&limit=20`)).items[0], 'API trigger run creation', 20_000)
  const terminal = await waitForValue(async () => { const value = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}`); return ['completed', 'failed', 'stopped', 'interrupted'].includes(value.status) ? value : null }, 'API trigger terminal run', 90_000)
  ...
  assert.deepEqual(byNode[triggerId], { status: 'ready', marker: 'api-fixture' })
  assert.deepEqual(byNode[printId], { level: 'info', message: 'API触发条件已满足' })
  const logs = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}/logs?cursor=0&limit=50`)
  assert.ok(logs.items.some(item => item.nodeId === triggerId && item.message.includes('API条件满足')))
  assert.ok(logs.items.some(item => item.nodeId === printId && item.message === 'API触发条件已满足'))
  checkpoint('本地 loopback API 返回条件数据，真实 worker 写入响应变量并执行后续日志节点')

  await wait(500)
  assert.deepEqual(cloakProcesses(userData), [])
  checkpoint('无浏览器动作的 API 触发流程完成后无 CloakBrowser、worker 或资源残留')
  await capture(studio, join(evidenceDir, 'completed.png'))
  const report = {
    evidenceId: 'BE-B6-api-trigger-formal-electron', checkedAt: new Date().toISOString(),
    ...
    nodes: [{ moduleType: 'api_trigger', nodeId: triggerId, result: byNode[triggerId] }, { moduleType: 'print_log', nodeId: printId, result: byNode[printId] }],
    fixture: { requests: fixture.requests, response: { status: 'ready', marker: 'api-fixture' } },
    buildSha256: await buildHash(),
    boundaries: { workspace: 'ephemeral', userDatabaseTouched: false, browserLaunch: 'none', network: 'local loopback only', interaction: 'formal Electron via CDP mouse and keyboard; public sidecar APIs only for fixture setup and evidence reads; no Store or page-internal business function access', platformPending: 'Windows/macOS Intel and packaged-entry reachability remain separately unverified' },
  }
  ...
  studio?.close(); main?.close(); await stop(desktop?.child); await fixture?.close(); await rm(userData, { recursive: true, force: true })
}

function checkpoint(message) { checks.push(message); console.log(message) }
async function startFixture() {
  const server = createServer((request, response) => {
    if (request.url !== '/status' || request.method !== 'GET') { response.writeHead(404); response.end(); return }
    fixture.requests += 1
    const body = Buffer.from(JSON.stringify({ status: 'ready', marker: 'api-fixture' }))
    response.writeHead(200, { 'content-type': 'application/json', 'content-length': body.length }); response.end(body)
  })
  const state = { requests: 0, url: '', close: () => new Promise(resolve => { server.close(resolve); server.closeAllConnections() }) }
