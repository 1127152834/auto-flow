...
export async function main(options={}) {
  ...
  const wrapperSha256=recordPages?createHash('sha256').update(await readFile(join(root,'scripts/qa-project-alignment-r2.mjs'))).digest('hex'):null
  ...
  const provenance={gitHead:gitHead.trim(),dirtyFiles:workingChanges.trim().split('\n').filter(Boolean),scriptSha256,wrapperSha256,options:{recordPages,manual,visualDirectory},desktopBuildSha256:buildHash.digest('hex')}
  ...
  const evidence=await mkdtemp(join(evidenceParent,'run-')), checks=[], checkScreenshotMap=[], injections=[], commandErrors=[], screenshots=[]
  ...
  const report=async(result,error)=>writeFile(join(evidence,'result.json'),JSON.stringify({result,checkedAt:new Date().toISOString(),platform:process.platform,arch:process.arch,mode:manual?'manual-tool':recordPages?'record-pages':visualDirectory?'visual-directory':'automatic',visualReview:'pending',provenance,checks,checkScreenshotMap,injections,commandErrors,screenshots,error,workspace:owner,excluded:['Windows','其他架构','打包应用','用户手动执行结果']},null,2)+'\n')
  const checkpoint=(message,screenshotIds=[])=>{checks.push(message);if(screenshotIds.length)checkScreenshotMap.push({check:message,screenshotIds});console.log(message)}
  ...
  async function waitFact(read,label,timeout=15000){const start=Date.now();while(Date.now()-start<timeout){const value=await read();if(value)return value;await wait(150)}throw new Error(`timed out waiting for ${label}`)}
  ...
  async function uiDataFlow() {
    ...
    for(let index=0;index<3;index++){
      ...
      for(const field of fields){await click('',`[aria-label="${field.name}值状态"]`);if(index===2&&field.name==='发布日期'){await click('',`[role=option][data-choice-value="null"]`);continue}await click('',`[role=option][data-choice-value="value"]`);if(field.type==='boolean'){await click('',`[aria-label="${field.name}"]`);await click(index===0?'是':'否','[role=option]')}else{const value=field.name==='标题'?['温室管理清单','智能温室控制方案🌱','温室种植技术要点'+ 'A'.repeat(100)][index]:field.name==='文章链接'?(index===2?'':`https://example.com/article/${index+1}`):field.type==='date'?'2026-09-13':String(index);await input(`#record-${field.ref.fieldId}`,value)}}
  ...
  async function recordPagesFlow(){
    ...
    await renderer.evaluate(`(()=>{const original=window.fetch.bind(window);window.fetch=async(...args)=>{const input=args[0],method=(args[1]?.method??'GET').toUpperCase(),path=typeof input==='string'?input:input.url;if(method==='GET'&&path.includes('/records/')){window.fetch=original;await new Promise(resolve=>window.__r2ReleaseRead=resolve)}return original(...args)};true})()`);injections.push({kind:'delayed-real-record-read',at:new Date().toISOString(),scope:'E4，仅延迟真实记录GET'});await click(viewLabel);await waitFor(renderer,"Boolean(document.querySelector('[aria-label=\"正在加载记录\"]'))",'正在加载记录');await capture('r2-detail-loading',{stateSource:'fault-injection',navigation:'pointer',level:'E4',reference:'R2-A',injection:'仅延迟真实记录GET'});await renderer.evaluate('window.__r2ReleaseRead();true')
    await waitFor(renderer,"Boolean(document.querySelector('[data-record-page=detail] [aria-label=基本信息]'))",'记录详情内容');await waitFor(renderer,"!document.querySelector('[role=status][data-tone]')",'详情正常态通知自然退出');await capture('r2-detail-normal',source)
    await click('编辑记录');await waitFor(renderer,"Boolean(document.querySelector('[data-record-page=edit] #record'))",'编辑记录表单');await waitFor(renderer,"!document.querySelector('[role=status][data-tone]')",'编辑页正常态通知自然退出');await capture('r2-edit-normal',{...source,reference:'R2-C'});const title=data.fields.find(field=>field.name==='标题'),number=data.fields.find(field=>field.name==='优先级')
    await renderer.evaluate(`(()=>{const original=window.fetch.bind(window);window.__r2ValidationOriginalFetch=original;window.__r2RecordWrites=0;window.fetch=async(...args)=>{const input=args[0],method=(args[1]?.method??'GET').toUpperCase(),path=typeof input==='string'?input:input.url;if(method==='PATCH'&&path.includes('/records/'))window.__r2RecordWrites++;return original(...args)};true})()`);await input(`#record-${number.ref.fieldId}`,'不是数字');await click('保存修改');await visible('请输入有效数字');assert.equal(await renderer.evaluate('window.__r2RecordWrites'),0,'前端数值校验不得发写请求');assert.equal(await renderer.evaluate('document.activeElement?.id'),`record-${number.ref.fieldId}`);await capture('r2-edit-validation',{stateSource:'ui-validation',navigation:'pointer',level:'E1',reference:'R2-D'});await renderer.evaluate('window.fetch=window.__r2ValidationOriginalFetch;true');await input(`#record-${number.ref.fieldId}`,'0')
    await input(`#record-${title.ref.fieldId}`,'温室管理清单（修订）');await click('返回记录列表');await waitFor(renderer,"Boolean(document.querySelector('[role=alertdialog]'))",'未保存离开确认');await capture('r2-edit-leave-confirm',{stateSource:'ui',navigation:'pointer',level:'E1',reference:'R2-E'});await click('继续编辑');assert.equal(await renderer.evaluate(`document.querySelector('#record-${title.ref.fieldId}').value`),'温室管理清单（修订）');await capture('r2-edit-dirty',{...source,reference:'R2-C'});await click('保存修改');await waitFor(renderer,"Boolean(document.querySelector('[data-record-page=detail] [aria-label=基本信息]'))",'保存后记录详情');await waitFor(renderer,"document.body.innerText.includes('温室管理清单（修订）')",'编辑结果显示')
    const routeParts=(await renderer.evaluate('location.hash')).replace(/^#\//,'').split('/'),recordPath=`${data.base}/records/${routeParts[7]}?datasetGeneration=${data.table.datasetGeneration}&recordKeyType=${routeParts[6]}`,beforeStatus=await api(recordPath)
    await click('','[aria-label="记录业务状态"]');const selectGeometry=await renderer.evaluate(`(()=>{const trigger=document.querySelector('[aria-label="记录业务状态"]'),popup=document.querySelector('[data-af-popup]'),a=trigger.getBoundingClientRect(),b=popup.getBoundingClientRect();return {triggerWidth:a.width,popupWidth:b.width,viewport:innerWidth}})()`);assert.ok(selectGeometry.popupWidth<=selectGeometry.viewport&&selectGeometry.popupWidth>=selectGeometry.triggerWidth-1,'状态Select宽度受视口限制且不窄于触发器');await writeFile(join(evidence,'r2-select-geometry.json'),JSON.stringify(selectGeometry,null,2));await key('Escape');const focusBefore=await renderer.evaluate('document.activeElement?.getAttribute(\'aria-label\')');await renderer.evaluate(`document.querySelector('[aria-label="记录业务状态"]').focus()`);await key('Enter');assert.ok(await renderer.evaluate("Boolean(document.querySelector('[data-af-popup]'))"),'键盘打开状态Select');await key('Escape');const focusAfter=await renderer.evaluate("document.activeElement?.getAttribute('aria-label')");assert.notEqual(focusBefore,focusAfter);assert.equal(focusAfter,'记录业务状态');checkpoint('E1: 状态Select键盘打开、Escape关闭并实际改变/归还焦点',['r2-detail-normal'])
    await click('','[aria-label="记录业务状态"]');await click('待补充','[role=option]');await click('保存状态');const assigned=await waitFact(async()=>{const value=await api(recordPath);return value.statusId&&value.statusRevision>beforeStatus.statusRevision?value:null},'真实状态写入事实');await waitFor(renderer,"!document.querySelector('button').disabled && Boolean([...document.querySelectorAll('button')].find(e=>e.textContent==='清空状态'&&!e.disabled))",'状态按钮恢复');await click('清空状态');await waitFact(async()=>{const value=await api(recordPath);return value.statusId===null&&value.statusRevision>assigned.statusRevision?value:null},'真实状态清空事实');await waitFor(renderer,"Boolean([...document.querySelectorAll('button')].find(e=>e.textContent==='清空状态'&&!e.disabled))",'清空按钮恢复')
    ...
    await zoom(2);assert.deepEqual(await renderer.evaluate('({w:innerWidth,h:innerHeight,dpr:devicePixelRatio})'),{w:720,h:512,dpr:2});await fits();const focusSignature="(()=>{const e=document.activeElement;return [e?.tagName,e?.getAttribute('aria-label'),e?.textContent?.trim()].join('|')})()",beforeTab=await renderer.evaluate(focusSignature);await key('Tab');assert.notEqual(await renderer.evaluate(focusSignature),beforeTab);await capture('r2-detail-200',{...source,navigation:'keyboard'});checkpoint('E1: 200%为720×512/DPR2、无横向溢出且Tab实际推进焦点',['r2-detail-200']);await zoom(1);await click('返回记录列表');checkpoint('E1/E2/E4: 加载、读错、typed 404均有独立来源证据',['r2-detail-loading','r2-detail-read-error','r2-detail-404']);checkpoint('E1: 非数字前端校验零写请求且焦点回字段；离开确认取消后草稿保留',['r2-edit-validation','r2-edit-leave-confirm']);checkpoint('E1: UI新增→详情→编辑→保存→状态设置/clear→返回完整记录流程',['r2-create-normal','r2-create-dirty','r2-create-saving','r2-detail-normal','r2-edit-normal','r2-edit-dirty'])
