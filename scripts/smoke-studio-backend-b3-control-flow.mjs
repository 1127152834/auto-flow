...
const projectFamilyOnly = projectMathOnly || projectUtilityOnly || projectWebBasicOnly || projectPageLoadOnly || projectAdvancedOnly || projectTabSwitchOnly || projectVariableOnly || projectListExportOnly || projectLogOnly
...
try {
  ...
  if (projectFamilyOnly) {
    const name = projectMathOnly ? '项目列表与数学任务验收' : projectUtilityOnly ? '项目实用工具任务验收' : projectWebBasicOnly ? '项目基础网页任务验收' : projectAdvancedOnly ? '项目高级网页与产物任务验收' : projectTabSwitchOnly ? '项目标签页切换任务验收' : projectVariableOnly ? '项目变量处理任务验收' : projectListExportOnly ? '项目列表导出任务验收' : projectLogOnly ? '项目日志任务验收' : '项目页面加载任务验收'
    ...
    assert.equal(attempts.items.filter(item => item.status === 'succeeded').length, projectListExportOnly || projectLogOnly ? 2 : projectAdvancedOnly || projectTabSwitchOnly ? 6 : projectWebBasicOnly ? 5 : 3)
    ...
    await click(main, projectListExportOnly ? '异常与证据' : projectLogOnly ? '日志' : '输入与输出', '[role="tab"]')
    await waitFor(main, `document.body?.innerText.includes(${JSON.stringify(projectListExportOnly ? '文件与图片产物' : projectLogOnly ? '业务完成' : outputExpectations.at(-1)[0])})`, 'project family output')
    if (projectListExportOnly) assert.equal(await main.evaluate("[...document.querySelectorAll('button')].filter(button => button.getAttribute('aria-label')?.startsWith('下载文件：')).length"), 2)
    if (projectLogOnly) {
