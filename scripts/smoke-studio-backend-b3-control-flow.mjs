...
const projectFamilyOnly = projectMathOnly || projectUtilityOnly || projectWebBasicOnly || projectPageLoadOnly || projectAdvancedOnly || projectTabSwitchOnly || projectVariableOnly || projectListExportOnly || projectLogOnly || projectTableOnly || projectHttpOnly
...
try {
  ...
  if (projectFamilyOnly) {
    ...
    assert.equal(attempts.items.filter(item => item.status === 'succeeded').length, projectTableOnly ? 10 : projectHttpOnly ? 4 : projectListExportOnly || projectLogOnly ? 2 : projectAdvancedOnly || projectTabSwitchOnly ? 6 : projectWebBasicOnly ? 5 : 3)
