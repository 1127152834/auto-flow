...
const evidenceDir = await mkdtemp(join(await mkdir(evidenceRoot, { recursive: true }).then(() => evidenceRoot), 'formal-scheduled-task-electron-'))
const userData = await mkdtemp(join(tmpdir(), 'autoflow-studio-b6-scheduled-task-'))
const workflowName = 'B6 定时任务正式闭环'
...
try {
  ...
  const timerId = await addNode(studio, 0, '定时任务', 'scheduled_task')
  await setInput(studio, 'input[placeholder="YYYY-MM-DD，如 2026-01-01"]', '2000-01-01')
  await setInput(studio, 'input[placeholder="HH:MM 或 HH:MM:SS，如 09:30"]', '00:00')

  const printId = await addNode(studio, 1, '打印日志', 'print_log')
  await setInput(studio, 'input[placeholder="要打印的日志信息"]', '定时任务已完成')
  await connectNodes(studio, timerId, printId)
  await waitFor(studio, "document.querySelectorAll('.react-flow__edge').length === 1", 'workflow edge')
  checkpoint('通过正式 Studio 真实 UI 添加并配置 过期日期定时任务和后续日志节点')
  ...
  assert.ok(saved)
  assert.deepEqual(saved.nodes.map(node => node.data.moduleType), ['scheduled_task', 'print_log'])
  assert.equal(saved.edges.length, 1)
  checkpoint('正式保存接口把定时节点、日志节点和字段配置写入临时 SQLite')
  ...
  assert.ok(byNode[timerId] === undefined || JSON.stringify(byNode[timerId]) === '{}')
  assert.deepEqual(byNode[printId], { level: 'info', message: '定时任务已完成' })

  const logs = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}/logs?cursor=0&limit=50`)
  assert.ok(logs.items.some(item => item.nodeId === timerId && item.message.includes('已过，立即执行')))
  assert.ok(logs.items.some(item => item.nodeId === printId && item.message === '定时任务已完成'))
  checkpoint('真实 worker 识别过期目标时间并继续执行，定时日志和后续节点结果均核验')
  ...
  const report = {
    evidenceId: 'BE-B6-scheduled-task-formal-electron', checkedAt: new Date().toISOString(),
    ...
    nodes: [
      { moduleType: 'scheduled_task', nodeId: timerId, result: byNode[timerId] },
    ...
    boundaries: {
      ...
      schedule: 'fixed past local date/time in an ephemeral workflow; no external scheduler',
      ...
      platformPending: 'long-delay cancellation and host sleep/timezone behavior remain separately verified by backend tests/platform runs',
    },
  }
