...
const evidenceDir = await mkdtemp(join(await mkdir(evidenceRoot, { recursive: true }).then(() => evidenceRoot), 'formal-scripts-electron-'))
const userData = await mkdtemp(join(tmpdir(), 'autoflow-studio-b6-scripts-'))
const workflowName = 'B6 脚本执行正式闭环'
const checks = []
...
try {
  ...
  const jsId = await addNode(studio, 0, 'JS脚本', 'js_script')
  await click(studio, '打开代码编辑器', 'div')
  await waitFor(studio, "document.body?.innerText.includes('JavaScript 代码编辑器')", 'JavaScript editor')
  await click(studio, '保存', 'div.fixed button')
  await waitFor(studio, "!document.body?.innerText.includes('JavaScript 代码编辑器')", 'JavaScript editor close')
  await setInput(studio, 'input[placeholder="变量名"]', 'js_result')

  const pythonId = await addNode(studio, 1, 'Python脚本', 'python_script')
  await click(studio, '打开代码编辑器', 'div')
  await waitFor(studio, "document.body?.innerText.includes('Python 代码编辑器')", 'Python editor')
  await click(studio, '保存', 'div.fixed button')
  await waitFor(studio, "!document.body?.innerText.includes('Python 代码编辑器')", 'Python editor close')
  await setInput(studio, 'input[placeholder="result"]', 'py_result')
  await setInput(studio, 'input[placeholder="stdout"]', 'py_stdout')

  const printId = await addNode(studio, 2, '打印日志', 'print_log')
  await setInput(studio, 'input[placeholder="要打印的日志信息"]', 'Python脚本已完成')

  await connectNodes(studio, jsId, pythonId)
  await connectNodes(studio, pythonId, printId)
  await waitFor(studio, "document.querySelectorAll('.react-flow__edge').length === 2", 'workflow edges')
  checkpoint('通过正式 Studio 真实 UI 添加并配置 JS、Python 和日志节点，保存默认脚本内容')
  ...
  assert.ok(saved)
  assert.deepEqual(saved.nodes.map(node => node.data.moduleType), ['js_script', 'python_script', 'print_log'])
  assert.equal(saved.edges.length, 2)
  checkpoint('正式保存接口把脚本节点、日志节点和字段配置写入临时 SQLite')
  ...
  assert.equal(byNode[jsId].result, null)
  assert.equal(byNode[pythonId].result.status, 'success')
  assert.equal(byNode[pythonId].result.result, 2)
  assert.ok(typeof byNode[pythonId].result.stdout === 'string' || byNode[pythonId].result.stdout === undefined)
  assert.deepEqual(byNode[printId], { level: 'info', message: 'Python脚本已完成' })

  const logs = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}/logs?cursor=0&limit=80`)
  assert.ok(logs.items.some(item => item.nodeId === jsId && item.message.includes('JS脚本执行成功')))
  assert.ok(logs.items.some(item => item.nodeId === pythonId && item.message.includes('脚本执行成功')))
  assert.ok(logs.items.some(item => item.nodeId === printId && item.message === 'Python脚本已完成'))
  checkpoint('真实 worker 执行浏览器 JS 与 Python 脚本，变量/返回值和后续日志均核验')
  ...
  const report = {
    evidenceId: 'BE-B6-scripts-formal-electron', checkedAt: new Date().toISOString(),
    ...
    nodes: [
      { moduleType: 'js_script', nodeId: jsId, result: byNode[jsId] },
      { moduleType: 'python_script', nodeId: pythonId, result: byNode[pythonId] },
    ...
    boundaries: {
      ...
      script: 'default WebRPA-compatible JS/Python editor contents; no user input or external file',
      ...
      platformPending: 'Windows packaged Python path and browser script platform differences remain separately unverified',
    },
  }
