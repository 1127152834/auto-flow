...
const workspace=await realpath(await mkdtemp(join(tmpdir(),'autoflow-m4-studio-')))
const target=await realpath(await mkdtemp(join(tmpdir(),'autoflow-m4-target-')))
...
const qa=join(root,'docs/migration/automation-studio-m4-qa');await mkdir(qa,{recursive:true})
...
try {
 ...
 const profile=await api('profiles',{method:'POST',body:JSON.stringify({name:'M4 浏览器验收',browserVersion:kernel.version,headless:true})})
 ...
 await input(studio,'#workflow-name','M4 正式画布配对与循环')
 ...
 await click(studio,'','新建');await click(studio,'','打开');await waitFor(studio,`(()=>{const e=[...document.querySelectorAll('[role=dialog] button')].find(e=>e.textContent.includes('M4 正式画布配对与循环'));if(e)e.id='m4-open-flow';return Boolean(e)})()`,'flow list');await click(studio,'#m4-open-flow')
 ...
 const run=await until(async()=>{const list=await api('workflows/runs');if(!list.items.length)return;const run=await api('workflows/runs/'+list.items[0].runId);return ['succeeded','failed'].includes(run.state)?run:null},'formal real control run',60000)
 assert.equal(run.state,'succeeded',JSON.stringify(run.error));assert.equal(run.artifactCount,2)
 ...
 await waitFor(studio,`document.body.innerText.includes('执行 2 次')`,'repeat canvas markers')
 await capture('executed');passed('saved document reopens and independent browser executes condition/loop/iframe twice; canvas shows repeated execution')
 await click(studio,'','只读结果 2');await waitFor(studio,`document.querySelectorAll('[aria-label="运行产物列表"] article').length===2`,'both artifacts');await capture('results')
 await click(studio,'','运行日志');await waitFor(studio,`document.body.innerText.includes('第 2 轮')`,'iteration log')
 passed('formal results and logs retain separate execution IDs and loop iterations')
 ...
 await writeFile(join(qa,`${mode}-studio.json`),JSON.stringify({platform:process.platform,arch:process.arch,mode,kernelVersion:kernel.version,checks,complete:checks.length===3},null,2))
