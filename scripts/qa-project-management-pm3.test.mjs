...
const script = fileURLToPath(new URL('./qa-project-management-pm3.mjs', import.meta.url))

test('self-test accepts only a marker-owned workspace below its owner', async () => {
  const { stdout } = await exec(process.execPath, [script, '--self-test'])
...
test('arguments reject missing kernel paths before launching Electron', async () => {
  await assert.rejects(
    exec(process.execPath, [script, '--kernel-directory']),
...
test('arguments reject unknown switches before launching Electron', async () => {
  await assert.rejects(
    exec(process.execPath, [script, '--write-main-workspace']),
...
test('scenario selection is validated before any workspace or Electron launch', async () => {
  const { stdout } = await exec(process.execPath, [script, '--self-test', '--scenario', 'stop'])
  assert.match(stdout, /qa helper self-test passed/)
  await assert.rejects(
    exec(process.execPath, [script, '--scenario', 'invented']),
