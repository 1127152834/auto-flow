...
const evidenceDir = await mkdtemp(join(await mkdir(evidenceRoot, { recursive: true }).then(() => evidenceRoot), 'formal-webhook-trigger-electron-'))
const userData = await mkdtemp(join(tmpdir(), 'autoflow-studio-b6-webhook-trigger-'))
const workflowName = 'B6 Webhook 触发正式闭环'
...
try {
  ...
  const webhookId = await addNode(studio, 0, 'Webhook触发器', 'webhook_trigger')
  const webhookKey = await waitForValue(async () => await studio.evaluate("document.querySelector('#webhookId')?.value || null"), 'generated webhook id', 10_000)
  assert.equal(typeof webhookKey, 'string')
  const printId = await addNode(studio, 1, '打印日志', 'print_log')
  await setInput(studio, 'input[placeholder="要打印的日志信息"]', 'Webhook已触发')
  await connectNodes(studio, webhookId, printId)
  await waitFor(studio, "document.querySelectorAll('.react-flow__edge').length === 1", 'workflow edge')
  checkpoint('通过正式 Studio 真实 UI 添加 Webhook 触发器、读取生成 ID 并连接后续日志节点')
  ...
  assert.ok(saved)
  assert.deepEqual(saved.nodes.map(node => node.data.moduleType), ['webhook_trigger', 'print_log'])
  assert.equal(saved.edges.length, 1)
  const savedWebhookId = saved.nodes.find(node => node.id === webhookId).data.webhookId
  assert.equal(savedWebhookId, webhookKey)
  checkpoint('正式保存接口把 Webhook ID、后续日志节点和连线写入临时 SQLite')

  await click(studio, '运行 (F5)', '[aria-label="运行 (F5)"]')
  await click(studio, '运行 (F5)', '[role="menuitem"]')
  const run = await waitForValue(async () => (await api(runtime, `/workflow-runs?documentId=${encodeURIComponent(saved.id)}&cursor=0&limit=20`)).items[0], 'webhook run creation', 20_000)
  await wait(500)
  let webhookResponse
  for (let attempt = 0; attempt < 100; attempt++) {
    webhookResponse = await fetch(`${runtime.sidecar.baseUrl}/api/triggers/webhook/${encodeURIComponent(savedWebhookId)}`, { method: 'POST', headers: { 'content-type': 'application/json', 'x-formal-test': 'b6', 'Idempotency-Key': `formal-webhook-${run.runId}` }, body: JSON.stringify({ marker: 'formal-webhook' }) })
    if (webhookResponse.status !== 404) break
    await wait(200)
  }
  const webhookBody = await webhookResponse.json()
  assert.equal(webhookResponse.status, 200)
  assert.deepEqual(webhookBody, { success: true })
  const terminal = await waitForValue(async () => { const value = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}`); return ['completed','failed','stopped','interrupted'].includes(value.status) ? value : null }, 'webhook terminal run', 60_000)
  ...
  assert.deepEqual(byNode[printId], { level: 'info', message: 'Webhook已触发' })
  const logs = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}/logs?cursor=0&limit=50`)
  assert.ok(logs.items.some(item => item.nodeId === webhookId && item.message.includes('Webhook已触发')))
  assert.ok(logs.items.some(item => item.nodeId === printId && item.message === 'Webhook已触发'))
  checkpoint('外部本地 HTTP 客户端真实 POST Webhook，返回 200，worker 收到后继续日志节点并完成')
  ...
  const report = {
    evidenceId: 'BE-B6-webhook-trigger-formal-electron', checkedAt: new Date().toISOString(),
    ...
    nodes: [
      { moduleType: 'webhook_trigger', nodeId: webhookId, webhookId: savedWebhookId },
    ...
    boundaries: {
      ...
      webhook: 'local loopback POST with a fixed marker; no external network',
      ...
      platformPending: 'external firewall and packaged-entry reachability remain separately unverified',
    },
  }
