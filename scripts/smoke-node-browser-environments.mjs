...
try{
 ...
 const document=await api('/workflows',{id:randomUUID(),clientRequestId:randomUUID(),name:'节点环境验收',schemaVersion:3,browserEnvironmentVersion:1,variables:[],nodes:[{id:'open',type:'open_page',position:{x:100,y:100},data:{moduleType:'open_page',label:'初始化实例',url:'about:blank',browserEnvironment:{source:'profile'}}},{id:'again',type:'open_page',position:{x:100,y:260},data:{moduleType:'open_page',label:'共用浏览器',url:'about:blank',browserEnvironment:{source:'profile',profileId:profile.id,proxy:{mode:'none'},kernel:{edition:'public',version}}}}],edges:[{id:'next',source:'open',target:'again'}]})
 ...
 assert.equal(await studio.evaluate("Boolean(document.querySelector('#browser-environment-source'))"),false)
 ...
 assert.deepEqual(saved.nodes[0].data.browserEnvironment,{source:'profile',profileId:profile.id,proxy:{mode:'none'},kernel:{edition:'public',version}})
 ...
 checks.push('UI run completes through real sidecar and worker with two matching nodes sharing a browser; template unchanged')
