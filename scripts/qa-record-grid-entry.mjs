...
async function key(key,modifiers=0){const code=key.length===1?`Key${key.toUpperCase()}`:key;const vk=key.length===1?key.toUpperCase().charCodeAt(0):({Enter:13,Tab:9,Escape:27}[key]);await renderer.command('Input.dispatchKeyEvent',{type:'keyDown',key,code,windowsVirtualKeyCode:vk,modifiers,...(key==='v'&&modifiers?{commands:['paste']}: {})});await renderer.command('Input.dispatchKeyEvent',{type:'keyUp',key,code,windowsVirtualKeyCode:vk,modifiers});await wait(80)}
...
try{
 ...
 for(const [name,keyName] of [['标题','title'],['文章链接','url'],['摘要','summary']]){
  await click('新建字段');await input('#field-name',name);await input('#field-key',keyName)
  if(name==='标题')await click('必填','label')
  await click('创建字段');await waitFor(renderer,"!document.querySelector('[role=dialog]')",'field saved')
 }
 ...
 await click('编辑');await visible('保存修改');await input('[aria-label="标题"]','已有记录仍可编辑');await click('保存修改');await visible('返回记录列表');await click('返回记录列表');await visible('已有记录仍可编辑');passed('E16-edit','Saved record edit still uses existing detail/edit flow and persists through UI')
