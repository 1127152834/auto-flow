...
const evidenceDir = await mkdtemp(join(await mkdir(evidenceRoot, { recursive: true }).then(() => evidenceRoot), 'formal-probability-electron-'))
const userData = await mkdtemp(join(tmpdir(), 'autoflow-studio-b6-probability-'))
const workflowName = 'B6 概率触发正式闭环'
...
try {
  ...
  const probabilityId = await addNode(studio, 0, '概率触发器', 'probability_trigger')
  await setProbabilityInputAt(studio, '50', '100')

  const path1Id = await addNode(studio, 1, '打印日志', 'print_log')
  await setInput(studio, 'input[placeholder="要打印的日志信息"]', '路径1已执行')
  const path2Id = await addNode(studio, 2, '打印日志', 'print_log')
  await setInput(studio, 'input[placeholder="要打印的日志信息"]', '路径2已执行')
  await connectNodesHandle(studio, probabilityId, path1Id, 'path1')
  await connectNodesHandle(studio, probabilityId, path2Id, 'path2')
  await waitFor(studio, "document.querySelectorAll('.react-flow__edge').length === 2", 'probability branch edges')
  checkpoint('通过正式 Studio 真实 UI 添加概率触发器、两条路径及分支连线')
  ...
  assert.ok(saved)
  assert.deepEqual(saved.nodes.map(node => node.data.moduleType), ['probability_trigger', 'print_log', 'print_log'])
  assert.equal(saved.edges.length, 2)
  checkpoint('正式保存接口把概率触发器、两条路径和字段配置写入临时 SQLite')

  await click(studio, '运行 (F5)', '[aria-label="运行 (F5)"]')
  await click(studio, '运行 (F5)', '[role="menuitem"]')
  const run1 = await waitForValue(async () => (await api(runtime, `/workflow-runs?documentId=${encodeURIComponent(saved.id)}&cursor=0&limit=20`)).items[0], 'first run creation', 20_000)
  const terminal1 = await waitForValue(async () => { const value = await api(runtime, `/workflow-runs/${encodeURIComponent(run1.runId)}`); return ['completed','failed','stopped','interrupted'].includes(value.status) ? value : null }, 'first terminal run', 60_000)
  assert.equal(terminal1.status, 'completed')
  const logs1 = await api(runtime, `/workflow-runs/${encodeURIComponent(run1.runId)}/logs?cursor=0&limit=50`)
  assert.ok(logs1.items.some(item => item.nodeId === path1Id && item.message === '路径1已执行'))
  assert.equal(logs1.items.some(item => item.nodeId === path2Id && item.message === '路径2已执行'), false)

  await selectNode(studio, probabilityId, 'probability_trigger')
  await setProbabilityInputAt(studio, '100', '0')
  await click(studio, '保存')
  await waitForValue(async () => { const doc = await api(runtime, `/workflows/${encodeURIComponent(saved.id)}`); return doc.nodes.some(node => node.id === probabilityId && Number(node.data?.probability) === 0) ? doc : null }, 'probability update save', 20_000)
  await click(studio, '运行 (F5)', '[aria-label="运行 (F5)"]')
  await click(studio, '运行 (F5)', '[role="menuitem"]')
  const run2 = await waitForValue(async () => { const items=(await api(runtime, `/workflow-runs?documentId=${encodeURIComponent(saved.id)}&cursor=0&limit=20`)).items; return items.find(item=>item.runId!==run1.runId) }, 'second run creation', 20_000)
  const terminal2 = await waitForValue(async () => { const value = await api(runtime, `/workflow-runs/${encodeURIComponent(run2.runId)}`); return ['completed','failed','stopped','interrupted'].includes(value.status) ? value : null }, 'second terminal run', 60_000)
  assert.equal(terminal2.status, 'completed')
  const logs2 = await api(runtime, `/workflow-runs/${encodeURIComponent(run2.runId)}/logs?cursor=0&limit=50`)
  assert.ok(logs2.items.some(item => item.nodeId === path2Id && item.message === '路径2已执行'))
  assert.equal(logs2.items.some(item => item.nodeId === path1Id && item.message === '路径1已执行'), false)
  checkpoint('真实 worker 分别以 100% 和 0% 配置运行，未选分支无副作用，两个运行均完成')
  ...
  const report = {
    evidenceId: 'BE-B6-probability-formal-electron', checkedAt: new Date().toISOString(),
    gitHead: execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim(),
    result: 'passed', platform: `${process.platform}-${process.arch}`, entry: 'development-build',
    workflowId: saved.id, profileId: profile.id, runIds: [run1.runId, run2.runId], checks,
    nodes: [
      { moduleType: 'probability_trigger', nodeId: probabilityId },
      { moduleType: 'print_log', nodeId: path1Id },
      { moduleType: 'print_log', nodeId: path2Id },
    ],
    buildSha256: await buildHash(),
    boundaries: {
      workspace: 'ephemeral', userDatabaseTouched: false, browserLaunch: 'none',
      probability: 'two real runs at 100% and 0%; no external data',
      ...
      platformPending: 'random 50% distribution and long-running cancellation remain covered by backend tests',
    },
  }
...
async function setProbabilityInputAt(cdp, currentValue, value) {
  const p = await waitFor(cdp, `(()=>{const e=[...document.querySelectorAll('input')].find(e=>e.getClientRects().length&&e.value===${JSON.stringify(currentValue)});if(!e)return null;e.scrollIntoView({block:'center',behavior:'instant'});const r=e.getBoundingClientRect();return{x:r.x+r.width/2,y:r.y+r.height/2}})()`, `probability input ${currentValue}`)
...
async function connectNodesHandle(cdp, sourceId, targetId, handleId) {
  const points = await waitFor(cdp, `(()=>{const a=document.querySelector('.react-flow__node[data-id=${JSON.stringify(sourceId)}] .react-flow__handle.source[data-handleid=${JSON.stringify(handleId)}]'),b=document.querySelector('.react-flow__node[data-id=${JSON.stringify(targetId)}] .react-flow__handle.target');if(!a||!b)return null;const ar=a.getBoundingClientRect(),br=b.getBoundingClientRect();return{a:{x:ar.x+ar.width/2,y:ar.y+ar.height/2},b:{x:br.x+br.width/2,y:br.y+br.height/2}}})()`, `handles ${sourceId} ${handleId} -> ${targetId}`)
