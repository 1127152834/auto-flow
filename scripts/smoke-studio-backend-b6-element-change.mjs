...
const evidenceDir = await mkdtemp(join(await mkdir(evidenceRoot, { recursive: true }).then(() => evidenceRoot), 'formal-element-change-electron-'))
const userData = await mkdtemp(join(tmpdir(), 'autoflow-studio-b6-element-change-'))
const fixturePath = join(userData, 'element-change.html')
const workflowName = 'B6 元素变化触发器正式闭环'
const checks = []
...
try {
  ...
  const profile = await api(runtime, '/v1/profiles', { method: 'POST', body: {
    name: 'B6 元素变化验收配置', description: '临时工作区；元素变化触发器正式验收', startUrl: 'about:blank', locale: 'zh-CN', timezone: 'Asia/Shanghai', geoip: false,
  ...
  await writeFile(fixturePath, `<!doctype html><meta charset="utf-8"><div id="list"><span>初始</span></div><script>setTimeout(() => { const item = document.createElement('p'); item.id = 'added'; item.textContent = '新增内容'; document.querySelector('#list').appendChild(item); }, 800)</script>`, 'utf8')
  const openId = await addNode(studio, '打开网页', 'open_page')
  await setInput(studio, 'input[placeholder="https://example.com"]', `file://${fixturePath}`)
  const triggerId = await addNode(studio, '元素变化触发器', 'element_change_trigger')
  await setInput(studio, 'input[placeholder="如: .comment-list 或 #messages"]', '#list')
  await setInput(studio, 'input[placeholder="new_element_selector"]', 'new_selector')
  await setInput(studio, 'input[placeholder="element_change_info"]', 'change_info')
  const printId = await addNode(studio, '打印日志', 'print_log')
  await setInput(studio, 'input[placeholder="要打印的日志信息"]', '元素变化已触发')
  await connectNodes(studio, openId, triggerId); await connectNodes(studio, triggerId, printId)
  await waitFor(studio, "document.querySelectorAll('.react-flow__edge').length === 2", 'workflow edges')
  checkpoint('通过正式 Studio 真实 UI 配置本地网页、元素选择器、输出变量并连接后续日志节点')
  ...
  checkpoint('正式保存接口写入网页地址、元素变化触发器、日志节点和两条连线')

  await click(studio, '运行 (F5)', '[aria-label="运行 (F5)"]'); await click(studio, '运行 (F5)', '[role="menuitem"]')
  const run = await waitForValue(async () => (await api(runtime, `/workflow-runs?documentId=${encodeURIComponent(saved.id)}&cursor=0&limit=20`)).items[0], 'element change run creation', 20_000)
  const terminal = await waitForValue(async () => { const value = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}`); return ['completed', 'failed', 'stopped', 'interrupted'].includes(value.status) ? value : null }, 'element change terminal run', 120_000)
  ...
  assert.equal(byNode[triggerId].newElementSelector, '#added'); assert.equal(byNode[triggerId].addedCount, 1); assert.equal(byNode[triggerId].newElementText, '新增内容')
  assert.deepEqual(byNode[printId], { level: 'info', message: '元素变化已触发' })
  const logs = await api(runtime, `/workflow-runs/${encodeURIComponent(run.runId)}/logs?cursor=0&limit=100`); assert.ok(logs.items.some(item => item.nodeId === triggerId && item.message.includes('子元素变化触发器已触发'))); assert.ok(logs.items.some(item => item.nodeId === printId && item.message === '元素变化已触发'))
  checkpoint('真实 CloakBrowser 加载本地网页并观察 MutationObserver 变化，写入新元素选择器后执行日志节点')

  await wait(500); assert.deepEqual(cloakProcesses(userData), []); checkpoint('元素变化流程完成后 CloakBrowser、worker 和资源均清理')
  await capture(studio, join(evidenceDir, 'completed.png'))
  const report = {
    evidenceId: 'BE-B6-element-change-formal-electron', checkedAt: new Date().toISOString(),
    ...
    nodes: [{ moduleType: 'open_page', nodeId: openId }, { moduleType: 'element_change_trigger', nodeId: triggerId, result: byNode[triggerId] }, { moduleType: 'print_log', nodeId: printId, result: byNode[printId] }],
    boundaries: { workspace: 'ephemeral', userDatabaseTouched: false, browserLaunch: 'real CloakBrowser kernel', fixture: 'local file origin with delayed DOM mutation', interaction: 'formal Electron via CDP mouse and keyboard; public sidecar APIs only for fixture setup and evidence reads; no Store or page-internal business function access', platformPending: 'Windows/macOS Intel and packaged-entry reachability remain separately unverified' },
