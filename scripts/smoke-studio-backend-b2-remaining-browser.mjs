...
const evidenceDir = await mkdtemp(join(evidenceRoot, 'formal-remaining-browser-electron-'))
const userData = await mkdtemp(join(tmpdir(), 'autoflow-studio-b2-remaining-browser-'))
const fixture = await readFile(join(root, 'apps/backend/tests/fixtures/workflow-b2-remaining-browser.html'))
const observedRequests = []
const server = createServer((request, response) => {
  observedRequests.push(request.url)
  let body, contentType
  if (request.url.startsWith('/api/remaining')) {
    body = Buffer.from('{"ok":true}'); contentType = 'application/json'
  } else if (request.url === '/data') {
    body = fixture; contentType = 'text/html; charset=utf-8'
  } else {
    body = Buffer.from('<!doctype html><title>AutoFlow B2 home</title><p>home</p>'); contentType = 'text/html; charset=utf-8'
  }
...
let desktop, main, studio, native, eventAbort
...
try {
  ...
  const profile = await api(runtime, '/v1/profiles', {
    method: 'POST', body: {
      name: 'B2 其余网页节点正式验收配置', description: '隔离工作区中的 CloakBrowser 配置', startUrl: 'about:blank',
  ...
  const workflowName = 'B2 其余网页节点正式闭环'
  const modules = [
    { label: '打开网页', type: 'open_page', inputs: [['[placeholder="https://example.com"]', `${origin}/home`]], selects: [['#openMode', '当前标签页']] },
    { label: '打开网页', type: 'open_page', inputs: [['[placeholder="https://example.com"]', `${origin}/data`]], selects: [['#openMode', '新标签页']] },
    { label: '切换标签页', type: 'switch_tab', inputs: [['[placeholder="tab_title"]', 'first_title']], selects: [['#switchMode', '切换到第一个']] },
    { label: '切换标签页', type: 'switch_tab', inputs: [['[placeholder="输入标签页标题"]', 'AutoFlow B2 数据页'], ['[placeholder="tab_index"]', 'data_index'], ['[placeholder="tab_title"]', 'data_title'], ['[placeholder="tab_url"]', 'data_url']], selects: [['#switchMode', '按标题切换'], ['#matchMode', '精确匹配']] },
    { label: '表格数据提取', type: 'extract_table_data', inputs: [['[placeholder="例如: table, #dataTable, .data-grid"]', '#orders-table'], ['[placeholder="table_data"]', 'table_rows']], checkbox: '#exportToExcel' },
    { label: '网络抓包', type: 'network_capture', inputs: [['[placeholder="模糊匹配URL"]', '/api/remaining'], ['#captureDuration', '1'], ['[placeholder="存储捕获结果的变量名"]', 'captured_urls']], selects: [['#captureMode', '浏览器抓包'], ['#filterType', '全部请求']] },
  ]
  const familyTypes = ['switch_tab', 'extract_table_data', 'network_capture']
  ...
  for (let index = 0; index < modules.length; index++) {
    ...
    if (module.checkbox) { await click(studio, '', module.checkbox); await waitFor(studio, `document.querySelector(${JSON.stringify(module.checkbox)})?.getAttribute('aria-checked') === 'true'`, `${module.type} checkbox`) }
    await moveNode(studio, node, index, modules.length)
  }
  assert.equal(new Set(nodeIds).size, modules.length)
  checkpoint('通过正式 Studio UI 添加并配置切换标签页、提取表格和浏览器抓包节点')
  ...
  checkpoint('通过画布连接手柄建立双标签切换、表格提取和抓包顺序链')
  ...
  assert.ok(saved)
  assert.equal(saved.nodes.length, modules.length); assert.equal(saved.edges.length, modules.length - 1)
  assert.equal(saved.nodes.find(node => node.id === nodeIds[3])?.data.tabTitle, 'AutoFlow B2 数据页')
  assert.equal(saved.nodes.find(node => node.id === nodeIds[4])?.data.exportToExcel, true)
  assert.equal(saved.nodes.find(node => node.id === nodeIds[5])?.data.captureMode ?? 'browser', 'browser')
  checkpoint('真实 HTTP/SQLite 保存标签匹配、表格Excel导出和浏览器抓包配置')

  await wait(300); await closeWindowNormally()
  ...
  await waitForNoStudio(desktop.debugOrigin); studio.close(); studio = undefined
  studio = await openStudioFromMain(main, desktop.debugOrigin)
  await studio.command('Emulation.setDeviceMetricsOverride', { width: 3000, height: 1800, deviceScaleFactor: 1, mobile: false })
  ...
  await waitFor(studio, `document.querySelectorAll('.react-flow__node').length === ${modules.length} && document.querySelectorAll('.react-flow__edge').length === ${modules.length - 1}`, 'persisted remaining browser workflow reopen')
  ...
  const startedRun = await waitForValue(async () => (await api(runtime, `/workflow-runs?documentId=${encodeURIComponent(saved.id)}&cursor=0&limit=20`)).items[0] ?? null, 'persisted workflow run', 20_000)
  ...
  const terminalRun = await waitForValue(async () => { const value = await api(runtime, `/workflow-runs/${encodeURIComponent(startedRun.runId)}`); return ['completed', 'failed', 'stopped', 'interrupted'].includes(value.status) ? value : null }, 'workflow terminal persistence', 120_000)
  ...
  assert.equal(resultFor(2)?.index, 0); assert.equal(resultFor(2)?.title, 'AutoFlow B2 home')
  assert.equal(resultFor(3)?.index, 1); assert.equal(resultFor(3)?.title, 'AutoFlow B2 数据页')
  assert.deepEqual(resultFor(4)?.tableData, [['订单', '金额'], ['A-001', '88'], ['A-002', '99']])
  assert.ok(resultFor(5)?.count > 0); assert.ok(resultFor(5)?.urls.every(url => url.includes('/api/remaining')))
  assert.equal(JSON.stringify(resultFor(5)).includes('fixture-secret'), false)
  ...
  const workbookArtifact = artifacts.items[0]
  assert.equal(workbookArtifact.nodeId, nodeIds[4])
  const workbook = await apiBytes(runtime, `/workflow-runs/${encodeURIComponent(runId)}/artifacts/${encodeURIComponent(workbookArtifact.artifactId)}`)
  assert.equal(workbook.subarray(0, 2).toString('utf8'), 'PK')
  assert.equal(createHash('sha256').update(workbook).digest('hex'), workbookArtifact.sha256)
  assert.ok(observedRequests.some(path => path.startsWith('/api/remaining?token=fixture-secret')))
  checkpoint('真实 CloakBrowser 完成标签选择、表格提取和浏览器抓包；Excel产物哈希、日志、结果及秘密脱敏通过')

  await waitForValue(async () => cloakProcesses(userData).length === 0 ? true : null, 'CloakBrowser process cleanup', 15_000)
  checkpoint('运行终态后CloakBrowser进程树和抓包监听资源释放')
  await capture(studio, join(evidenceDir, 'completed.png'))
  const report = {
    evidenceId: 'BE-B2-remaining-browser-formal-electron', checkedAt: new Date().toISOString(), gitHead: execFileSync('git', ['rev-parse', 'HEAD'], { cwd: root, encoding: 'utf8' }).trim(),
    buildSha256: await buildHash(), workflowId: saved.id, profileId: profile.id, runId, result: 'passed', checks, platform: `${process.platform}-${process.arch}`, entry: 'development-build',
    remainingBrowser: { familyTypes, moduleTypes: modules.map(module => module.type), nodeIds, resultCount: results.items.length, logCount: logs.items.length, artifacts: artifacts.items, workbookSha256: workbookArtifact.sha256, observedRequestCount: observedRequests.length, cloakBrowserProcessesObserved: sawCloakBrowser },
