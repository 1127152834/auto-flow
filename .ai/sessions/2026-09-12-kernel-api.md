...
- License 退出和授权下载使用同一 dataDir 下的跨进程 `.license.lock`。授权下载从读取凭据到 queued operation 完成登记一直持锁；退出在同一锁内检查活动授权任务并删除凭据，保证退出成功后不会再有携带旧 key 的任务启动。授权下载争用该门禁映射为 `KERNEL_BUSY`，退出争用保持 `LICENSE_IN_USE`。锁顺序只有 License 门禁到安装门禁，不存在反向嵌套。
...
- `uv run --directory apps/backend pytest -q`：231 passed。
