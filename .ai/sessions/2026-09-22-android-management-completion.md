...
- 追加代码提交：`c60148fb`；新增镜像服务端核验/删除核实、批量失败项 lineage、备份归档路径安全、诊断快照脱敏、容量未知 fail-closed 和会话结束栅栏。
- 已提交：基线/夹具、诊断契约、生命周期安全、operation 迁移、会话 heartbeat、镜像/模板/备份/容量/脱敏基础。
- 验证：Android 后端聚焦 235 passed（含未知结果核实、应用操作未知结果保护、备份公开 DTO、批量/清理 workspace fence 与幂等、批量 retryOf lineage、服务端镜像元数据和精确 digest 删除核实、启动恢复隔离和真实工作区归属回归）；Android 前端 81 passed（13 files，含迟到控制会话响应丢弃、批量失败项重试、镜像删除未知结果核实和会话结束前停止 heartbeat）；typecheck、lint、OpenAPI check、structure、build 通过；smoke 参数保护 3 passed；全脚本 95 tests 中 92 passed、3 个既有 Studio failures；全后端 `276 passed, 1 warning` 后在既有 proxy runtime password-schema 断言失败，不在 Android 范围。
...
- 最终 guarded smoke：隔离 workspace 返回 `status=passed`，应用清单 `102/102`、截图 `553476` bytes、停止/启动/移除运行环境/清理数据完成；workspace 标签查询无残留容器和卷，Lima 随后停止。
