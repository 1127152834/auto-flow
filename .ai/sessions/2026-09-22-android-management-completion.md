...
- 验证：Android 后端聚焦 101 passed（含真实输出解析、卷 copy、备份完整性/镜像归属、镜像内容删除适配、观察器退避、按工作区清理保留数据/备份和逐项环境检查回归）；Android 前端 24 passed（10 files）；typecheck、lint、OpenAPI check、structure、build 通过；smoke 参数保护 2 passed；全脚本 95 tests 中 92 passed、3 个既有 Studio failures；全后端首个失败为缺少未安装的 PyInstaller。
