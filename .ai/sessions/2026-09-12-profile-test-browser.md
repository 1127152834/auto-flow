...
## 后续桌面 404 排查（2026-09-12，confirmed）

用户报告当前桌面点击测试启动返回 404。实际窗口已显示新版卡片，但其运行中 Python sidecar 的 OpenAPI 缺少 `/api/v1/profiles/{profile_id}/test-browser`。通过设置页“重启服务”后，新进程 OpenAPI 包含该路由，确认是前端热更新与旧后端进程并存；仅刷新 renderer 不能加载 Python 新接口。

重启后实际点击暴露代理缓存过期：既有仓储在距同步超过 5 分钟后计算 projection.stale，credential loader 因此拒绝加载。通过代理页“刷新代理”同步后，保留用户配置及固定代理选择，再点击成功打开真实 CloakBrowser。进程验证保存的 seed 已应用、使用临时数据目录、代理通过本地 relay、实际为有头窗口；原生 UI 确认 about:blank 窗口存在并留给用户查看。没有修改业务代码或用户配置。五分钟缓存策略仍沿用既有实现，后续再次过期时需要刷新代理。
