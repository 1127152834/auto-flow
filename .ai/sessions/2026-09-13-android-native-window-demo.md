...
- 后续用户要求继续：复查固定版 SDL/scrcpy 源码发现前台点击沿用 SDL 已记录鼠标位置，形成“合成点击未更新移动位置”的 proposed 假设；尚未抓取事件序列证明。再次通过页面打开真实实例后，暂停 CUA 输入，已请求用户做物理鼠标/触控板的首次、重连、移动缩放对照；结果 pending。不修改坐标、不宣称修复，详见验证记录后续输入定位章节。
