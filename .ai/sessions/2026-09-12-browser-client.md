...
共享客户端保留代理模块的 204 与 snake_case `ApiError`，同时消费浏览器接口独立生成的 camelCase `BrowserApiError`。普通请求超时覆盖响应正文解析；内核 provider RPC 相关读取使用 60 秒预算。SSE 使用认证 fetch、仅建连超时和调用方 AbortSignal，按 1/2/5 秒退避重连，每次依赖服务端首帧全量快照恢复，不重放写命令；终态只在状态首次转入 completed/cancelled/failed 时通知。
