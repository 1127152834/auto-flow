...
- 日期：2026-09-12
- 状态：confirmed；用户批准后代码与自动/只读验收已完成，实网写入待指定允许中断的代理。
...
- 待解决：当前代理写入前置条件、非空计划与命令最终响应；不能把 not_bound 自动等价为暂停、旧产品或永久不支持。operation 持久化执行与轮询已补齐，详见 docs/migration/proxy-remote-controls-verification.md。

- 地点实读：243 城市记录归并为 125 目标（城市别名共享 ID）；容量不累加。周期采用官网可见 5/10/30/60 分钟。
- 工作方式：codex/proxy-remote-controls 独立分支；不改动其他任务的自动化和模型设计草稿。
