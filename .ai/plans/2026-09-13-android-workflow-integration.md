...
- 正式工作流只有编辑、文档 CRUD 和保存；`WorkflowService` 尚无执行入口，catalog 为 `runnable=False`，HTTP schema 也固定 `Literal[False]`。不能仅添加安卓节点图标就宣称可执行。
- [M1 决策](../decisions/2026-09-13-workflows-m1.md) 已确认编辑器实现；[里程碑](../../docs/superpowers/plans/2026-09-13-automation-studio-core-parity-milestones.md) 的开头“独立空窗口”已落后于实际代码。以 M1 决策和源码为准。M2 的真实运行与基础日志仍需设计实施。
- [已批准架构](../../docs/architecture/README.md) 将 Android 列为可选能力，不能阻塞核心应用启动。正式业务仍属于现有 FastAPI sidecar；reference 不作为正式运行依赖。

## 推荐的下一步目标

交付“主系统中的安卓管理 + 一条真实安卓工作流”，继续以 Mac Apple Silicon 为首个平台。管理模块可独立推进；工作流节点与 M2 的通用执行契约一起落地，不另建安卓专用工作流引擎。
...
B 与 M2 设计可并行，但 C 不能依赖一个尚不存在的执行器；A 的结果决定元素节点承诺。上述切片是建议顺序，不改变已有工作流主线的已确认范围。
