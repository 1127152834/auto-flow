...
- 当前阶段：完整目标 active/partial；Android 定向恢复与属性测试 118 passed、1 warning，真实 Mac 新卷恢复及持久属性/链接实验通过；全量后端 3964 passed、26 skipped、全量前端 424 files/5595 tests、脚本 95/95、类型/lint/OpenAPI/结构/build 与 Ruff 已通过。逐项命令与剩余工作以最新 QA 记录为准，不用聚焦通过代表完整交付。
...
| 批次 | 任务 | 范围 | 状态 |
| --- | --- | --- | --- |
| AM1 | T01–T07 | 单实例稳定管理、环境/状态/会话/操作及数据保留 | T01–T03 自动化通过；T04 自动化与同事务投影验证通过；T05 手动/只读原生窗口进程及关闭后实例存活已真实验证；人工输入、切端、完整 HTTP 重启和受控中断仍未验收 |
