...
- 当前实施基线：`codex/android-management-complete@bf4a9de7` 加本次隔离增量；原起点 `a92f0688`。
- 实施分支：`codex/android-management-complete`，隔离 worktree。
- 当前阶段：完整目标 active/partial；Android 定向恢复与属性测试 118 passed、1 warning，真实 Mac 新卷恢复及持久属性/链接实验通过；全量后端 3964 passed、26 skipped。AM1 前端控制/导航增量 47 项聚焦测试、全量 424 files/5613 tests、脚本 95/95、类型/lint/OpenAPI/结构/build 均通过；Ruff 与后端全量以前一 AM4 增量输出为准。逐项命令与剩余工作以最新 QA 记录为准，不用聚焦通过代表完整交付。
...
| 批次 | 任务 | 范围 | 状态 |
| --- | --- | --- | --- |
| AM1 | T01–T07 | 单实例稳定管理、环境/状态/会话/操作及数据保留 | T01–T04 后端自动化通过；T05 原生窗口进程及关闭后实例存活已真实验证，页面迟到响应/旧输入/路由守卫/原生归属/回收核实经 RED→GREEN；详情移除退役分配及 `/runs` 查询。管理操作历史列表、人工输入、切端、完整 HTTP 重启和受控中断仍未验收或实现 |
