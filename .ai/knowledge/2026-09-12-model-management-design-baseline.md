...
- 日期：2026-09-12
- 状态：confirmed（用户要求、源码和现场取证事实、修订原型确认）；设计/实施文件仍为 proposed。
...
- 连接字段修改后的验证更新由旧实现支持，不再把它列为拟增加能力；密钥只返回配置标记已纳入新内部契约方案，沿用同一密码输入与按钮分支，不增加额外操作控件。
- 当前[原型及交互说明](../../docs/prototype/model-management/model-management-interactions.md)包含主页面、供应商流程和模型流程三张同一设计的图片，已获用户确认。正式规格、API 契约与实施计划见[计划索引](../plans/2026-09-12-model-management.md)，业务实现尚未开始。
